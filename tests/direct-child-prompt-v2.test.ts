...
describe("AgentExecutionDecisionV2 direct child prompt contract", () => {
  it("asks the model for V2 actions and marks diagnostics as non-selectable", () => {
    ...
    expect(prompt).toContain("concrete role, definition, does, delegationScope, expectedOutputs, and riskBoundary")
    expect(prompt).toContain("Broad coordination, management, review, or summary ability is weak evidence by itself")
    expect(prompt).toContain("When only weak or generic profile fit exists, choose self_solve with unresolved_reason")
