...
describe("task007 advanced dashboard and settings", () => {
  ...
  it("defines advanced settings around process tabs without multi-AI creation", () => {
    ...
    expect(tabs.map((tab) => tab.label)).toEqual(["AI 연결", "오케스트레이션", "채널", "연장", "메모리", "스케줄", "도구 권한", "백업/배포"])
    ...
    expect(resolveAdvancedSettingsTabFromPath("/advanced/orchestration")).toBe("orchestration")
    expect(resolveAdvancedSettingsTabFromPath("/advanced/extensions")).toBe("yeonjang")
    expect(resolveAdvancedSettingsPath("orchestration")).toBe("/advanced/orchestration")
