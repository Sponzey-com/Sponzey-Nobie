...
PID_FILE="$PIDS_DIR/yeonjang-linux.pid"
...
while (($# > 0)); do
  case "$1" in
    --restart)
      ...
      ;;
    *)
      echo "사용법: bash scripts/start-yeonjang-linux.sh [--restart]"
      exit 1
      ;;
  esac
  shift
...
stop_existing() {
  ...
  local pid
  pid="$(cat "$PID_FILE")"
  echo "기존 Yeonjang Linux GUI를 종료합니다. PID=$pid"
  ...
  echo "기존 Yeonjang Linux GUI가 남아 있어 강제 종료합니다."
...
echo "Yeonjang Linux GUI 빌드를 확인합니다..."
