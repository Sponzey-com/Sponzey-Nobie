...
export type BackupTargetKind =
  ...
  | "vector_db"
  | "setup_state"
  | "logical_sqlite_table"
  | "excluded_path"

export type BackupTargetReason =
  | "required"
  | "optional_missing"
  | "secret_reentry_required"
  | "large_retention_binary"
  | "cache_or_build_output"
  | "transient_runtime"
  | "logical_coverage"
...
export interface BackupSnapshotManifest {
  ...
  secretReentryRequired: Array<{ scope: string; reason: string }>
  checksum: string
}
...
export interface SnapshotVerificationResult {
  ok: boolean
  checked: number
  failures: Array<{ relativePath: string; reason: "missing" | "checksum_mismatch" }>
}

export type RestoreRehearsalCheckName =
  | "manifest_checksum"
  | "file_copy"
  | "sqlite_integrity"
  | "migration_status"
...
export type MigrationPreflightCheckName =
  | "backup_available"
  | "snapshot_checksum"
  | "schema_version"
  | "disk_space"
...
export const MIGRATION_ROLLBACK_RUNBOOK: MigrationRollbackRunbook = {
  id: "migration-rollback-runbook",
...
export function buildBackupTargetInventory(options: BackupSnapshotOptions = {}): BackupTargetInventory {
  ...
  for (const excluded of buildExcludedTargets(stateDir, workDir)) targets.push(excluded)
...
export function createBackupSnapshot(options: BackupSnapshotOptions = {}): BackupSnapshotManifest {
  ...
  if (options.checkpointSqlite !== false) checkpointSqliteDatabase(inventory.dbPath)
  ...
  const manifestWithoutChecksum = {
    kind: "nobie.backup.snapshot" as const,
    version: 1 as const,
    id,
    createdAt: now,
    snapshotDir,
...
export function verifyBackupSnapshotManifest(manifest: BackupSnapshotManifest): SnapshotVerificationResult {
  const expectedManifestChecksum = checksumJson({ ...manifest, checksum: undefined })
  const failures: SnapshotVerificationResult["failures"] = []
  if (expectedManifestChecksum !== manifest.checksum) failures.push({ relativePath: "manifest.json", reason: "checksum_mismatch" })
  for (const file of manifest.files) {
    if (!existsSync(file.snapshotPath)) {
      failures.push({ relativePath: file.relativePath, reason: "missing" })
      continue
    }
    if (sha256File(file.snapshotPath) !== file.checksum) failures.push({ relativePath: file.relativePath, reason: "checksum_mismatch" })
  }
  return { ok: failures.length === 0, checked: manifest.files.length + 1, failures }
}

export function runRestoreRehearsal(options: RestoreRehearsalOptions): RestoreRehearsalReport {
  const restoredDir = resolve(options.restoreDir)
  mkdirSync(restoredDir, { recursive: true })
  const checks: RestoreRehearsalCheck[] = []
  ...
  if (!verification.ok) return buildRestoreReport(options, restoredDir, checks, restoredFiles, undefined, 0)
...
function buildFileTarget(input: Omit<BackupInventoryTarget, "sizeBytes" | "checksum">): BackupInventoryTarget {
  const target: BackupInventoryTarget = { ...input, sourcePath: resolve(input.sourcePath) }
...
function buildExcludedTargets(stateDir: string, workDir: string): BackupInventoryTarget[] {
  const candidates = [
    { id: "exclude:artifacts", sourcePath: join(stateDir, "artifacts"), relativePath: "state/artifacts", reason: "large_retention_binary" as const },
    { id: "exclude:logs", sourcePath: join(stateDir, "logs"), relativePath: "state/logs", reason: "transient_runtime" as const },
    { id: "exclude:cache", sourcePath: join(stateDir, "cache"), relativePath: "state/cache", reason: "cache_or_build_output" as const },
    { id: "exclude:node_modules", sourcePath: join(workDir, "node_modules"), relativePath: "node_modules", reason: "cache_or_build_output" as const },
    { id: "exclude:dist", sourcePath: join(workDir, "packages", "core", "dist"), relativePath: "packages/core/dist", reason: "cache_or_build_output" as const },
  ]
...
function checkpointSqliteDatabase(dbPath: string): void {
...
function checkSqliteIntegrity(dbPath: string): { ok: boolean; message: string } {
  let db: BetterSqlite3.Database | undefined
  try {
    db = new BetterSqlite3(dbPath, { readonly: true, fileMustExist: true })
    const rows = db.prepare<[], { integrity_check: string }>("PRAGMA integrity_check").all()
...
function buildRestoreReport(
  ...
  checks: RestoreRehearsalCheck[],
  restoredFiles: string[],
  migrationStatus: MigrationVersionStatus | undefined,
  promptSourceCount: number,
...
function safeMigrationStatus(dbPath: string): MigrationVersionStatus | undefined {
  try {
...
function summarizePreflightRisk(checks: MigrationPreflightCheck[]): MigrationPreflightRisk {
  if (checks.some((check) => check.risk === "blocking" && !check.ok)) return "blocking"
  if (checks.some((check) => check.risk === "high" && !check.ok)) return "high"
  if (checks.some((check) => check.risk === "medium")) return "medium"
  return "low"
}

export function formatInventoryPathForDisplay(path: string, baseDir: string): string {
