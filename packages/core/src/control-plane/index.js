...
function createSingleConnectionRoutingProfiles(targetId) {
...
function hasConfiguredOpenAIOAuthConnection(config) {
...
function isActiveConnection(config, providerType) {
    return config.ai.connection.provider === providerType;
}
function hasConfiguredConnection(config, providerType) {
    if (!isActiveConnection(config, providerType))
        return false;
    const connection = config.ai.connection;
    if (providerType === "openai") {
        if (connection.auth?.mode === "chatgpt_oauth")
            return hasConfiguredOpenAIOAuthConnection(config);
...
export function buildSetupDraft() {
    ...
    const customBackends = config.ai.connection.provider === "custom"
        ? [{
                ...
                status: config.ai.connection.provider ? "ready" : "disabled",
                summary: "",
                tags: ["general"],
