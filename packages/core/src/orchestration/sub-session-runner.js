...
function recordLateResultNoReplySafely(input) {
    try {
...
function resolveMemoryBootstrapChannelBoundary(input) {
...
function buildPreparedSubSessionMemoryBootstrap(input, now) {
    if (input.memoryBootstrap)
        return input.memoryBootstrap;
...
export class SubSessionRunner {
    ...
    async runSubSession(input, handler) {
        ...
        try {
            ...
            if (subSession.status === "completed" ||
                ...
                await this.dependencies.appendParentEvent(subSession.parentRunId, `sub_session_post_terminal_error:${subSession.subSessionId}:${errorReport.reasonCode}`);
                return {
                    subSession,
                    status: subSession.status,
                    errorReport,
    ...
    async executeWithModelPolicy(input) {
        ...
        try {
            const result = await input.handler({ ...input.input, modelExecutionPolicy: initialSnapshot }, {
