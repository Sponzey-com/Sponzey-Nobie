...
export function planSubSessionExecutionWaves(items, group) {
...
export class SubSessionRunner {
    ...
    async runSubSession(input, handler) {
        ...
        try {
            ...
            const result = await this.runWithTimeout(() => handler(input, {
            ...
            }), controller, input.timeoutMs, (timer) => {
                timeout = timer;
            });
            ...
            this.recordSubSessionLifecycleEvent(subSession, terminalStatus === "failed" ? "sub_session_failed" : "sub_session_completed", terminalStatus === "failed" ? "failed" : "succeeded", `서브 세션 결과를 parent review로 회수했습니다: ${terminalStatus}`, { resultReportId: result.resultReportId, reviewStatus: review.status });
