...
function nodeRole(node) {
    ...
    return (metadataString(metadata.roleName) ??
        metadataString(metadata.role) ??
        metadataString(metadata.title) ??
        node.nodeType);
}
function nodeExecutionCandidate(node) {
...
function activeTopologyRecords(records) {
    return records
...
function selectTopologyGraph(input) {
    ...
    if (!activeRecord || activeRecord.activeVersion === undefined)
        return undefined;
...
function emptyTopologyEnvelope(record) {
    const topology = {
        schemaVersion: 1,
        entityType: "topology",
...
function loadDbConfigRegistrySnapshot(input) {
    if (input.registrySnapshot)
...
function appendEdge(input) {
    ...
    const children = input.directChildren.get(input.edge.parentAgentId) ?? new Set();
...
function appendChildrenRelationMismatchIssues(input) {
    ...
    for (const node of input.topology.nodes) {
        ...
        input.issues.push({
            ...
            severity: "warning",
            message: `Topology node ${node.id} children metadata differs from delegates_to relations. Relations are used as source of truth.`,
...
function cycleEdgeIds(edges) {
    const adjacency = new Map();
    const edgeIdByPair = new Map();
    for (const edge of edges) {
        if (edge.source !== "topology_relation" || !edge.executionCandidate)
            continue;
    ...
    const visit = (nodeId, path, pathSet) => {
        ...
        for (const nextId of nextIds)
            visit(nextId, nextPath, nextPathSet);
    };
...
function appendTopologyEdges(input) {
    ...
    const topologyVersion = input.envelope.version.version;
    const nodeIds = new Set(topology.nodes.filter((node) => node.status !== "archived").map((node) => node.id));
    const incomingChildNodeIds = new Set();
    ...
    for (const node of topology.nodes) {
        if (node.status === "archived" || incomingChildNodeIds.has(node.id))
            continue;
...
function materializeDirectChildren(directChildren) {
    const result = {};
...
function materializeEdgeIndex(edges) {
    const result = {};
    for (const edge of edges) {
...
function finalizeSnapshot(input) {
    ...
    const availableExecutorIds = (directChildAgentIdsByParent[input.currentExecutorId] ?? [])
        .filter((agentId) => input.agentsById[agentId]?.executionCandidate === true
        && input.edges.some((edge) => edge.parentAgentId === input.currentExecutorId
    ...
    return {
        ...
        edges: input.edges.sort((left, right) => left.parentAgentId.localeCompare(right.parentAgentId) ||
...
function buildDbConfigExecutionGraphSnapshot(input) {
    ...
    const edges = [];
    const directChildren = new Map();
    const validationIssues = registrySnapshot.diagnostics.map((diagnostic) => ({
...
//# sourceMappingURL=execution-graph-snapshot.js.map