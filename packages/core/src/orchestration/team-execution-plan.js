...
function hasTeamRole(member, role) {
    const expected = normalizeRole(role);
    return (normalizeRole(member.primaryRole) === expected ||
...
function validationCriteria(kind, required) {
    const common = [
        "result_status_present",
        required ? "required_assignment_completed" : "optional_assignment_reported",
    ];
    if (kind === "member")
        return [...common, "member_output_addresses_role"];
    if (kind === "synthesis")
        return [...common, "synthesis_covers_member_outputs"];
    if (kind === "review")
        return [...common, "review_records_accept_revise_or_block"];
...
function taskSnapshot(input) {
    ...
    const expectedOutputs = [
        outputContract({
            outputId: `${id}:output`,
            description: input.taskKind === "member"
...
function assignment(input) {
    const taskKinds = uniqueStrings(input.tasks.map((task) => task.taskKind));
    ...
    const memberExecutionState = input.member?.executionState === "active" || input.member?.executionState === "fallback"
        ? input.member.executionState
        : undefined;
    const nonMemberTask = input.tasks.find((task) => task.taskKind !== "member");
    const taskExecutionState = nonMemberTask?.taskKind === "synthesis" ||
...
function fallbackReason(coverage, missingAgentId) {
    ...
    return (preferred.find((reason) => primary?.excludedReasonCodes.includes(reason)) ??
        primary?.excludedReasonCodes[0] ??
        "primary_unavailable");
}
function isAllowedFallbackAssignment(coverage, member) {
    if (member.executionState !== "fallback" || !member.fallbackForAgentId)
        return false;
    return ALLOWED_FALLBACK_REASON_CODES.has(fallbackReason(coverage, member.fallbackForAgentId));
}
function isPlanExecutableMember(coverage, member) {
    return member.executionState === "active" || isAllowedFallbackAssignment(coverage, member);
}
function fallbackAssignments(coverage) {
    return coverage.members.flatMap((member) => {
        if (!isAllowedFallbackAssignment(coverage, member) || !member.fallbackForAgentId)
            return [];
        return {
...
function coverageReportSnapshot(input) {
    ...
    const roleProviders = (role) => input.executableMembers
        .filter((member) => hasTeamRole(member, role))
        .map((member) => member.agentId);
    const capabilityProviders = (capability) => input.executableMembers
    ...
    const report = {
        ...
        exclusions: input.coverage.members
            ...
            reasonCodes: member.executionState === "fallback"
...
function diagnosticFromComposition(diagnostic) {
    return {
...
export function buildTeamExecutionPlan(input, dependencies = {}) {
    ...
    const executableMembers = coverage.members.filter((member) => isPlanExecutableMember(coverage, member));
    ...
    const memberTasks = executableMembers.map((member) => taskSnapshot({
        planId,
        taskKind: "member",
        agentId: member.agentId,
        team,
    ...
    const executableLead = team.leadAgentId
        ? executableMembers.find((member) => member.agentId === team.leadAgentId && member.executionState === "active")
        : undefined;
    ...
    const reviewerTasks = reviewerMembers.map((member) => taskSnapshot({
        planId,
        taskKind: "review",
        agentId: member.agentId,
        team,
    ...
    const verifierTasks = verifierMembers.map((member) => taskSnapshot({
        planId,
    ...
    const assignments = [...tasksByAgent.entries()].map(([agentId, tasks]) => assignment({
        agentId,
...
//# sourceMappingURL=team-execution-plan.js.map