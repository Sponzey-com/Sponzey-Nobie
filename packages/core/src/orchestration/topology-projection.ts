...
import {
  ...
  type OrchestrationRegistryDiagnostic,
  type RegistryCoverageDimensionSnapshot,
  type RegistryServiceDependencies,
  type RegistryTeamCoverageSnapshot,
  type RegistryTeamHealthSnapshot,
  type RegistryTeamMemberCoverageSnapshot,
...
function teamNodeId(teamId: string): string {
  return `team:${teamId}`
}

function roleNodeId(teamId: string, agentId: string, primaryRole: string): string {
  return `team-role:${teamId}:${agentId}:${slug(primaryRole)}`
}

function leadNodeId(teamId: string, agentId: string): string {
  return `team-lead:${teamId}:${agentId}`
}

function emptyCoverageDimension(): RegistryCoverageDimensionSnapshot {
  return {
    required: [],
    covered: [],
    missing: [],
...
function nodePosition(
  nodeId: string,
  generated: AgentTopologyPosition,
  layout: AgentTreeLayoutPreference,
...
function diagnosticSeverity(
  value: AgentHierarchyDiagnostic["severity"] | OrchestrationRegistryDiagnostic["severity"],
): AgentTopologyDiagnosticSeverity {
  if (value === "blocked" || value === "invalid") return value
  if (value === "warning") return "warning"
  return "info"
}

function hierarchyDiagnostic(diagnostic: AgentHierarchyDiagnostic): AgentTopologyDiagnostic {
  return {
...
function registryDiagnostic(diagnostic: OrchestrationRegistryDiagnostic): AgentTopologyDiagnostic {
  return {
...
function nodeMetadata(node: RelationshipGraphNode): JsonObject {
  return (node.metadata ?? {}) as JsonObject
}

function numberMetadata(node: RelationshipGraphNode, key: string): number | null {
  const value = nodeMetadata(node)[key]
  return typeof value === "number" && Number.isFinite(value) ? value : null
}

function stringMetadata(node: RelationshipGraphNode, key: string): string | undefined {
  const value = nodeMetadata(node)[key]
  return typeof value === "string" && value.trim().length > 0 ? value : undefined
}

function booleanMetadata(node: RelationshipGraphNode, key: string): boolean | undefined {
  const value = nodeMetadata(node)[key]
...
function agentTopologyNode(input: {
  ...
  if (blockedReason) badges.push(blockedReason)
...
function teamMemberships(team: TeamConfig): TeamMembership[] {
  if (Array.isArray(team.memberships) && team.memberships.length > 0) {
    return team.memberships.map((membership, index) => ({
      ...membership,
      membershipId:
        membership.membershipId || `${team.teamId}:membership:${membership.agentId}:${index}`,
      teamId: team.teamId,
      primaryRole:
        membership.primaryRole || membership.teamRoles[0] || team.roleHints[index] || "member",
      teamRoles: uniqueSorted(
...
function memberCoverageByAgent(
  coverage: RegistryTeamCoverageSnapshot | undefined,
): Map<string, RegistryTeamMemberCoverageSnapshot> {
...
function buildTeamBuilderCandidates(input: {
  ...
  const memberships = new Map(
    teamMemberships(input.team.config).map((item) => [item.agentId, item]),
  )
  const coverageByAgent = memberCoverageByAgent(input.team.coverage)
  return input.agents
    .map((agent) => {
    ...
    .sort((left, right) => {
      if (left.directChild !== right.directChild) return left.directChild ? -1 : 1
...
function buildTeamInspector(input: {
  ...
  const directChildAgentIds = [...(input.directChildrenByParent[ownerAgentId] ?? [])].sort(
...
function teamTopologyNode(input: {
  ...
  if (input.team.config.leadAgentId) badges.push("lead")
  return {
    id,
    kind: "team",
...
function roleBadgeNode(input: {
  teamId: string
  member: AgentTopologyTeamMemberInspector
  index: number
  yOffset: number
  layout: AgentTreeLayoutPreference
}): AgentTopologyNode {
  ...
  return {
    ...
    label:
      input.member.teamRoles.length > 1
...
function membershipEdgeStyle(member: AgentTopologyTeamMemberInspector): AgentTopologyEdgeStyle {
  if (member.reasonCodes.includes("owner_direct_child_required")) return "membership_reference"
  if (!member.directChild || member.executionState === "reference") return "membership_reference"
  if (member.reasonCodes.some((reasonCode) => reasonCode.includes("missing"))) return "invalid"
  return "membership"
}

function teamMembershipEdges(input: {
  team: TeamRegistryEntry
  teamInspector: AgentTopologyTeamInspector
}): AgentTopologyEdge[] {
  ...
  for (const member of input.teamInspector.members) {
    ...
    const diagnostics = member.reasonCodes.map((reasonCode) => ({
      reasonCode,
      severity:
        reasonCode === "member_agent_missing" || reasonCode === "lead_not_active_member"
          ? "invalid"
          : "warning",
      message: `${member.agentId} team membership has ${reasonCode}.`,
      teamId,
      agentId: member.agentId,
    })) satisfies AgentTopologyDiagnostic[]
  ...
  if (input.team.config.leadAgentId) {
    ...
    edges.push({
      ...
      diagnostics: input.teamInspector.diagnostics.filter(
        (diagnostic) => diagnostic.agentId === input.team.config.leadAgentId,
      ),
    })
    edges.push({
...
export function createAgentTopologyService(dependencies: AgentTopologyServiceDependencies = {}) {
  ...
  function buildProjection(): AgentTopologyProjection {
    ...
    const registryDiagnostics = [
      ...
      ...snapshot.teams.flatMap(
        (team) => team.health?.diagnostics ?? team.coverage?.diagnostics ?? [],
      ),
    ].map(registryDiagnostic)
    const diagnostics = [...hierarchyDiagnostics, ...registryDiagnostics]
    const diagnosticsByAgentId = new Map<string, AgentTopologyDiagnostic[]>()
    const diagnosticsByTeamId = new Map<string, AgentTopologyDiagnostic[]>()
    ...
    const maxAgentY = agentNodes.reduce(
      (max, node) => Math.max(max, node.position.y),
      agentNodes.length > 0 ? 80 : 0,
    )
    const teamYOffset = maxAgentY + 190
    const roleYOffset = teamYOffset
    const inspectors: AgentTopologyProjection["inspectors"] = {
      agents: {},
      teams: {},
    }
    ...
    const teamNodes: AgentTopologyNode[] = []
    const roleNodes: AgentTopologyNode[] = []
    const leadNodes: AgentTopologyNode[] = []
    const membershipEdges: AgentTopologyEdge[] = []
    ...
    const hierarchyEdges = tree.edges.map(
  ...
  function validateEdge(
    input: AgentTopologyEdgeValidationInput,
    ...
    const diagnostics: AgentTopologyDiagnostic[] = candidate
    ...
    if (wantsActive && !candidate?.directChild) {
      diagnostics.unshift(
        directChildDiagnostic({ teamId: team.teamId, agentId, ownerAgentId: team.ownerAgentId }),
      )
    }
    return {
      ok: true,
  ...
  function validateActiveTeamMembers(team: TeamConfig): AgentTopologyEdgeValidationResult {
    const projection = buildProjection()
    ...
    const diagnostics: AgentTopologyDiagnostic[] = []
