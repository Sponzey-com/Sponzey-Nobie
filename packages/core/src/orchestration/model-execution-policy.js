...
export const DEFAULT_PROVIDER_MODEL_CAPABILITY_MATRIX = [
    {
        providerId: "openai",
        modelId: "gpt-5.4",
        available: true,
...
function findModelCapability(matrix, providerId, modelId) {
    const provider = normalize(providerId);
    const model = normalize(modelId);
    return matrix.find((entry) => normalize(entry.providerId) === provider &&
        (normalize(entry.modelId) === model || entry.modelId === "*"));
}
function findDoctorSnapshot(doctor, providerId, modelId) {
    const rows = Array.isArray(doctor) ? doctor : doctor ? [doctor] : [];
    const provider = normalize(providerId);
    const model = normalize(modelId);
    return rows.find((entry) => normalize(entry.providerId) === provider && normalize(entry.modelId) === model);
}
export function estimateModelExecutionCost(input) {
    return Number(((Math.max(0, input.inputTokens) / 1000) * input.capability.inputCostPer1kTokens +
        (Math.max(0, input.outputTokens) / 1000) * input.capability.outputCostPer1kTokens).toFixed(6));
}
...
function buildSnapshot(input) {
    ...
    return {
        ...
        maxOutputTokens: Math.min(input.profile.maxOutputTokens ?? input.active.maxOutputTokens, input.active.maxOutputTokens),
...
function resolveActiveModel(input) {
    ...
    const fallback = fallbackModelId && fallbackModelId !== input.profile.modelId
        ? findModelCapability(input.matrix, input.profile.providerId, fallbackModelId)
        : undefined;
    const primaryDoctor = findDoctorSnapshot(input.doctor, input.profile.providerId, input.profile.modelId);
    const primaryReasonCodes = unique([
        ...(primary?.reasonCodes ?? []),
        ...(primaryDoctor?.reasonCodes ?? []),
    ]);
    const primaryBlockedReason = input.forceFallbackReasonCode ??
        (!primary
            ? "model_not_supported"
            : !primary.available
                ? "model_unavailable"
    ...
    if (primaryBlockedReason && fallback?.available && fallbackModelId) {
        const fallbackDoctor = findDoctorSnapshot(input.doctor, input.profile.providerId, fallbackModelId);
    ...
    const profile = fallbackApplied && input.profile.fallbackModelId
    ...
    if (snapshot.costBudget !== undefined &&
...
export function resolveModelExecutionPolicy(input) {
    ...
    const estimatedOutputTokens = input.estimatedOutputTokens ?? Math.max(1, Math.ceil((profile.maxOutputTokens ?? 1024) * 0.25));
...
export function resolveFallbackModelExecutionPolicy(input) {
    ...
    if (!profile?.fallbackModelId || input.current.snapshot?.fallbackApplied)
    ...
    return {
        ...
        estimatedCost: estimateModelExecutionCost({
            capability: {
                ...
                inputCostPer1kTokens: input.snapshot.estimatedInputTokens > 0
...
//# sourceMappingURL=model-execution-policy.js.map