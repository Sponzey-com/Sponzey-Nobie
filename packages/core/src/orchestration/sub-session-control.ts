...
function sanitizeDetail(value: unknown, depth = 0): unknown {
  ...
  if (typeof value === "string") return sanitizeSubSessionControlText(value)
...
function parseSubSessionRow(row: DbRunSubSession | undefined): SubSessionContract | undefined {
...
function parseSpawnBody(body: unknown): {
  ...
  if (!input) return { approvalRequired: false, error: "invalid_spawn_request" }
  const commandValidation = validateCommandRequest(input.command)
  if (!commandValidation.ok) return { approvalRequired: false, error: "invalid_command_request" }
  ...
  return {
    ...
    approvalRequired:
      envelope.approvalRequired === true ||
...
function recordSpawnMetric(ack: SubSessionSpawnAck): void {
  recordLatencyMetric({
...
function appendTimeline(parentRunId: string, label: string): void {
  if (!getRootRun(parentRunId)) return
...
export function spawnSubSessionAck(body: unknown): SubSessionSpawnAck {
  ...
  const existing = parseSubSessionRow(
    getRunSubSessionByIdempotencyKey(input.command.identity.idempotencyKey),
  )
  if (existing) {
    return makeAck({
      startedAt,
...
export function requireSubSessionAccess(
  subSessionId: string,
  expectedParentRunId?: string,
):
  | { ok: true; subSession: SubSessionContract }
  | { ok: false; statusCode: 403 | 404; reasonCode: string } {
  const subSession = parseSubSessionRow(getRunSubSession(subSessionId))
  if (!subSession) return { ok: false, statusCode: 404, reasonCode: "sub_session_not_found" }
...
export function getSubSessionInfo(
  subSessionId: string,
  expectedParentRunId?: string,
): { ok: true; info: SubSessionInfo } | { ok: false; statusCode: 403 | 404; reasonCode: string } {
...
function runEventLogs(
  ...
  if (!getRootRun(parentRunId)) return []
  return getDb()
    .prepare<[string, string, number], { id: string; at: number; label: string }>(
      `SELECT id, at, label
       FROM run_events
...
function controlEventLogs(
  ...
  return getDb()
    ...
    >(
      `SELECT id, created_at, event_type, summary, detail_json
       FROM control_events
...
export function listSubSessionLogs(input: {
  ...
  | { ok: true; logs: SubSessionLogEntry[] }
  | { ok: false; statusCode: 403 | 404; reasonCode: string } {
...
function controlTextFor(action: SubSessionControlAction, body: unknown): string {
...
function feedbackLoopResult(
  subSession: SubSessionContract,
  action: Extract<SubSessionControlAction, "feedback" | "redelegate">,
  body: unknown,
  auditCorrelationId?: string,
): SubSessionControlResult | { ok: false; statusCode: 409; reasonCode: string } {
  ...
  if (continuation.action === "limited_success_finalized") {
    ...
    appendTimeline(
      ...
      `sub_session_limited_success_finalized:${subSession.subSessionId}`,
    )
    return {
      ok: true,
      accepted: true,
  ...
  const feedbackPackage = buildFeedbackLoopPackage({
    ...
    retryBudgetRemaining:
      review.feedbackRequest?.retryBudgetRemaining ??
  ...
  appendTimeline(
    subSession.parentRunId,
    `sub_session_${action}_feedback_created:${subSession.subSessionId}:${feedbackPackage.feedbackRequest.feedbackRequestId}`,
  )
  return {
    ok: true,
    accepted: true,
...
function parseControlResultReport(
  value: unknown,
  subSession: SubSessionContract,
): { ok: true; resultReport: ResultReport } | { ok: false; reasonCode: string } {
  if (!value) return { ok: false, reasonCode: "result_report_required_for_feedback_loop" }
  const validation = validateResultReport(value)
  if (!validation.ok) return { ok: false, reasonCode: "invalid_result_report" }
...
function expectedOutputsForSubSession(subSession: SubSessionContract) {
...
function directChildAgentIds(parentAgentId: string): string[] {
  return listAgentRelationships({ parentAgentId, status: "active" }).map(
    (relationship) => relationship.child_agent_id,
  )
}
...
export function controlSubSession(input: {
  ...
  body?: unknown
  parentRunId?: string
}): SubSessionControlResult | { ok: false; statusCode: 403 | 404 | 409; reasonCode: string } {
  const access = requireSubSessionAccess(input.subSessionId, input.parentRunId)
  if (!access.ok) return access
  const subSession = access.subSession
  const record = isRecord(input.body) ? input.body : {}
  const auditCorrelationId =
    trimmedString(record.auditCorrelationId) ?? subSession.identity.auditCorrelationId
  ...
  if (input.action === "retry") {
    if (
      !TERMINAL_STATUSES.has(subSession.status) ||
...
export function killAllSubSessionsForRun(input: {
  ...
  for (const row of listRunSubSessionsForParentRun(input.parentRunId)) {
    ...
    if (!subSession || !ACTIVE_CONTROL_STATUSES.has(subSession.status)) continue
    if (!canTransitionSubSessionStatus(subSession.status, "cancelled")) continue
    transitionSubSessionStatus(subSession, "cancelled", nowMs())
    updateRunSubSession(subSession)
