...
import { getOrchestrationEventByDedupeKey, getOrchestrationEventById, insertAuditLog, insertDiagnosticEvent, insertOrchestrationEvent, listOrchestrationEvents, } from "../db/index.js";
...
function correlationIdFor(input) {
    return (trimOrNull(input.correlationId) ??
...
function dedupeKeyFor(input, payload) {
...
export function parseOrchestrationReplayCursor(cursor) {
...
export function validateOrchestrationEventInput(input) {
    const issueCodes = [];
    if (!EVENT_KIND_SET.has(input.eventKind))
        issueCodes.push("event_kind_invalid");
    if (!input.summary.trim())
        issueCodes.push("event_summary_missing");
    if (!correlationIdFor(input).trim())
        issueCodes.push("event_correlation_missing");
...
export function recordOrchestrationEvent(input) {
    ...
    if (duplicate)
        return { event: eventFromRow(duplicate), inserted: false };
    const event = eventFromRow(insertOrchestrationEvent({
...
export function listOrchestrationEventLedger(query = {}) {
...
function updateAgentCount(map, id, eventKind) {
    if (!id)
        return;
...
function updateTeamCount(map, id, eventKind) {
    if (!id)
        return;
...
function updateExchangeCount(map, id, eventKind) {
    if (!id)
        return;
...
export function buildOrchestrationMonitoringSnapshot(query = {}) {
    ...
    for (const event of events) {
        ...
        if (event.payload.duplicateSuppressed === true)
            duplicateSuppressedCount += 1;
        if (event.subSessionId) {
            const previous = subSessions.get(event.subSessionId);
            const status = event.eventKind === "sub_session_completed"
                ? "completed"
        ...
        if (event.eventKind === "model_resolved" || event.eventKind === "model_fallback") {
            models.push({
                ...
                fallbackApplied: event.eventKind === "model_fallback" || event.payload.fallbackApplied === true,
            });
        }
        if (event.eventKind === "resource_lock_wait" ||
    ...
    const snapshot = {
        ...
        summary: {
            total: events.length,
            activeSubSessionCount: subSessionValues.filter((item) => item.status === "queued" || item.status === "running").length,
            completedSubSessionCount: subSessionValues.filter((item) => item.status === "completed")
                .length,
            failedSubSessionCount: subSessionValues.filter((item) => item.status === "failed" || item.status === "cancelled").length,
...
export function buildRestartResumeProjection(query = {}) {
    const snapshot = buildOrchestrationMonitoringSnapshot(query);
    const activeSubSessions = snapshot.subSessions.filter((item) => item.status === "queued" || item.status === "running");
    return {
...
export function formatOrchestrationEventSse(event) {
    return [
        `id: ${event.cursor}`,
...
export function openOrchestrationEventRawPayload(input) {
    ...
    if (!row)
        return { ok: false, reasonCode: "orchestration_event_not_found" };
...
export function installOrchestrationEventProjection() {
...
export function resetOrchestrationEventProjectionForTest() {
    for (const unsubscribe of projectionUnsubscribers)
        unsubscribe();
    projectionUnsubscribers = [];
