...
import { CONTRACT_SCHEMA_VERSION, type JsonObject } from "../contracts/index.js"
import type {
  AgentPromptBundle,
  CommandRequest,
  DataExchangePackage,
...
import {
  type SubSessionFeedbackCycleDirective,
...
export type FeedbackLoopContinuationAction =
  | "feedback_request"
...
export function validateRedelegationTarget(
  input: RedelegationTargetValidationInput,
): RedelegationTargetValidationResult {
  const reasonCodes: string[] = []
  if (input.policy !== "alternative_direct_child") {
    return { ok: true, reasonCodes }
  }
  if (!input.targetAgentId) reasonCodes.push("redelegation_target_required")
  ...
  if (input.permissionAllowed === false) reasonCodes.push("redelegation_permission_blocked")
  if (input.capabilityAllowed === false) reasonCodes.push("redelegation_capability_blocked")
  if (input.modelAvailable === false) reasonCodes.push("redelegation_model_unavailable")
  if (input.resourceLocksAvailable === false) reasonCodes.push("redelegation_resource_lock_blocked")
  return {
...
export function buildFeedbackLoopPackage(
  input: BuildFeedbackLoopPackageInput,
): FeedbackLoopPackage {
  ...
  const previousSubSessionIds = unique(
    input.previousSubSessionIds ?? input.resultReports.map((report) => report.subSessionId),
  )
...
export function buildRedelegatedSubSessionInput(
  input: BuildRedelegatedSubSessionInput,
): RunSubSessionInput {
  const idProvider = input.idProvider ?? (() => randomUUID())
  ...
  return {
    ...
    memoryBootstrap: buildChildOwnMemoryBootstrap({
      ...
      ...(input.feedbackRequest.synthesizedContextExchangeId
        ...
        : {}),
      additionalPinnedItems: buildSubSessionFeedbackPinnedItems(
        buildSubSessionFeedbackCapsulePayload({
          resultReports: [],
          ...
          expectedOutputRevision: input.feedbackRequest.expectedRevisionOutputs.map(
...
function collectCarryForwardOutputs(
  resultReports: ResultReport[],
): FeedbackRequest["carryForwardOutputs"] {
  const byOutputId = new Map<string, FeedbackRequest["carryForwardOutputs"][number]>()
  for (const report of resultReports) {
    for (const output of report.outputs) {
      if (output.status === "missing") continue
      const next = {
        outputId: output.outputId,
        status: output.status === "partial" ? ("partial" as const) : ("satisfied" as const),
...
function impossibleReasonPayload(
  value: SubAgentResultReview["impossibleReason"],
): JsonObject | null {
