...
function defaultParentAgentId(parentAgentId) {
    return parentAgentId?.trim() || DEFAULT_PARENT_AGENT_ID;
}
function directChildIdsFor(registry, parentAgentId) {
...
function agentBlockedReasonCodes(registry, parentAgentId, agentId) {
...
function validateAgentFocus(registry, parentAgentId, target) {
...
function validateTeamFocus(registry, parentAgentId, target) {
...
function validateSubSessionFocus(registry, parentAgentId, target) {
    ...
    if (!agentValidation.ok) {
        return {
            ...agentValidation,
            reasonCode: agentValidation.reasonCode === "focus_target_not_direct_child"
...
function bindValidationResult(validation, binding) {
    if (!validation.ok)
...
function safePermissionProfile(profileId, riskCeiling) {
    return {
        profileId,
        riskCeiling,
...
function memoryPolicyFor(agentId) {
    return {
...
function templateAgentConfig(template, overrides) {
    ...
    return {
        ...
        capabilityPolicy: {
            permissionProfile: safePermissionProfile(`profile:${slug(template.templateId)}:draft`, template.riskCeiling),
...
export function instantiateAgentTemplate(input) {
    ...
    if (!template)
        return { ok: false, reasonCode: "agent_template_not_found" };
...
function teamMembership(teamId, agentId, role, index) {
    return {
...
function templateTeamConfig(template, overrides) {
    const now = nowMs();
    const displayName = asString(overrides.displayName) ?? template.displayName;
    const teamId = asString(overrides.teamId) ?? `team:template:${slug(template.templateId)}`;
    const memberAgentIds = asStringArray(overrides.memberAgentIds) ??
    ...
    return {
        ...
        memberships: memberAgentIds.map((agentId, index) => teamMembership(teamId, agentId, roleHints[index] ?? "member", index)),
        memberAgentIds,
        roleHints,
        profileVersion: 1,
        createdAt: now,
...
export function instantiateTeamTemplate(input) {
    ...
    if (!template)
        return { ok: false, reasonCode: "team_template_not_found" };
    const overrides = isRecord(input.overrides) ? input.overrides : {};
...
function importedName(raw) {
    if (!isRecord(raw))
        return "Imported Agent";
    return (asString(raw.displayName) ??
        asString(raw.name) ??
        asString(raw.agentName) ??
        asString(raw.role) ??
        "Imported Agent");
}
function importedDescription(raw) {
    if (!isRecord(raw))
        return "Imported external profile.";
    return (asString(raw.description) ??
...
export function lintAgentDescription(description) {
...
function commandTail(commandText, consumedParts) {
...
function parseFocusTarget(value) {
    const raw = value?.trim();
    if (!raw)
        return undefined;
    const [prefix, ...rest] = raw.split(":");
    const suffix = rest.join(":");
    if (prefix === "agent" && suffix)
        return { kind: "agent", id: suffix.startsWith("agent:") ? suffix : `agent:${suffix}` };
    if (prefix === "team" && suffix)
        return { kind: "team", id: suffix.startsWith("team:") ? suffix : `team:${suffix}` };
    ...
    if (raw.startsWith("team:"))
        return { kind: "team", id: raw };
    if (raw.startsWith("sub:"))
        return { kind: "sub_session", id: raw };
    return undefined;
}
function subSessionListForCommand(parentRunId) {
    if (parentRunId) {
...
export function createOneClickBackgroundTask(input) {
    ...
    if (!message)
        return { ok: false, reasonCode: "background_task_message_required", statusCode: 400 };
    if (!parentRunId)
        return { ok: false, reasonCode: "background_task_parent_run_required", statusCode: 400 };
    if (!targetAgentId)
        return { ok: false, reasonCode: "background_task_target_agent_required", statusCode: 400 };
