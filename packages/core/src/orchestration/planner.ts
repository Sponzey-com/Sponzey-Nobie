...
export function classifyFastPath(input: FastPathClassifierInput): FastPathClassificationResult {
  ...
  const classification: FastPathClassification = "delegation_candidate"
  const reasonCodes = [
...
function inferredRequiredCapabilitiesForScope(_scope: StructuredTaskScope): string[] {
  return []
}
...
function plannerCurrentExecutorId(input: Pick<OrchestrationPlannerInput, "parentAgentId">): string {
  return input.parentAgentId ?? "agent:nobie"
}

function isRootNobieExecutor(executorId: string | undefined): boolean {
...
function directFallbackPlan(input: {
  ...
  fallbackMode?: OrchestrationPlan["fallbackStrategy"]["mode"]
...
function selectedExecutorFromDelegatingDecision(
  decision: AgentExecutionDecision | undefined,
): string | undefined {
  if (!decision?.selected_executor_id?.trim()) return undefined
  if (
    decision.execution_route === "delegate_to_child" ||
...
function directFallbackModeForExecutionDecision(
  decision: AgentExecutionDecision,
): OrchestrationPlan["fallbackStrategy"]["mode"] {
  if (decision.execution_route === "ask_user") return "ask_user"
  if (decision.execution_route === "ask_parent") return "ask_parent"
  if (decision.execution_route === "return_to_parent") return "return_to_parent"
  if (decision.execution_route === "root_nobie_direct") return "root_nobie_direct"
  if (decision.execution_route === "explicit_provider") return "ask_user"
  if (decision.execution_route === "explicit_provider_target") return "ask_user"
  if (decision.execution_route === "boundary_failure") return "ask_user"
  if (decision.execution_route === "self_solve") return "self_solve"
...
export function buildOrchestrationPlan(
  ...
  if (input.agentExecutionDecision && !decisionSelectedExecutorId) {
    return directFallbackPlan({
      ...
      fallbackMode:
        input.agentExecutionDecision.execution_route === "root_nobie_direct" &&
