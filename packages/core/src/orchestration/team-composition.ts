...
export type TeamMemberExecutionState =
  | "active"
  | "reference"
  | "unresolved"
  | "excluded"
...
interface AgentCompositionSummary {
  agentId: string
  status: AgentConfig["status"]
  specialtyTags: string[]
  permissionProfile?: PermissionProfile
  skillMcpAllowlist?: SkillMcpAllowlist
  load?: AgentRegistryEntry["currentLoad"]
}
...
function persistedMembershipStatusByAgent(
  teamId: string,
): Map<string, TeamMembership["status"] | "unresolved"> {
  const result = new Map<string, TeamMembership["status"] | "unresolved">()
...
function markCoverageProvidedByOwnerLead(
  coverage: TeamCoverageDimension,
  ownerAgentId: string,
  leadAgentId: string | undefined,
): TeamCoverageDimension {
  if (leadAgentId !== ownerAgentId || !coverage.required.includes("lead")) return coverage
  const leadProviders = uniqueStrings([...(coverage.providers.lead ?? []), ownerAgentId]).sort(
    (left, right) => left.localeCompare(right),
  )
  const covered = uniqueStrings([...coverage.covered, "lead"]).sort((left, right) =>
...
function subset(left: string[] | undefined, right: string[] | undefined): boolean {
  const rightSet = new Set(right ?? [])
  return (left ?? []).every((value) => rightSet.has(value))
}

function broaderFallbackReason(
  fallback: AgentCompositionSummary | undefined,
  primary: AgentCompositionSummary | undefined,
): string | undefined {
  if (!fallback || !primary || !fallback.permissionProfile || !primary.permissionProfile)
    return undefined
  if (
    CAPABILITY_RISK_ORDER[fallback.permissionProfile.riskCeiling] >
...
function isAgentOverloaded(agent: AgentCompositionSummary | undefined): boolean {
  return Boolean(agent?.load && agent.load.utilization >= 1)
}

function healthFromCoverage(coverage: TeamCoverageReport): TeamHealthReport {
...
export function createTeamCompositionService(
  ...
  function agentSummaries(): Map<string, AgentCompositionSummary> {
    ...
    for (const agent of agentRegistry().list()) {
      if (agent.agentType !== "sub_agent") continue
  ...
  function directChildIdsFor(ownerAgentId: string): Set<string> {
  ...
  function evaluateTeam(
    team: TeamConfig,
    options: { usePersistedMembershipStatuses: boolean },
    ...
    const persistedStatuses = options.usePersistedMembershipStatuses
      ...
      : new Map<string, TeamMembership["status"] | "unresolved">()
    const diagnostics: TeamCompositionDiagnostic[] = []
    ...
    const primaryMembershipByAgent = new Map(
      rawMemberships.map((membership) => [membership.agentId, membership]),
    )
    const memberCoverages: TeamCompositionMemberCoverage[] = rawMemberships.map((membership) => {
      ...
      if (!agent) excludedReasonCodes.push("member_agent_missing")
      if (membershipStatus === "unresolved") excludedReasonCodes.push("member_unresolved")
      if (membershipStatus === "removed") excludedReasonCodes.push("membership_removed")
      if (membershipStatus === "inactive") excludedReasonCodes.push("membership_inactive")
      if (owner && owner.status !== "enabled") excludedReasonCodes.push("team_owner_unavailable")
      if (agent && agent.status !== "enabled") excludedReasonCodes.push(`member_${agent.status}`)
      if (isAgentOverloaded(agent)) excludedReasonCodes.push("member_overloaded")
      if (!directChild) excludedReasonCodes.push("owner_direct_child_required")
      ...
      const fallbackBroaderReason = membership.fallbackForAgentId
        ...
        : undefined
      if (fallbackBroaderReason) excludedReasonCodes.push(fallbackBroaderReason)
      ...
      if (membershipStatus === "fallback_only") {
        if (!membership.fallbackForAgentId) excludedReasonCodes.push("fallback_primary_missing")
        else if (!primaryUnavailable) excludedReasonCodes.push("fallback_primary_available")
      }
      ...
      const fallbackCandidate =
        membershipStatus === "fallback_only" &&
      ...
      let executionState: TeamMemberExecutionState = "excluded"
      if (!agent || membershipStatus === "unresolved") executionState = "unresolved"
      else if (!directChild) executionState = "reference"
      else if (fallbackCandidate) executionState = "fallback"
    ...
    for (const member of memberCoverages) {
      ...
      if (
        member.excludedReasonCodes.includes("member_disabled") ||
      ...
      const fallbackBroaderReason = member.excludedReasonCodes.find(
        (reason) => reason.startsWith("fallback_") && reason.endsWith("_broader"),
      )
    ...
    const roleCoverage = markCoverageProvidedByOwnerLead(
      roleProviders(executableMembers, uniqueStrings(team.requiredTeamRoles ?? [])),
      ownerAgentId,
      team.leadAgentId,
    )
    const capabilityCoverage = capabilityProviders(
      executableMembers,
      uniqueStrings(team.requiredCapabilityTags ?? []),
    )
    ...
    const coverageWithoutHealth: Omit<TeamCoverageReport, "executionCandidate"> = {
      teamId: team.teamId,
      ownerAgentId,
    ...
    const coverage: TeamCoverageReport = {
  ...
  function resolveTeam(input: string | unknown): TeamCompositionValidationResult {
