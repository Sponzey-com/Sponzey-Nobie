import type { TaskIntentEnvelope } from "../agent/intake.js"
import { CONTRACT_SCHEMA_VERSION, type DeliveryContract, type IntentContract, type ToolTargetContract } from "./index.js"

function mapIntentType(value: TaskIntentEnvelope["intent_type"]): IntentContract["intentType"] {
  switch (value) {
...
function inferActionType(envelope: TaskIntentEnvelope): IntentContract["actionType"] {
  if (envelope.intent_type === "schedule_request") {
    if (envelope.schedule_spec.status === "needs_clarification") return "ask_user"
    if (envelope.schedule_spec.detected) return "create_schedule"
  }
