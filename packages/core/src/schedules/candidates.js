import { buildDeliveryKey, buildPayloadHash, buildScheduleIdentityKey, } from "../contracts/index.js";
import { getDb, getSchedule } from "../db/index.js";
...
export function parseScheduleContractJson(value) {
    if (!value)
        return null;
    try {
...
export function scheduleContractTimeEquals(a, b) {
...
export function scheduleContractDestinationEquals(a, b) {
...
function activeClause(includeDisabled) {
    return includeDisabled ? "" : "AND enabled = 1";
}
function sessionClause(sessionId) {
...
function querySchedulesByColumn(params) {
    ...
    return getDb()
       ...
       LIMIT ?`)
        .all(params.value, ...session.args, params.limit);
}
function addCandidate(map, spec) {
    ...
    if (existing.contract == null && spec.contract != null)
        existing.contract = spec.contract;
}
export function findScheduleCandidatesByContract(input) {
    ...
    for (const schedule of querySchedulesByColumn({
        ...
        if (!contract || !scheduleContractTimeEquals(input.contract, contract))
            continue;
        addCandidate(candidates, {
            schedule,
            contract,
    ...
    for (const schedule of querySchedulesByColumn({
        ...
        const contract = parseScheduleContractJson(schedule.contract_json);
        if (!contract || !scheduleContractDestinationEquals(input.contract, contract))
            continue;
        addCandidate(candidates, {
            schedule,
            contract,
