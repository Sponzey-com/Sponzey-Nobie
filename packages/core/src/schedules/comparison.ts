...
export type ScheduleContractComparisonReasonCode =
...
export function parseScheduleContractComparisonResult(
  raw: string,
  ...
  try {
    const parsed = JSON.parse(json) as ParsedComparisonResult
...
function normalizeReasonCode(
  ...
  if (
    value === "same_schedule_identity"
    || value === "different_payload"
    || value === "different_time"
    || value === "different_destination"
    || value === "target_ambiguous"
  ) return value
  if (decision === "same") return "same_schedule_identity"
  if (decision === "different") return "different_payload"
...
export async function compareScheduleContractsWithAI(params: {
  ...
  return parseScheduleContractComparisonResult(
    raw,
