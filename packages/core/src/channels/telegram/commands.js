export function registerCommands(bot, channel) {
    bot.command("start", async (ctx) => {
        const name = ctx.from?.first_name ?? "there";
        await ctx.reply(`👋 Hello, ${name}! I'm 스폰지 노비 · Sponzey Nobie, your personal AI assistant.\n\n` +
    ...
    bot.command("status", async (ctx) => {
        ...
        await ctx.reply(`📊 *Session Status*\n\n` +
            ...
            `Active Tasks: ${channel.getRunningCount()}`, { parse_mode: "Markdown" });
    });
    bot.command("help", async (ctx) => {
        await ctx.reply(`🤖 *스폰지 노비 · Sponzey Nobie Commands*\n\n` +
            ...
            `Just send any text message to chat with the AI!`, { parse_mode: "Markdown" });
    });
}
