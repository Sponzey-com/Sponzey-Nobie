...
interface TelegramMessageEntityPayload {
  type?: string
  offset?: number
  length?: number
...
interface TelegramMessagePayload {
  ...
  entities?: TelegramMessageEntityPayload[]
  caption_entities?: TelegramMessageEntityPayload[]
...
export function buildTelegramCapabilityManifest(): ChannelCapabilities {
...
export function validateTelegramWebhookSecretToken(input: {
  ...
  if (!configured) return { valid: false, reason: "missing_configured_secret" }
  if (!received) return { valid: false, reason: "missing_received_secret" }
  if (configured !== received) return { valid: false, reason: "mismatch" }
...
export function normalizeTelegramInboundUpdate(
  ...
  if (!message?.message_id || !message.chat?.id || !message.from?.id) return []
...
export function normalizeTelegramInteractionUpdate(
  rawPayload: unknown,
  options: {
    channelId?: string
    connectionId?: string
  ...
  if (!callback?.id || !callback.from?.id) return []
...
export function buildTelegramContinuationLookupCandidate(
  envelope: InboundEnvelope,
  options: { lookupWindowMs?: number } = {},
): TelegramContinuationLookupCandidate | null {
  if (envelope.provider !== "telegram" || !envelope.replyToMessageId || !envelope.room?.id) return null
  return {
    provider: "telegram",
...
export function createTelegramChannelAdapter(options: TelegramChannelAdapterOptions = {}): ChannelAdapter {
  return defineChannelAdapter(new TelegramChannelAdapter(options))
}

function asTelegramUpdate(rawPayload: unknown): TelegramUpdatePayload {
  if (!isRecord(rawPayload)) return {}
  return rawPayload as TelegramUpdatePayload
}

function normalizeTelegramUser(user: TelegramUserPayload): ChannelIdentity {
...
function normalizeTelegramRoom(chat: TelegramChatPayload): ChannelRoom {
...
function normalizeTelegramRoomType(type: string | undefined): NonNullable<ChannelRoom["type"]> {
...
function normalizeTelegramMessageText(message: TelegramMessagePayload): string {
...
function normalizeTelegramAttachments(message: TelegramMessagePayload): ChannelAttachment[] {
...
function selectLargestTelegramPhoto(photos: TelegramPhotoPayload[] | undefined): TelegramPhotoPayload | undefined {
  if (!photos || photos.length === 0) return undefined
  return photos.reduce<TelegramPhotoPayload | undefined>((largest, photo) => {
    if (!largest) return photo
    return (photo.file_size ?? 0) > (largest.file_size ?? 0) ? photo : largest
  }, undefined)
}

function resolveAttachmentKind(mimeType: string | undefined): ChannelAttachment["kind"] {
...
function resolveTelegramTimestamp(message: TelegramMessagePayload, now: (() => number) | undefined): number {
  return typeof message.date === "number" ? message.date * 1000 : now?.() ?? Date.now()
}

function parseTelegramCallbackData(data: string | undefined): {
  kind: InteractionEnvelope["kind"]
  actionId?: string
  approvalDecision?: InteractionEnvelope["approvalDecision"]
  correlationId?: string
} {
  if (!data) return { kind: "choose_option" }
  ...
  if (scope === "cancel") return { kind: "cancel", actionId: actionId || scope, ...(correlationId ? { correlationId } : {}) }
  if (scope === "retry") return { kind: "retry", actionId: actionId || scope, ...(correlationId ? { correlationId } : {}) }
...
function displayNameFromUser(user: TelegramUserPayload): string | undefined {
  return [user.first_name, user.last_name].filter(Boolean).join(" ").trim() || user.username
}

function displayNameFromChat(chat: TelegramChatPayload): string | undefined {
