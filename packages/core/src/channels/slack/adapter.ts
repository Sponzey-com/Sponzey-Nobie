...
interface SlackInteractionPayload {
  ...
  actions?: SlackBlockActionPayload[]
}
...
export function buildSlackCapabilityManifest(): ChannelCapabilities {
...
export function normalizeSlackInboundEvent(
  ...
  if (event.user && options.botUserId && event.user === options.botUserId) return []
  if (eventType === "message" && event.subtype && event.subtype !== "file_share") return []
  ...
  return [{
    ...
    ...(isThreadReply && event.thread_ts
      ? { continuationContext: { parentMessageId: event.thread_ts, source: "thread" as const } }
      : {}),
    dedupeKey: `slack:${roomId}:${messageId}`,
  }]
}

export function normalizeSlackInteractionPayload(
  rawPayload: unknown,
  options: {
    channelId?: string
    connectionId?: string
  ...
  if (!rawActionId || !value || !userId) return []
...
export function buildSlackContinuationLookupCandidate(
  envelope: InboundEnvelope,
  options: { lookupWindowMs?: number; teamId?: string } = {},
): SlackContinuationLookupCandidate | null {
  if (envelope.provider !== "slack" || !envelope.room?.id) return null
...
export class SlackChannelAdapter implements ChannelAdapter {
  ...
  private channel: SlackChannel | null = null
  ...
  async healthCheck(): Promise<ChannelHealthCheck> {
    ...
    if (runtime.isRunning) return { status: "healthy", checkedAt: this.now(), message: "Runtime is running." }
    if (runtime.lastError) return { status: "failed", checkedAt: this.now(), message: runtime.lastError }
  ...
  async normalizeInbound(rawPayload: unknown): Promise<InboundEnvelope[]> {
    const options: Parameters<typeof normalizeSlackInboundEvent>[1] = {
      channelId: this.channelId,
      connectionId: this.connectionId,
      now: this.now,
    }
    if (this.botUserId !== undefined) options.botUserId = this.botUserId
    if (this.botDisplayName !== undefined) options.botDisplayName = this.botDisplayName
...
export function createSlackChannelAdapter(options: SlackChannelAdapterOptions = {}): ChannelAdapter {
  return defineChannelAdapter(new SlackChannelAdapter(options))
}

function asSlackEventsApiPayload(rawPayload: unknown): SlackEventsApiPayload {
  const record = asRecord(rawPayload)
  const socketPayload = asRecord(record.payload)
  if (isSlackEventsPayload(socketPayload)) return socketPayload as SlackEventsApiPayload
  if (isSlackEventsPayload(record)) return record as SlackEventsApiPayload
  return {}
}

function asSlackInteractionPayload(rawPayload: unknown): SlackInteractionPayload {
  const record = asRecord(rawPayload)
  const socketPayload = asRecord(record.payload)
  if (socketPayload.type === "block_actions") return socketPayload as SlackInteractionPayload
  if (record.type === "block_actions") return record as SlackInteractionPayload
  return {}
}

function isSlackEventsPayload(value: Record<string, unknown>): boolean {
  return Boolean(value.event && asRecord(value.event).type)
}

function normalizeSlackUser(user: SlackUserPayload): ChannelIdentity {
  return {
    id: String(user.id ?? ""),
...
function normalizeSlackRoom(roomId: string, channelType?: string): ChannelRoom {
  return {
    id: roomId,
...
function normalizeSlackInteractionRoom(roomId: string, name?: string): ChannelRoom {
  return {
    id: roomId,
...
function normalizeSlackWorkspace(teamId: string): ChannelWorkspace {
  return {
    id: teamId,
  }
}

function normalizeSlackRoomType(roomId: string, channelType?: string): NonNullable<ChannelRoom["type"]> {
  if (channelType === "im" || roomId.startsWith("D")) return "direct"
  if (channelType === "mpim" || roomId.startsWith("G")) return "group"
  return "channel"
}

function normalizeSlackFiles(files: SlackFilePayload[]): ChannelAttachment[] {
  return files
    .map((file): ChannelAttachment | null => {
    ...
    .filter((file): file is ChannelAttachment => Boolean(file))
}

function normalizeSlackMentions(
  text: string,
...
function parseSlackActionId(actionId: string): {
  ...
  if (normalized === "cancel") return { kind: "cancel", actionId: normalized }
  if (normalized === "retry") return { kind: "retry", actionId: normalized }
  return { kind: "choose_option", actionId: normalized || "unknown" }
}

function parseSlackTimestamp(value: string | undefined, now: (() => number) | undefined): number {
...
function applySlackThreadPolicy(message: OutboundMessage): OutboundMessage {
...
function slackDeliveryTargetFromOutbound(message: OutboundMessage): SlackDeliveryTarget {
  return {
