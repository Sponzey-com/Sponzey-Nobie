...
interface GoogleChatUserPayload {
  name?: string
  displayName?: string
  avatarUrl?: string
  email?: string
...
interface GoogleChatThreadPayload {
  name?: string
}
...
interface GoogleChatMessagePayload {
  name?: string
  sender?: GoogleChatUserPayload
  createTime?: string
  text?: string
  argumentText?: string
  slashCommand?: { commandId?: string | number; commandName?: string }
  thread?: GoogleChatThreadPayload
  space?: GoogleChatSpacePayload
  attachment?: GoogleChatAttachmentPayload[]
  annotations?: unknown[]
}

interface GoogleChatActionPayload {
...
export function buildGoogleChatCapabilityManifest(): ChannelCapabilities {
...
export function buildGoogleChatWorkspaceDoctor(config?: GoogleChatConfig | undefined): GoogleChatWorkspaceDoctor {
  const issues: GoogleChatDoctorIssue[] = []
...
export function validateGoogleChatRequestAuth(input: {
  ...
  if (!expected) return { valid: false, reason: "missing_verification_token" }
  const received = input.receivedToken?.trim() || parseBearerToken(input.authorization)
  if (!received) return { valid: false, reason: "missing_request_auth" }
  return received === expected
    ? { valid: true, reason: "matched" }
    : { valid: false, reason: "mismatch" }
}

export function normalizeGoogleChatInboundEvent(
  rawPayload: unknown,
  options: {
    channelId?: string
    connectionId?: string
  ...
  if (!isGoogleChatAuthValid(request, options.verificationToken)) return []
...
export function normalizeGoogleChatCardAction(
  ...
  if (!isGoogleChatAuthValid(request, options.verificationToken)) return []
  const body = request.body
  if (body.type !== "CARD_CLICKED") return []
...
export function buildGoogleChatContinuationLookupCandidate(
  envelope: InboundEnvelope,
  options: { lookupWindowMs?: number } = {},
): GoogleChatContinuationLookupCandidate | null {
  if (envelope.provider !== "google_chat" || !envelope.threadId || !envelope.room?.id) return null
  return {
    provider: "google_chat",
...
export class GoogleChatChannelAdapter implements ChannelAdapter {
  readonly channelId: string
  readonly provider = "google_chat" as const
  readonly connectionId: string

  private readonly config: GoogleChatConfig | undefined
  private readonly transport: GoogleChatAdapterTransport | undefined
  ...
  async healthCheck(): Promise<ChannelHealthCheck> {
    ...
    if (runtime.lastError) return { status: "failed", checkedAt: this.now(), message: runtime.lastError, detail: googleChatDoctorDetail(policy.doctor) }
    return {
  ...
  async normalizeInbound(rawPayload: unknown): Promise<InboundEnvelope[]> {
    const options: Parameters<typeof normalizeGoogleChatInboundEvent>[1] = {
      channelId: this.channelId,
      connectionId: this.connectionId,
      now: this.now,
    }
    if (this.config?.verificationToken !== undefined) options.verificationToken = this.config.verificationToken
    if (this.botUserId !== undefined) options.botUserId = this.botUserId
  ...
  async normalizeInteraction(rawPayload: unknown): Promise<InteractionEnvelope[]> {
    const options: Parameters<typeof normalizeGoogleChatCardAction>[1] = {
      channelId: this.channelId,
      connectionId: this.connectionId,
      now: this.now,
    }
...
export function createGoogleChatChannelAdapter(options: GoogleChatChannelAdapterOptions = {}): ChannelAdapter {
  return defineChannelAdapter(new GoogleChatChannelAdapter(options))
}

function normalizeGoogleChatMessage(
  body: GoogleChatEventPayload,
  rawPayload: unknown,
  options: Parameters<typeof normalizeGoogleChatInboundEvent>[1],
): InboundEnvelope[] {
  const opts = options ?? {}
  ...
  if (!sender.id) return []
  if (sender.providerType === "bot") return []
  if (opts.botUserId && sender.id === opts.botUserId) return []
  ...
  return [{
    ...
    rawPayloadRef: createRawPayloadRef({ provider: "google_chat", payload: rawPayload, createdAt: timestamp }),
    ...(threadId
      ? { continuationContext: { parentMessageId: threadId, source: "thread" as const } }
      : {}),
    dedupeKey: `google_chat:${spaceId}:${threadId ?? "main"}:${messageId}`,
  }]
}

function extractGoogleChatRequest(rawPayload: unknown): {
  body: GoogleChatEventPayload
  ...
  const body = isRecord(record.body) ? record.body as GoogleChatEventPayload : record as GoogleChatEventPayload
  const request: {
    body: GoogleChatEventPayload
    receivedToken?: string
    authorization?: string
  ...
  if (token !== undefined) request.receivedToken = token
  if (authorization !== undefined) request.authorization = authorization
  return request
}

function isGoogleChatAuthValid(
  request: ReturnType<typeof extractGoogleChatRequest>,
  verificationToken: string | undefined,
): boolean {
  const authInput: Parameters<typeof validateGoogleChatRequestAuth>[0] = {}
  if (verificationToken !== undefined) authInput.verificationToken = verificationToken
  if (request.receivedToken !== undefined) authInput.receivedToken = request.receivedToken
  if (request.authorization !== undefined) authInput.authorization = request.authorization
  const verification = validateGoogleChatRequestAuth(authInput)
  return verification.valid
}

function normalizeGoogleChatUser(user: GoogleChatUserPayload): ChannelIdentity {
...
function normalizeGoogleChatRoom(space: GoogleChatSpacePayload): ChannelRoom {
...
function normalizeGoogleChatAttachments(attachments: GoogleChatAttachmentPayload[]): ChannelAttachment[] {
  return attachments
...
function parseGoogleChatAction(
  actionMethodName: string | undefined,
  parameters: Array<{ key?: string; value?: string }> | undefined,
  commonParameters: Record<string, string> | undefined,
): {
  ...
  for (const parameter of parameters ?? []) {
    if (parameter.key && parameter.value) parameterMap.set(parameter.key, parameter.value)
  }
  for (const [key, value] of Object.entries(commonParameters ?? {})) {
    if (value) parameterMap.set(key, value)
  }
  const parameterValue = parameterMap.get("value") ?? parameterMap.get("runId") ?? parameterMap.get("correlationId")
  if (!raw) return { kind: "choose_option", ...(parameterValue ? { value: parameterValue } : {}) }
  ...
  if (scope === "cancel") return { kind: "cancel", actionId: actionId || scope, ...(tail ? { correlationId: tail } : {}), value: parameterValue ?? raw }
  if (scope === "retry") return { kind: "retry", actionId: actionId || scope, ...(tail ? { correlationId: tail } : {}), value: parameterValue ?? raw }
...
function extractResourceId(value: string | undefined, segment: string): string {
...
function parseGoogleChatTimestamp(value: string | undefined, now: (() => number) | undefined): number {
...
function applyGoogleChatThreadPolicy(message: OutboundMessage): OutboundMessage {
...
function parseBearerToken(value: string | null | undefined): string {
...
function googleChatDoctorDetail(doctor: GoogleChatWorkspaceDoctor): JsonValue {
