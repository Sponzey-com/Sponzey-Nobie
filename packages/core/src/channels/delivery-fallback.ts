...
export type ChannelDeliveryFallbackAction =
  | "send_as_is"
  | "split_text"
  | "summarize_then_link"
  | "download_link"
...
export type ChannelArtifactFallbackMode =
  | "none"
  | "native_file"
  | "download_link"
  | "inline_preview"
...
export interface ResolveChannelDeliveryFallbackPlanInput {
  ...
  message: Pick<
    OutboundMessage,
    "text" | "attachments" | "actions" | "threadPolicy" | "chunkPolicy" | "deliveryMode" | "redactionPolicy"
  >
...
export function resolveChannelDeliveryFallbackPlan(
  input: ResolveChannelDeliveryFallbackPlanInput,
  ...
  const issues: ChannelDeliveryFallbackIssue[] = []
  const notices: string[] = []
  const unsupportedCapabilities = new Set<ChannelDeliveryCapability | string>()
  let action: ChannelDeliveryFallbackAction = "send_as_is"
  let artifactMode: ChannelArtifactFallbackMode = "none"
  ...
  const requiresExplicitApproval =
    input.artifactSensitivity === "sensitive"
...
export function splitTextForChannel(text: string, maxLength: number): string[] {
  const safeMax = Math.max(1, Math.floor(maxLength || DEFAULT_MAX_MESSAGE_LENGTH))
  if (text.length <= safeMax) return text ? [text] : []
...
export function buildCapabilityFallbackNotice(
  receipt: Pick<DeliveryReceipt, "status" | "capability" | "errorMessage" | "errorCode">,
): { title: string; message: string; severity: ChannelDeliveryFallbackSeverity } | undefined {
  if (receipt.status !== "unsupported_capability") return undefined
  return {
...
function resolveMaxMessageLength(input: ResolveChannelDeliveryFallbackPlanInput): number {
...
function resolveRequestedCapabilities(
  input: ResolveChannelDeliveryFallbackPlanInput,
): Array<ChannelDeliveryCapability | string> {
  const requested = new Set<ChannelDeliveryCapability | string>(input.requestedCapabilities ?? [])
  if ((input.message.actions?.length ?? 0) > 0) requested.add("supportsButtons")
  if ((input.message.attachments?.length ?? 0) > 0) requested.add("supportsFiles")
  if (input.message.threadPolicy.mode !== "none") requested.add("supportsThreads")
  return [...requested]
}
...
function isCapabilitySupported(
  ...
  if (!Object.prototype.hasOwnProperty.call(capabilities, capability)) return false
  return (capabilities as unknown as Record<string, unknown>)[capability] === true
}

function buildUnsupportedCapabilityIssue(
  capability: ChannelDeliveryCapability | string,
...
function resolveArtifactMode(input: {
  ...
  if (input.capabilities.supportsFiles) return "native_file"
...
function dedupeIssues(issues: ChannelDeliveryFallbackIssue[]): ChannelDeliveryFallbackIssue[] {
