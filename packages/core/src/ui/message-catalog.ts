...
export type UiMessageKey =
  ...
  | "component.tasks"
  | "setup.ready.summary"
  | "setup.needs_setup.summary"
  | "setup.needs_setup.warning"
  | "setup.open.action"
  | "ai.ready.summary"
  | "ai.needs_setup.summary"
  | "ai.needs_setup.warning"
  | "ai.open.action"
  | "channels.ready.summary"
  | "channels.idle.summary"
  | "channels.idle.warning"
  | "channels.open.action"
  | "yeonjang.disabled.summary"
  | "yeonjang.connected.summary"
  | "yeonjang.empty.warning"
  | "yeonjang.open.action"
  | "tasks.ready.summary"
  | "tasks.running.summary"
  | "tasks.approval.summary"
  | "tasks.approval.warning"
  | "tasks.open.action"
  | "beginner.ready.summary"
  | "beginner.attention.summary"
  | "beginner.ready.status"
  | "beginner.attention.status"
  | "error.beginner.title"
  | "error.advanced.title"
  | "error.admin.title"
  | "error.repeated.title"
  | "error.repeated.action"

export const UI_MESSAGE_CATALOG: Record<UiMessageKey, Record<UiMessageLocale, string>> = {
...
const BEGINNER_FORBIDDEN_TERMS: RegExp[] = [
  /\bphase\b/iu,
  /\btask\b/iu,
  /\bverdict\b/iu,
  /policy\s*version/iu,
...
const ADVANCED_FORBIDDEN_TERMS: RegExp[] = [
...
export function uiMessage(key: UiMessageKey, locale: UiMessageLocale = "ko", params?: Record<string, string | number>): string {
  const entry = UI_MESSAGE_CATALOG[key]
  return interpolate(entry[locale] ?? entry.ko, params)
}

export function assertUiMessageCatalogCoverage(): void {
  for (const [key, entry] of Object.entries(UI_MESSAGE_CATALOG)) {
    if (!entry.ko?.trim()) throw new Error(`missing ko message: ${key}`)
    if (!entry.en?.trim()) throw new Error(`missing en message: ${key}`)
  }
}

export function findDisallowedUiTerms(mode: UiMessageMode, text: string): string[] {
