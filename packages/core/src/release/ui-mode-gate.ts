import { resolveAdminUiActivation, resolveUiMode, type UiMode } from "../ui/mode.js"
import { redactUiValue, type UiRedactionAudience } from "../ui/redaction.js"
...
const ROUTE_REDIRECTS: Array<{ from: string; expectedTo: string | null }> = [
...
function step(id: string, label: string, evidence: string): UiModeSmokeStep {
  return { id, label, required: true, status: "passed", evidence }
}

function scenarioStatus(steps: UiModeSmokeStep[]): UiReleaseGateStatus {
  if (steps.some((item) => item.required && item.status === "failed")) return "failed"
  if (steps.some((item) => item.required && (item.status === "warning" || item.status === "skipped"))) return "warning"
  return "passed"
}

export function buildUiModeSmokeMatrix(options: UiModeReleaseGateOptions = {}): UiModeSmokeScenario[] {
  const scenarios: UiModeSmokeScenario[] = [
    {
      mode: "beginner",
      status: "passed",
      requiredPassCount: 0,
...
function sampleDomainState(adminEnabled: boolean): UiShellDomainState {
  return {
...
function redactionEvidence(audience: UiRedactionAudience): UiModeRedactionEvidence {
  const sample = {
...
function viewModelRedactionEvidence(): UiModeRedactionEvidence[] {
...
function resolverEvidence(): UiModeResolverEvidence {
...
function adminGuardEvidence(): UiAdminGuardEvidence {
...
function appendSuffix(target: string, source: string, base: string): string {
  const suffix = source.slice(base.length)
  return suffix.startsWith("/") ? `${target}${suffix}` : target
}

function resolveReleaseRouteRedirect(pathname: string): string | null {
  ...
  for (const [from, to] of mappings) {
    if (normalized === from || normalized.startsWith(`${from}/`)) return appendSuffix(to, normalized, from)
  }
  return null
}

function routeRedirectEvidence(): UiRouteRedirectEvidence[] {
...
function regressionGuards(options: UiModeReleaseGateOptions): UiRegressionGuardEvidence[] {
  const guards: UiRegressionGuardEvidence[] = [
...
export function buildUiModeReleaseGateSummary(options: UiModeReleaseGateOptions = {}): UiModeReleaseGateSummary {
  ...
  for (const scenario of smokeMatrix) {
    if (scenario.status === "failed") blockingFailures.push(`ui_mode_smoke_failed:${scenario.mode}`)
    if (scenario.status === "warning") warnings.push(`ui_mode_smoke_warning:${scenario.mode}`)
  }
  if (!resolver.adminAvailableOnlyWithFlag) blockingFailures.push("ui_mode_resolver_admin_flag_boundary_failed")
  if (!resolver.canSwitchInUi) warnings.push("ui_mode_switching_disabled")
  for (const item of redaction) {
    if (!item.passed) blockingFailures.push(`ui_redaction_failed:${item.audience}`)
  }
  if (!adminGuard.passed) blockingFailures.push("admin_guard_policy_failed")
  for (const item of routeRedirects) {
    if (!item.passed) blockingFailures.push(`route_redirect_failed:${item.from}`)
  }
  for (const item of regression) {
    if (item.status === "failed") blockingFailures.push(`regression_guard_failed:${item.id}`)
  }
