...
function releaseModeFor(id) {
    ...
    if (!mode)
        throw new Error(`Unknown sub-agent release mode: ${id}`);
    return mode;
}
function defaultMigrationPreflight() {
    return {
        ok: true,
        risk: "low",
...
export function runSubAgentRestartResumeSoak(input = {}) {
    ...
    if (!base.rootRunCreateSmokePassed)
        blockingFailures.push("root_run_create_smoke_failed");
    if (!base.finalAnswerSmokePassed)
        blockingFailures.push("final_answer_smoke_failed");
    if (!base.projectionRecovered)
        blockingFailures.push("runtime_projection_not_recovered");
    if (!base.finalizerRecovered)
        blockingFailures.push("finalizer_not_recovered");
...
export function buildSubAgentRollbackEvidence(input = {}) {
    const now = input.now ?? new Date();
    const base = {
    ...
    if (!base.existingRunCreateSmokePassed)
        blockingFailures.push("existing_run_create_smoke_failed");
    if (!base.finalAnswerSmokePassed)
        blockingFailures.push("final_answer_smoke_failed");
    if (!base.migrationStatePreserved)
        blockingFailures.push("migration_state_not_preserved");
    return {
...
function buildDryRunSummary(input) {
    const registryStatus = input.benchmarkSuite.aggregate.hotRegistrySnapshotP95Ms != null &&
    ...
    const migrationStatus = input.migrationPreflight.currentSchemaVersion <= input.migrationPreflight.latestSchemaVersion
        ? "passed"
        : "failed";
    const deliveryDedupeCount = input.performanceEvidence.counters.find((counter) => counter.id === "delivery_dedupe_count")
        ?.count ?? 0;
    const deliveryStatus = input.benchmarkSuite.aggregate.duplicateFinalAnswerCount === 0 &&
    ...
    return {
        ...
        delivery: {
            duplicateFinalAnswerCount: input.benchmarkSuite.aggregate.duplicateFinalAnswerCount +
...
function hasScenario(suite, scenarioId) {
    return suite.scenarios.some((scenario) => scenario.scenarioId === scenarioId);
}
function benchmarkThresholdFailures(suite, soak, thresholds) {
    const failures = [];
    ...
    if (suite.aggregate.firstProgressP95Ms == null ||
...
export function buildSubAgentReleaseReadinessSummary(options = {}) {
    ...
    const subAgentFlag = options.featureFlags?.find((flag) => flag.featureKey === "sub_agent_orchestration");
    const currentFeatureFlagMode = subAgentFlag?.mode ?? requestedModeDefinition.featureFlagMode;
    const currentCompatibilityMode = subAgentFlag?.compatibilityMode ?? requestedModeDefinition.compatibilityMode;
    ...
    const deliveryDedupeCounter = performanceEvidence.counters.find((counter) => counter.id === "delivery_dedupe_count")?.count ??
        0;
    const checks = [
        gate({
            id: "release_mode_sequence",
            title: "Release mode sequence",
            pass: SUB_AGENT_RELEASE_MODE_SEQUENCE.map((mode) => mode.id).join(">") ===
        ...
        gate({
            ...
            pass: hasScenario(benchmarkSuite, "bench.parallel_research_3") &&
        ...
        gate({
            ...
            pass: benchmarkSuite.aggregate.totalLlmCallCount > 0 &&
        ...
        gate({
            ...
            pass: benchmarkSuite.aggregate.duplicateFinalAnswerCount === 0 &&
...
//# sourceMappingURL=sub-agent-release-gate.js.map