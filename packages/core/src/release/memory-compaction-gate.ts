...
function checkQualitySnapshot(quality: ReturnType<typeof buildMemoryQualitySnapshot>): MemoryCompactionReleaseGateCheck {
  return {
...
function checkAppendOnlyArchive(runs: ReturnType<typeof listMemoryCompactionRuns>): MemoryCompactionReleaseGateCheck {
  const completed = runs.filter((run) => run.status === "completed")
  const missingArchive = completed.filter((run) => {
    const sourceMessageCount = Number(run.metadata?.["sourceMessageCount"] ?? 0)
    if (sourceMessageCount <= 0) return false
...
function checkModelAudit(runs: ReturnType<typeof listMemoryCompactionRuns>): MemoryCompactionReleaseGateCheck {
  const completed = runs.filter((run) => run.status === "completed")
...
function checkDriftWarnings(inspector: ReturnType<typeof buildMemoryInspectorSnapshot>): MemoryCompactionReleaseGateCheck {
...
function checkHeuristicFallback(runs: ReturnType<typeof listMemoryCompactionRuns>): MemoryCompactionReleaseGateCheck {
  const completed = runs.filter((run) => run.status === "completed")
  const heuristicFallbackRuns = completed.filter((run) => {
    ...
    return Boolean(
      audit
      && typeof audit === "object"
      && (audit as Record<string, unknown>)["heuristicFallbackApplied"] === true,
    )
  })
  return {
...
export function buildMemoryCompactionReleaseGateSummary(input: {
  ...
  const gateStatus: MemoryCompactionReleaseGateStatus = blockingFailures.length > 0
    ? "failed"
    : warnings.length > 0
      ? "warning"
      : "passed"
