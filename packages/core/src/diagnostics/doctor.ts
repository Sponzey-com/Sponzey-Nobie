...
function makeCheck(
  ...
  return { name, status, message, detail: sanitizeValue(detail) as Record<string, unknown>, guide }
}
...
function sanitizeValue(value: unknown): unknown {
  ...
  if (value && typeof value === "object") {
    return Object.fromEntries(Object.entries(value as Record<string, unknown>).map(([key, item]) => {
      if (/api[_-]?key|token|secret|password|credential|authorization/i.test(key) && typeof item === "string") return [key, "***"]
...
function summarize(checks: DoctorCheckResult[]): DoctorReport["summary"] {
  return {
...
function overallStatus(checks: DoctorCheckResult[]): DoctorStatus {
...
function checkRuntimeManifest(manifest: RuntimeManifest): DoctorCheckResult {
  return manifest.id
...
function checkProviderChat(manifest: RuntimeManifest): DoctorCheckResult {
...
function checkProviderEmbedding(manifest: RuntimeManifest): DoctorCheckResult {
  const provider = manifest.provider
...
function countRecentWebRetrievalEvents(): {
  ...
  plannerSchemaFailureCount: number
  ...
  for (const event of events) {
    ...
    if (serialized.includes("conflict") || serialized.includes("insufficient_conflict")) conflictCount += 1
    if (serialized.includes("planner_") && (serialized.includes("schema") || serialized.includes("invalid") || serialized.includes("parse_failed"))) plannerSchemaFailureCount += 1
    if (serialized.includes("attempt") && (serialized.includes("failed") || serialized.includes("timeout") || serialized.includes("error"))) failedAttemptCount += 1
  }
...
function checkGatewayExposure(manifest: RuntimeManifest): DoctorCheckResult {
...
function checkCredentialRedaction(manifest: RuntimeManifest): DoctorCheckResult {
...
function checkTelegram(manifest: RuntimeManifest): DoctorCheckResult {
  const telegram = manifest.channels.telegram
  if (!telegram.enabled) return makeCheck("channel.telegram", "unknown", "Telegram 채널이 비활성화되어 있습니다.")
  if (!telegram.credentialConfigured) return makeCheck("channel.telegram", "blocked", "Telegram bot token이 설정되어 있지 않습니다.")
  if (!telegram.targetConfigured) return makeCheck("channel.telegram", "warning", "Telegram 허용 사용자/그룹 대상이 비어 있습니다.")
  return makeCheck("channel.telegram", "ok", "Telegram 채널 설정이 존재합니다.")
}

function checkSlack(manifest: RuntimeManifest): DoctorCheckResult {
  const slack = manifest.channels.slack
  if (!slack.enabled) return makeCheck("channel.slack", "unknown", "Slack 채널이 비활성화되어 있습니다.")
  if (!slack.credentialConfigured) return makeCheck("channel.slack", "blocked", "Slack bot/app token이 설정되어 있지 않습니다.")
  if (!slack.targetConfigured) return makeCheck("channel.slack", "warning", "Slack 허용 채널 대상이 비어 있습니다.")
  return makeCheck("channel.slack", "ok", "Slack 채널 설정이 존재합니다.")
}

function checkWebui(manifest: RuntimeManifest): DoctorCheckResult {
  const webui = manifest.channels.webui
  if (!webui.enabled) return makeCheck("channel.webui", "unknown", "WebUI가 비활성화되어 있습니다.")
...
function checkDbMigration(manifest: RuntimeManifest): DoctorCheckResult {
  ...
  if (!db.exists) return makeCheck("db.migration", "warning", "DB 파일이 아직 생성되지 않았습니다. 최초 실행 시 migration이 적용됩니다.", { latestVersion: db.latestVersion })
  if (!db.upToDate) return makeCheck("db.migration", "warning", "적용되지 않은 DB migration이 있습니다.", { currentVersion: db.currentVersion, latestVersion: db.latestVersion, pendingVersions: db.pendingVersions })
  return makeCheck("db.migration", "ok", "DB migration이 최신입니다.", { currentVersion: db.currentVersion, latestVersion: db.latestVersion })
}

function checkMigrationLock(manifest: RuntimeManifest): DoctorCheckResult {
...
function checkFeatureFlags(manifest: RuntimeManifest): DoctorCheckResult {
  ...
  if (rollback.length > 0) return makeCheck("feature.flags", "warning", "일부 feature flag가 rollback mode입니다.", detail, "rollback mode가 의도된 상태인지 확인하고 rollout evidence를 점검하세요.")
  return makeCheck("feature.flags", "ok", "Feature flag snapshot이 정상적으로 확인되었습니다.", detail)
}

function checkRolloutEvidence(manifest: RuntimeManifest): DoctorCheckResult {
  ...
  if (rollout.evidence.blockedCount > 0) return makeCheck("rollout.evidence", "blocked", "Rollout evidence에 blocked 항목이 있습니다.", detail, "enforced 전환을 중단하고 blocked evidence의 run/control timeline을 확인하세요.")
  if (rollout.shadowCompare.mismatchCount > 0 || rollout.evidence.warningCount > 0) return makeCheck("rollout.evidence", "warning", "Shadow compare 또는 rollout evidence에 검토가 필요한 항목이 있습니다.", detail)
  return makeCheck("rollout.evidence", "ok", "Rollout evidence에 blocking mismatch가 없습니다.", detail)
}

function checkPlanDrift(): DoctorCheckResult {
  try {
    ...
    if (report.summary.blockedCount > 0) return makeCheck("plan.drift", "blocked", "Plan drift check에 blocked 항목이 있습니다.", detail, "phase/task 문서와 evidence 상태를 먼저 정리하세요.")
...
function checkPromptRegistry(manifest: RuntimeManifest): DoctorCheckResult {
  const prompts = manifest.promptSources
  if (prompts.count === 0) return makeCheck("prompt.registry", "blocked", "prompt source registry를 읽지 못했습니다.", { diagnostics: prompts.diagnostics })
  if (!prompts.localeParityOk) return makeCheck("prompt.registry", "warning", "prompt source locale parity 문제가 있습니다.", { diagnostics: prompts.diagnostics })
  return makeCheck("prompt.registry", "ok", "prompt source registry가 정상입니다.", { count: prompts.count, checksum: prompts.checksum })
}

function checkMemoryFts(manifest: RuntimeManifest): DoctorCheckResult {
  const memory = manifest.memory
  if (!memory.dbExists) return makeCheck("memory.fts", "warning", "DB 파일이 없어 FTS 상태를 확인하지 못했습니다.")
  if (memory.ftsAvailable === true) return makeCheck("memory.fts", "ok", "Memory FTS 테이블이 존재합니다.")
  if (memory.ftsAvailable === false) return makeCheck("memory.fts", "warning", "Memory FTS 테이블을 찾지 못했습니다.")
  return makeCheck("memory.fts", "unknown", "Memory FTS 상태를 확인하지 못했습니다.")
}

function checkMemoryVector(manifest: RuntimeManifest): DoctorCheckResult {
  const memory = manifest.memory
  if (!memory.embeddingProvider || !memory.embeddingModel) return makeCheck("memory.vector", "warning", "Embedding 설정이 없어 vector 검색은 비활성 또는 제한 상태입니다.")
  if (memory.vectorTableAvailable === true) return makeCheck("memory.vector", "ok", "Memory vector 테이블이 존재합니다.", { embeddingRows: memory.embeddingRows })
  if (memory.vectorTableAvailable === false) return makeCheck("memory.vector", "warning", "Memory vector 테이블을 찾지 못했습니다.")
  return makeCheck("memory.vector", "unknown", "Memory vector 상태를 확인하지 못했습니다.")
}

function checkQueueBackpressure(): DoctorCheckResult {
  try {
    ...
    if (recovering.length > 0) return makeCheck("queue.backpressure", "warning", "일부 queue가 backpressure 복구 상태입니다.", detail)
