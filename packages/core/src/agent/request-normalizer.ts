...
function translateRelativeDelaySyntax(text: string): string {
  return text
...
function translateLiteralDeliveryRequest(text: string): string | null {
...
function translateKnownPhrases(text: string): string {
  let normalized = text
...
export function normalizeRequestForIntake(message: string): IntakeNormalizedRequest {
  ...
  const normalizedEnglish = normalizeWhitespace(
    translateKnownPhrases(
      translateRelativeDelaySyntax(originalMessage),
    ),
  )
