...
export function buildTaskIntakeSystemPrompt(options: TaskIntakePromptOptions = {}): string {
  ...
  return [
    ...
    "",
    "### 7. Respect runtime environment facts",
    "If runtime context says there is exactly one connected Yeonjang extension, treat that extension as the default execution device.",
    "Do not ask the user which device to use unless the user explicitly mentions another device, another computer, or multiple device candidates.",
    "For screen capture, camera capture, or local system control requests, prefer the connected extension information from runtime context over generic device clarification.",
    "",
    "## Output Schema",
    ...
    '    "suggested_target": "embedded | anthropic | openai | gemini | local_reasoner | auto",',
    ...
    `{ "goal": "...", "context": "...", "delegate_mode": "same_session | new_session | auto", "preferred_target": "auto | embedded | anthropic | openai | gemini | local_reasoner", "max_delegation_turns": ${maxDelegationTurns}, "review_required": true }`,
    "",
