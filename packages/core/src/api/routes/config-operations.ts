...
export function registerConfigOperationsRoute(app: FastifyInstance): void {
  ...
  app.post<{ Body: { backupPath?: string } }>("/api/config/db/import", { preHandler: authMiddleware }, async (req, reply) => {
    if (!req.body?.backupPath) return reply.status(400).send({ ok: false, error: "backupPath is required" })
    try {
  ...
  app.post<{ Body: { workDir?: string; exportPath?: string; overwrite?: boolean } }>("/api/config/prompt-sources/import", { preHandler: authMiddleware }, async (req, reply) => {
    if (!req.body?.exportPath) return reply.status(400).send({ ok: false, error: "exportPath is required" })
    try {
      const workDir = resolveWorkDir(req.body.workDir)
      return {
        ok: true,
