...
export function registerSchedulesRoute(app) {
    ...
    app.get("/api/schedules/:id", { preHandler: authMiddleware }, async (req, reply) => {
    ...
    app.put("/api/schedules/:id", { preHandler: authMiddleware }, async (req, reply) => {
        const { id } = req.params;
    ...
    app.delete("/api/schedules/:id", { preHandler: authMiddleware }, async (req, reply) => {
        const s = getSchedule(req.params.id);
        if (!s)
            return reply.status(404).send({ error: "Not found" });
    ...
    app.get("/api/schedules/:id/runs", { preHandler: authMiddleware }, async (req, reply) => {
        const { id } = req.params;
        if (!getSchedule(id))
            return reply.status(404).send({ error: "Not found" });
    ...
    app.post("/api/schedules/:id/run", { preHandler: authMiddleware }, async (req, reply) => {
        const s = getSchedule(req.params.id);
        if (!s)
            return reply.status(404).send({ error: "Not found" });
    ...
    app.patch("/api/schedules/:id/toggle", { preHandler: authMiddleware }, async (req, reply) => {
        const s = getSchedule(req.params.id);
        if (!s)
            return reply.status(404).send({ error: "Not found" });
