...
import { applyEnterpriseTopologyGuiCommands, createEnterpriseTopologyGuiDraft, EnterpriseTopologyGuiOperationError, isEnterpriseTopologyGuiCommandKind, } from "../../topology/gui-operations.js";
import { compileTopology } from "../../topology/compiler.js";
import { buildAgentTeamTopologyImportPreview } from "../../topology/agent-team-import.js";
import { createEnterpriseTopologyRegistry, } from "../../topology/registry.js";
import { normalizeTopologyDocumentFormat, parseTopologyImportDocument, stringifyTopologyDocument, } from "../../topology/import-export.js";
...
import { validateTopology } from "../../topology/validator.js";
...
import { createNodeDefinitionSuggestion, normalizeNodeDefinitionSuggestionRequest, } from "../../topology/node-definition-suggestion.js";
...
import { getWorkOrderTemplate, getWorkOrderTemplateContext, } from "../../topology-runtime/work-order-templates.js";
import { authMiddleware } from "../middleware/auth.js";
const guiDrafts = new Map();
export function resetTopologyGuiDraftStoreForTest() {
    guiDrafts.clear();
}
...
function sendActivationResult(reply, result) {
    if (result.ok) {
        return reply.send({
            ok: true,
            activation: result,
...
function parseTopologyImport(body) {
...
function asAgentTeamImportMode(value) {
    return value === "skip" ? "skip" : "team";
}
function parseGuiDraftCommands(body) {
...
function getTopologyDraftSource(topologyId, body) {
    if (isRecord(body) && Object.prototype.hasOwnProperty.call(body, "topology")) {
        ...
        return { ok: true, topology: structuredClone(body.topology) };
    }
    ...
    if (topologyRecord === null)
        return { ok: false, statusCode: 404, error: "topology_not_found" };
    ...
    if (version === undefined)
        return { ok: false, statusCode: 404, error: "topology_version_not_found" };
    const versionRecord = registry.getVersion(topologyId, version);
    if (versionRecord === null)
        return { ok: false, statusCode: 404, error: "topology_version_not_found" };
    return { ok: true, topology: versionRecord.topology };
}
function persistGuiDraftTopology(topology, body) {
    if (!isRecord(body) || body.persist !== true)
        return undefined;
...
function guiDraftPersistencePayload(persisted) {
...
function buildWorkOrderPreview(draft, result) {
...
function buildCompiledPreviewPayload(draft, result) {
    if (!result.ok) {
        return {
            ok: false,
...
function updateDraftValidation(draft, validation) {
    return {
        ...draft,
        validation,
        updatedAt: Date.now(),
...
function asSimulationMode(value, fallback) {
    return value === "failure" || value === "success" ? value : fallback;
}
function createManualTopologyRunId(topologyId, at = Date.now()) {
    const suffix = Math.random().toString(36).slice(2, 8);
    return `topology-run:manual:${topologyId}:${at}:${suffix}`;
}
function createSatisfiedOutputs(outputIds) {
...
function createMissingOutputs(outputIds) {
    return outputIds.map((outputId) => ({
        outputId,
        status: "missing",
    }));
}
function createManualTopologyToolDispatcher(simulationMode) {
    return {
        async dispatch(name) {
...
async function runManualGuiDraftTopology(input) {
    ...
    const result = await runNodeRuntime({
        ...
        toolExecution: {
            ...
            approvalDecisionsByToolId: Object.fromEntries(compiledNode.allowedToolIds.map((toolId) => [toolId, "approved"])),
        },
        recovery: {
...
export function registerTopologyRoutes(app) {
    ...
    app.get("/api/topologies/:topologyId", { preHandler: authMiddleware }, async (req, reply) => {
        const registry = createEnterpriseTopologyRegistry();
    ...
    app.get("/api/topologies/:topologyId/versions", { preHandler: authMiddleware }, async (req) => {
        const registry = createEnterpriseTopologyRegistry();
        return {
            ok: true,
            versions: registry.listVersions(req.params.topologyId),
    ...
    app.post("/api/topologies/import", { preHandler: authMiddleware }, async (req, reply) => {
        const parsed = parseTopologyImport(req.body);
        if (!parsed.ok)
            return sendValidationError(reply, "invalid_enterprise_topology", parsed.issues);
    ...
    app.post("/api/topologies/import/agent-team-preview", { preHandler: authMiddleware }, async (req) => {
        const body = isRecord(req.body) ? req.body : {};
    ...
    app.post("/api/topologies/:topologyId/versions", { preHandler: authMiddleware }, async (req, reply) => {
    ...
    app.post("/api/topologies/:topologyId/gui-draft", { preHandler: authMiddleware }, async (req, reply) => {
        const body = isRecord(req.body) ? req.body : {};
    ...
    app.get("/api/topologies/:topologyId/gui-draft", { preHandler: authMiddleware }, async (req, reply) => {
        const existing = guiDrafts.get(req.params.topologyId);
        if (existing)
            return { ok: true, draft: existing, reused: true, source: "memory" };
        const registry = createEnterpriseTopologyRegistry();
    ...
    app.patch("/api/topologies/:topologyId/gui-draft/operations", { preHandler: authMiddleware }, async (req, reply) => {
        const draft = guiDrafts.get(req.params.topologyId);
        if (!draft)
            return reply.status(404).send({ ok: false, error: "gui_draft_not_found" });
        const parsed = parseGuiDraftCommands(req.body);
        if (!parsed.ok)
            return sendValidationError(reply, "invalid_gui_operation_payload", parsed.issues);
        try {
    ...
    app.get("/api/topologies/:topologyId/gui-draft/issues", { preHandler: authMiddleware }, async (req, reply) => {
        const draft = guiDrafts.get(req.params.topologyId);
        if (!draft)
            return reply.status(404).send({ ok: false, error: "gui_draft_not_found" });
        return {
    ...
    app.post("/api/topologies/:topologyId/gui-draft/validate", { preHandler: authMiddleware }, async (req, reply) => {
        const draft = guiDrafts.get(req.params.topologyId);
        if (!draft)
            return reply.status(404).send({ ok: false, error: "gui_draft_not_found" });
    ...
    app.post("/api/topologies/:topologyId/gui-draft/compile", { preHandler: authMiddleware }, async (req, reply) => {
        const draft = guiDrafts.get(req.params.topologyId);
        if (!draft)
            return reply.status(404).send({ ok: false, error: "gui_draft_not_found" });
    ...
    app.get("/api/topologies/:topologyId/gui-draft/compiled-preview", { preHandler: authMiddleware }, async (req, reply) => {
        const draft = guiDrafts.get(req.params.topologyId);
        if (!draft)
            return reply.status(404).send({ ok: false, error: "gui_draft_not_found" });
        return buildCompiledPreviewPayload(draft, compileTopology(draft.topology));
    });
    app.post("/api/topologies/:workspaceId/:topologyId/executor-nodes/suggest-definition", { preHandler: authMiddleware }, async (req, reply) => {
    ...
    app.post("/api/topologies/:topologyId/executor-nodes/suggest-definition", { preHandler: authMiddleware }, async (req, reply) => {
        const topologyId = decodeURIComponent(req.params.topologyId).trim();
        if (!topologyId) {
            return reply.status(400).send({
                ok: false,
    ...
    app.post("/api/topologies/:topologyId/gui-draft/executor-graph/plan", { preHandler: authMiddleware }, async (req, reply) => {
        const draft = guiDrafts.get(req.params.topologyId);
        if (!draft)
            return reply.status(404).send({ ok: false, error: "gui_draft_not_found" });
    ...
    app.post("/api/topologies/:topologyId/gui-draft/run", { preHandler: authMiddleware }, async (req, reply) => {
        const draft = guiDrafts.get(req.params.topologyId);
        if (!draft)
            return reply.status(404).send({ ok: false, error: "gui_draft_not_found" });
    ...
    app.post("/api/topologies/:topologyId/versions/:version/activate", { preHandler: authMiddleware }, async (req, reply) => {
    ...
    app.post("/api/topologies/:topologyId/rollback", { preHandler: authMiddleware }, async (req, reply) => {
        const body = isRecord(req.body) ? req.body : {};
    ...
    app.post("/api/topologies/:topologyId/archive", { preHandler: authMiddleware }, async (req, reply) => {
        const registry = createEnterpriseTopologyRegistry();
    ...
    app.get("/api/topologies/:topologyId/export", { preHandler: authMiddleware }, async (req, reply) => {
...
function sendNodeDefinitionSuggestionResult(reply, result) {
    if (result.ok)
        return result;
...
//# sourceMappingURL=topologies.js.map