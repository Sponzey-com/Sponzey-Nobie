...
export function registerPluginsRoute(app: FastifyInstance): void {
  ...
  app.get<{ Params: { name: string } }>("/api/plugins/:name", { preHandler: authMiddleware }, async (req, reply) => {
  ...
  app.post<{
    Body: { name: string; version: string; description?: string; entryPath: string; config?: Record<string, unknown> }
  }>("/api/plugins", { preHandler: authMiddleware }, async (req, reply) => {
  ...
  app.patch<{
  ...
  }>("/api/plugins/:name", { preHandler: authMiddleware }, async (req, reply) => {
    ...
    if (config !== undefined) {
      db.prepare("UPDATE plugins SET config = ?, updated_at = ? WHERE name = ?").run(
  ...
  app.delete<{ Params: { name: string } }>("/api/plugins/:name", { preHandler: authMiddleware }, async (req, reply) => {
    const { name } = req.params
