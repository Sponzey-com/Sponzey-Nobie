...
- `/api/tasks`의 task projection은 이제 free-form recent event label만 넘기지 않고, `activities`의 표준 kind(`attempt.*`, `recovery.*`, `delivery.*`)와 `monitor` 관측 포인트(`activeAttemptCount`, `duplicateExecutionRisk`, `deliveryStatus` 등)를 함께 내려 상태 모니터가 더 안정적인 기준으로 읽을 수 있게 정리 중입니다.
