...
function shouldExclude(name, excludePatterns) {
    if (DEFAULT_EXCLUDE.includes(name))
        return true;
    return excludePatterns.some((p) => matchesGlob(name, p));
}
function walkDir(dir, excludePatterns, depth, results, maxResults) {
...
function searchFileNames(searchPaths, query, excludePatterns, maxResults) {
    const pattern = `*${query}*`;
    const allFiles = [];
...
function searchContentWithRg(searchPaths, query, caseSensitive, maxResults, rgPath) {
    const args = ["--json", "--max-count", "10"];
    if (!caseSensitive)
    ...
    if (result.stdout) {
        ...
        for (const raw of lines) {
            ...
            if (parsed !== null &&
                ...
                if (!data)
                    continue;
                const pathObj = data["path"];
                const lines_obj = data["lines"];
                const lineNumber = data["line_number"];
                const filePath = pathObj?.["text"];
                const lineText = lines_obj?.["text"];
...
function searchContentNode(searchPaths, query, caseSensitive, maxResults, excludePatterns) {
    const allFiles = [];
    for (const sp of searchPaths) {
