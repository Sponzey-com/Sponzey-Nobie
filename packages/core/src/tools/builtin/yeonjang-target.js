...
function resolveAliasAgainstSnapshots(snapshots, rawCandidate) {
    ...
    if (looksLikeWindowsAlias(candidate)) {
        ...
        if (windowsMatches.length === 1)
            return windowsMatches[0]?.extensionId;
        const nonMainSnapshots = snapshots.filter((snapshot) => normalize(snapshot.extensionId) !== normalize(DEFAULT_YEONJANG_EXTENSION_ID));
        if (nonMainSnapshots.length === 1)
            return nonMainSnapshots[0]?.extensionId;
    }
    return undefined;
}
function inferAliasFromUserMessage(userMessage) {
    ...
    if (explicit)
        return explicit;
    if (WINDOWS_PATTERNS.some((pattern) => pattern.test(normalizedMessage)))
        return "windows";
    if (MAC_PATTERNS.some((pattern) => pattern.test(normalizedMessage)))
        return "macos";
    return undefined;
}
