...
async function listWindows(): Promise<WindowInfo[]> {
  ...
  if (platform === "darwin") {
    ...
    const procs = JSON.parse(stdout) as Array<{ Id: number; MainWindowTitle: string; ProcessName: string }>
    return procs.map((p) => ({ id: String(p.Id), title: p.MainWindowTitle, app: p.ProcessName }))
  }
  return []
}

async function focusWindow(titleOrApp: string): Promise<void> {
  const platform = process.platform
  if (platform === "darwin") {
...
export const windowListTool: AgentTool<Record<string, never>> = {
  ...
  execute: async (): Promise<ToolResult> => {
    try {
      ...
      if (!windows.length) return { success: true, output: "열려 있는 창이 없습니다." }
      const text = windows
...
export const windowFocusTool: AgentTool<WindowFocusParams> = {
  name: "window_focus",
