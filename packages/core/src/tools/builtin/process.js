...
async function getProcesses() {
    ...
    // macOS / Linux
    const { stdout } = await execFileAsync("ps", ["aux", "--no-header"].filter((a) => platform !== "darwin" || a !== "--no-header"));
    return parsePsAux(stdout);
}
function parsePsAux(output) {
    const results = [];
...
function formatTable(procs) {
    const header = `${"PID".padStart(7)}  ${"CPU%".padStart(5)}  ${"MEM%".padStart(5)}  NAME`;
    const divider = "-".repeat(header.length);
    const rows = procs.map((p) => `${String(p.pid).padStart(7)}  ${p.cpu.toFixed(1).padStart(5)}  ${p.mem.toFixed(1).padStart(5)}  ${p.name}`);
