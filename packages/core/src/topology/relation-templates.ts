...
const RELATION_TEMPLATE_META: Record<EnterpriseRelationType, Omit<TopologyRelationTemplatePreset, "allowedPairs">> = {
  delegates_to: {
...
export const TOPOLOGY_RELATION_TEMPLATE_CATALOG: TopologyRelationTemplateCatalog = {
  schemaVersion: 1,
...
function smartConnectReasonKo(
  preset: TopologyRelationTemplatePreset,
  direction: TopologySmartConnectDirection,
): string {
  if (direction === "target_to_source") return "반대 방향으로 연결하면 자연스럽다."
  if (preset.runtimeCandidate) return "실행 경로로 사용할 수 있는 연결이다."
  return "이 두 항목에 사용할 수 있는 연결이다."
}

function smartConnectReasonEn(
  preset: TopologyRelationTemplatePreset,
  direction: TopologySmartConnectDirection,
): string {
  if (direction === "target_to_source") return "This connection is natural in the reverse direction."
  if (preset.runtimeCandidate) return "This connection can be used as a runtime path."
