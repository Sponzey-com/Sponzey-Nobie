...
export const TOPOLOGY_COMPILER_VERSION = "enterprise-topology-compiler.v1" as const
...
export type CompileTopologyResult =
  | { ok: true; snapshot: CompiledTopologySnapshot; validation: TopologyValidationResult }
  | { ok: false; validation: TopologyValidationResult; issues: TopologyValidatorIssue[] }

interface DelegationGraph {
  edges: Record<string, string[]>
  parents: Record<string, string[]>
  relationIdsByPair: Map<string, string[]>
}

interface ToolScopeBuilder {
  declaredToolIds: Set<string>
  declaredSystemIds: Set<string>
  backingSystemIds: Set<string>
  toolRelationIds: Set<string>
  systemRelationIds: Set<string>
}

export function compileTopology(topology: EnterpriseTopology, options: CompileTopologyOptions = {}): CompileTopologyResult {
  ...
  const compiledTopologySnapshotId =
    options.compiledTopologySnapshotId ?? buildCompiledTopologySnapshotId(topology.id, sourceTopologyVersion, sourceTopologyHash)
  ...
  const snapshot: CompiledTopologySnapshot = {
    ...
    } satisfies CompiledAuthorityRule])),
    parentChildTree,
    delegationScopeMap,
...
export function compileTopologyOrThrow(topology: EnterpriseTopology, options: CompileTopologyOptions = {}): CompiledTopologySnapshot {
...
export function getCompiledEntryNode(snapshot: CompiledTopologySnapshot): CompiledNode | undefined {
...
export function getCompiledChildCandidates(snapshot: CompiledTopologySnapshot, nodeId: string): CompiledNode[] {
...
export function buildCompiledEntityRefKey(reference: EnterpriseEntityRef): string {
  return `${reference.entityType}|${reference.id}`
}

export function normalizeSourceTopologyVersion(
...
export function computeTopologySourceHash(topology: EnterpriseTopology): string {
  return createHash("sha256").update(stableStringify(topology)).digest("hex")
}

export function buildCompiledTopologySnapshotId(
  topologyId: string,
  sourceTopologyVersion: string,
  sourceTopologyHash: string,
): string {
...
function buildNodeIndex(topology: EnterpriseTopology, tree: CompiledDelegationTree): Record<string, CompiledNode> {
...
function buildDelegationGraph(topology: EnterpriseTopology): DelegationGraph {
...
function buildCompiledDelegationTree(topology: EnterpriseTopology, graph: DelegationGraph): CompiledDelegationTree {
...
function buildDelegationScopeMap(
  topology: EnterpriseTopology,
  tree: CompiledDelegationTree,
): Record<string, CompiledDelegationScope> {
  return Object.fromEntries(topology.nodes.map((node) => {
    ...
    return [node.id, {
      ...
      descendantNodeIds,
      maxDepth,
    } satisfies CompiledDelegationScope]
  }))
}

function collectDescendants(nodeId: string, edges: Record<string, string[]>): { descendantNodeIds: string[]; maxDepth: number } {
  ...
  for (let index = 0; index < queue.length; index += 1) {
    ...
    if (item === undefined || visited.has(item.nodeId)) continue
...
function buildAuthorityScopeIndex(topology: EnterpriseTopology): Record<string, CompiledAuthorityScope> {
  const index: Record<string, CompiledAuthorityScope> = {}

  topology.relations.forEach((relation) => {
    if (relation.relationType !== "approves") return
...
function ensureAuthorityScope(
  index: Record<string, CompiledAuthorityScope>,
  target: EnterpriseEntityRef,
...
function buildToolScopeIndex(topology: EnterpriseTopology): Record<string, CompiledToolScope> {
  const builders: Record<string, ToolScopeBuilder> = Object.fromEntries(topology.nodes.map((node) => [node.id, {
    declaredToolIds: new Set<string>(),
    declaredSystemIds: new Set<string>(),
    backingSystemIds: new Set<string>(),
    toolRelationIds: new Set<string>(),
    systemRelationIds: new Set<string>(),
  }]))
  ...
  topology.relations.forEach((relation) => {
    ...
    if (relation.relationType === "uses_tool" && relation.to.entityType === "enterprise_tool") {
      ...
      if (tool?.systemId !== undefined) builder.backingSystemIds.add(tool.systemId)
    }
  ...
  return Object.fromEntries(topology.nodes.map((node) => {
    ...
    } satisfies CompiledToolScope]
  }))
}

function dataDomainIdsForSystems(
  systemById: ReadonlyMap<string, EnterpriseTopology["systems"][number]>,
  ...
  for (const systemId of systemIds) {
    ...
    for (const domainId of system.dataDomainIds) {
      if (!domains.includes(domainId)) domains.push(domainId)
    }
  }
  return domains
}

function buildProcessFlowIndex(topology: EnterpriseTopology): Record<string, CompiledProcessFlow> {
  const index: Record<string, CompiledProcessFlow> = Object.fromEntries(topology.processes.map((process) => [process.id, {
    processId: process.id,
    ...(process.ownerNodeId !== undefined ? { ownerNodeId: process.ownerNodeId } : {}),
    stepNodeIds: [...process.stepNodeIds],
    ...(process.accountablePositionId !== undefined ? { accountablePositionId: process.accountablePositionId } : {}),
    transitionRelationIds: [],
  } satisfies CompiledProcessFlow]))
  ...
  topology.relations.forEach((relation) => {
    ...
    if (relation.from.entityType === "process_definition") {
      ...
      if (flow !== undefined) flow.transitionRelationIds = addUnique(flow.transitionRelationIds, relation.id)
    }
...
function buildResponsibilityIndex(topology: EnterpriseTopology): CompiledResponsibilityIndex {
  const byScopeKey: Record<string, CompiledResponsibilityScope> = {}
  const byResponsibleKey: Record<string, string[]> = {}
  ...
  topology.responsibilities.forEach((entry) => {
    ...
    if (entry.accountable !== undefined) existing.accountableRefs = addUniqueRefs(existing.accountableRefs, [entry.accountable])
...
function cloneRef<T extends EnterpriseEntityRef>(reference: T): T {
  return { entityType: reference.entityType, id: reference.id } as T
}
...
function addUniqueRefs<T extends EnterpriseEntityRef>(values: readonly T[], nextValues: readonly T[]): T[] {
  const result = [...values]
  for (const nextValue of nextValues) {
