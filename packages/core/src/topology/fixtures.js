...
import { createTopologyDocumentEnvelope, } from "./draft-store.js";
function fixtureIssue(path, code, message) {
    return { path, code, reasonCode: code, message };
}
export function inferTopologyFixtureFormat(sourceRef) {
...
export function parseTopologyDocumentText(text, options = {}) {
    ...
    try {
        if (format === "yaml") {
            return {
                ...
                format,
                issues: [
                    fixtureIssue("$", "unsupported_topology_fixture_format", "YAML topology fixtures are reserved for import helpers and require a parser before runtime use."),
                ],
            };
        }
        ...
        return {
            ...
            format,
            issues: [
                fixtureIssue("$", "topology_fixture_parse_failed", error instanceof Error ? error.message : "Failed to parse topology fixture."),
            ],
        };
    }
    const validation = validateEnterpriseTopology(parsed);
    if (!validation.ok)
        return { ok: false, format, issues: validation.issues };
    return { ok: true, topology: validation.value, issues: [], format };
}
export function loadTopologyFixtureFile(filePath) {
...
export function createTopologyFixtureStore(records) {
    ...
    return {
        ...
        require(fixtureId) {
            const record = fixtures.get(fixtureId);
            if (!record)
                throw new Error(`Missing topology fixture: ${fixtureId}`);
            return structuredClone(record);
        },
    };
}
export function loadTopologyFixtureDirectory(directoryPath) {
