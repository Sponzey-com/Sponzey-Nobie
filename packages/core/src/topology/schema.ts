...
export const TOPOLOGY_VALIDATOR_BLOCKING_SEVERITIES = ["blocked", "invalid"] as const
...
const PEOPLE_OR_STRUCTURE_ENTITIES = ["node", "team", "org_unit", "position", "person"] as const
const WORK_TARGET_ENTITIES = ["node", "process_definition", "enterprise_system", "enterprise_tool"] as const
const AUTHORITY_HOLDER_ENTITIES = ["position", "person", "org_unit"] as const
const ACCOUNTABLE_HOLDER_ENTITIES = ["position", "person", "org_unit", "node"] as const
const EXECUTION_DEPENDENCY_ENTITIES = ["node", "process_definition", "enterprise_system", "enterprise_tool"] as const

function includesEntityType(values: readonly EnterpriseEntityType[], value: EnterpriseEntityType): boolean {
  return values.includes(value)
}

function endpointPairs(
  fromValues: readonly EnterpriseEntityType[],
  toValues: readonly EnterpriseEntityType[],
...
} as const satisfies Record<EnterpriseRelationType, EnterpriseRelationEndpointRule>

export function isEnterpriseRelationEndpointAllowed(
  relationType: EnterpriseRelationType,
  from: EnterpriseEntityType,
  to: EnterpriseEntityType,
): boolean {
