...
export type DelegationPathValidationStatus =
  | "not_requested"
  | "not_checked"
  | "valid"
...
function normalizeConnectionPath(currentExecutorId: string, path: string[]): string[] {
...
export function delegationCandidatesFromRegistry(input: DelegationRegistryCandidateInput): DelegationCandidate[] {
...
function availabilityScore(availability: DelegationCandidate["availability"]): number {
  if (availability === "available") return 4
  if (availability === "busy") return 3
  if (availability === "permission_required") return 2
  return 1
}
...
function candidateFromAgent(
  ...
  taskAnalysis: DelegationRegistryCandidateInput["taskAnalysis"],
): DelegationCandidate {
...
function agentAvailability(agent: AgentRegistryEntry): DelegationCandidate["availability"] {
  if (agent.status !== "enabled" || !agent.delegationEnabled) return "offline"
  if (!agent.capabilitySummary.available || !agent.modelSummary.available) return "permission_required"
  if (agent.currentLoad.activeSubSessions >= agent.currentLoad.maxParallelSessions) return "busy"
  return "available"
}

function teamAvailability(team: TeamRegistryEntry): DelegationCandidate["availability"] {
  if (team.status !== "enabled") return "offline"
  if (team.activeMemberAgentIds.length === 0) return "busy"
  if ((team.coverage?.capabilityCoverage.missing.length ?? 0) > 0) return "permission_required"
  return "available"
}
