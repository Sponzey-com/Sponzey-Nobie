...
import type { ExecutorConnectionDraft, ExecutorDraft, ExecutorGraphWorkspace } from "./executor-graph.js"
import { createExecutorDraftFromInference } from "./executor-inference.js"
import {
  redactNodeDefinitionSuggestionRequest,
  type NodeDefinitionRedactionMode,
} from "./node-definition-redaction.js"
...
export type NodeDefinitionDialogState =
  | "idle"
  | "editing_prompt"
  | "loading"
...
export interface NodeDefinitionSuggestionWarning {
  code:
    ...
    | "internal_term_removed"
    | "alternative_too_short"
...
export type NodeDefinitionSuggestionErrorCode =
  | "llm_not_configured"
  | "llm_response_invalid"
  | "no_target_fields"
  | "redaction_failed"
...
export type NodeDefinitionSuggestionResult =
...
export function normalizeNodeDefinitionQuickChips(values: unknown): string[] {
...
export function initialNodeDefinitionQuickChipsFromDraft(
  draft: Pick<
    NodeDefinitionDraft,
    "name" | "description" | "capabilityHints" | "understandingSummary" | "quickChips"
  >,
): string[] {
...
export function defaultNodeDefinitionFieldLocks(
  overrides: Partial<NodeDefinitionFieldLocks> = {},
...
export function fieldLocksForNodeDefinitionTrigger(
  triggerField: NodeDefinitionTriggerField,
  userLocks: Partial<NodeDefinitionFieldLocks> = {},
): NodeDefinitionFieldLocks {
  if (triggerField === "whole_node") return defaultNodeDefinitionFieldLocks(userLocks)
  ...
  if (triggerField === "description") locks.name = true
...
export function targetFieldsForNodeDefinitionTrigger(
  triggerField: NodeDefinitionTriggerField,
  locks: NodeDefinitionFieldLocks,
): NodeDefinitionField[] {
  if (triggerField === "whole_node") return NODE_DEFINITION_FIELDS.filter((field) => !locks[field])
  return locks[triggerField] ? [] : [triggerField]
}

export function nodeDefinitionDraftFromExecutor(
  executor: ExecutorDraft,
  locks: Partial<NodeDefinitionFieldLocks> = {},
): NodeDefinitionDraft {
  return {
...
export function buildNodeDefinitionGraphContext(input: {
  ...
  const incoming: NodeContextSummary[] = []
  const outgoing: NodeContextSummary[] = []
...
export function normalizeNodeDefinitionSuggestionRequest(
...
export function buildNodeDefinitionPromptInput(request: NodeDefinitionSuggestionRequest): string {
...
function splitDefinitionQuickChips(chips: string[]): { roles: string[]; styles: string[]; extra: string[] } {
  const roleSet = new Set<string>(NODE_DEFINITION_ROLE_CHIPS)
  const styleSet = new Set<string>(NODE_DEFINITION_STYLE_CHIPS)
  const roles: string[] = []
  const styles: string[] = []
  const extra: string[] = []
  for (const chip of chips) {
    if (roleSet.has(chip)) roles.push(chip)
    else if (styleSet.has(chip)) styles.push(chip)
    else extra.push(chip)
  }
  return { roles, styles, extra }
}

export function applyNodeDefinitionAlternative(
  input: ApplyNodeDefinitionAlternativeInput,
  ...
  const diff: NodeDefinitionDraftDiffItem[] = []
  const appliedFields: NodeDefinitionField[] = []
  const ignoredLockedFields: NodeDefinitionField[] = []
  for (const field of NODE_DEFINITION_FIELDS) {
    ...
    if (isEmptyImportantPatch(field, nextValue)) continue
    const before = draft[field]
    if (JSON.stringify(before) === JSON.stringify(nextValue)) continue
    ;(draft as unknown as Record<NodeDefinitionField, unknown>)[field] = nextValue
...
export async function createNodeDefinitionSuggestion(
  input: {
    request: Partial<NodeDefinitionSuggestionRequest>
...
export function validateNodeDefinitionSuggestionPayload(input: {
  ...
  const warnings: NodeDefinitionSuggestionWarning[] = []
  const alternatives = parsed.alternatives
    .slice(0, 3)
    .map((item, index) => normalizeAlternative(item, index, input.request, warnings))
    .filter((item): item is NodeDefinitionAlternative => item !== null)
...
function buildNodeDefinitionPrompt(request: NodeDefinitionSuggestionRequest): string {
  return [
...
function resolveSuggestionModelInfo(
  request: NodeDefinitionSuggestionRequest,
  modelConfig: AIConnectionConfig | undefined,
): NodeDefinitionSuggestionResponse["modelInfo"] | null {
...
function buildDeterministicAlternativePayload(request: NodeDefinitionSuggestionRequest): unknown {
...
function normalizeAlternative(
  ...
  request: NodeDefinitionSuggestionRequest,
  warnings: NodeDefinitionSuggestionWarning[],
): NodeDefinitionAlternative | null {
  ...
  if (summary.length < 4) warnings.push({ code: "alternative_too_short", message: `${title} 요약이 너무 짧습니다.` })
  return {
...
function fallbackRoleNameForAlternative(
  ...
  if (explicit) return truncate(cleanUserFacingText(explicit, warnings), 32)
  const quickChipGroups = splitDefinitionQuickChips(request.quickChips)
...
function normalizeAlternativePatch(
  patch: unknown,
  request: NodeDefinitionSuggestionRequest,
  warnings: NodeDefinitionSuggestionWarning[],
): NodeDefinitionAlternative["patch"] {
  if (!patch || typeof patch !== "object" || Array.isArray(patch)) return {}
  const record = patch as Record<string, unknown>
  const output: NodeDefinitionAlternative["patch"] = {}
  for (const [key, value] of Object.entries(record)) {
    ...
    if (!request.targetFields.includes(key)) continue
    const normalized = normalizePatchField(key, value)
    if (isEmptyImportantPatch(key, normalized)) continue
...
function normalizeDraft(value: unknown): NodeDefinitionDraft {
...
function normalizeFieldLocks(value: unknown): NodeDefinitionFieldLocks {
  const record = value && typeof value === "object" && !Array.isArray(value) ? value as Record<string, unknown> : {}
  return defaultNodeDefinitionFieldLocks(Object.fromEntries(
    NODE_DEFINITION_FIELDS.map((field) => [field, record[field] === true]),
  ) as Partial<NodeDefinitionFieldLocks>)
}
...
function normalizeTargetFields(
  ...
  const raw = Array.isArray(values) ? values.filter((value): value is NodeDefinitionField => isNodeDefinitionField(value)) : []
...
function normalizeGraphContext(value: unknown): NodeDefinitionGraphContext {
...
function normalizeContextSummaries(value: unknown, direction: "incoming" | "outgoing"): NodeContextSummary[] {
...
function normalizeSuggestionHistory(value: unknown): NodeDefinitionSuggestionHistoryItem[] {
  if (!Array.isArray(value)) return []
  return value
    .filter((item): item is Record<string, unknown> => Boolean(item) && typeof item === "object" && !Array.isArray(item))
    .map((item) => ({
...
function parseRawSuggestionPayload(raw: unknown): Record<string, unknown> {
...
function cleanUserFacingText(value: string, warnings: NodeDefinitionSuggestionWarning[]): string {
  let current = value
...
function hasTooSimilarAlternatives(alternatives: NodeDefinitionAlternative[]): boolean {
...
function isTriggerField(value: unknown): value is NodeDefinitionTriggerField {
  return value === "whole_node" || isNodeDefinitionField(value)
}

function isNodeDefinitionField(value: unknown): value is NodeDefinitionField {
  return typeof value === "string" && (NODE_DEFINITION_FIELDS as readonly string[]).includes(value)
}

function normalizePatchField(field: NodeDefinitionField, value: unknown): string | string[] {
...
function isEmptyImportantPatch(field: NodeDefinitionField, value: unknown): boolean {
...
function isRateLimitSuggestionError(error: unknown): boolean {
  if (!error || typeof error !== "object") return false
  const record = error as { status?: unknown; code?: unknown; message?: unknown }
  if (record.status === 429) return true
  const value = [record.code, record.message].filter((item): item is string => typeof item === "string").join(" ")
