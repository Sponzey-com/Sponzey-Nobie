...
export const TOPOLOGY_FLOW_TEMPLATES = [
    {
...
export const TOPOLOGY_TEMPLATE_CATALOG = {
    schemaVersion: 1,
    nodePresets: [
        {
...
export function buildTopologyFlowTemplateDraft(templateId, input = {}) {
    ...
    if (templateId === "customer-request-flow") {
        ...
        pushRelations(topology, flowRelation("relation:customer-request-intake-review", "다음 업무", "delegates_to", "node", intake.id, "node", review.id, now), flowRelation("relation:customer-request-review-response", "다음 업무", "delegates_to", "node", review.id, "node", response.id, now));
        return topology;
    }
    if (templateId === "approval-request-flow") {
        ...
        pushRelations(topology, flowRelation("relation:approval-request-step", "승인 단계", "delegates_to", "node", request.id, "node", approval.id, now), flowRelation("relation:approver-approval-step", "승인", "approves", "org_unit", approver.id, "node", approval.id, now), flowRelation("relation:approval-request-approver", "승인 그룹 알림", "informs", "node", request.id, "org_unit", approver.id, now));
        return topology;
    }
...
function flowTemplateById(templateId) {
...
function flowNode(id, name, nodeType, now, template, options) {
    return {
        schemaVersion: ENTERPRISE_TOPOLOGY_SCHEMA_VERSION,
        entityType: "node",
        id,
...
function flowOrgUnit(id, name, now) {
    return {
        schemaVersion: ENTERPRISE_TOPOLOGY_SCHEMA_VERSION,
        entityType: "org_unit",
        id,
...
function flowApprovalRule(id, approverOrgUnitId, nodeId, now) {
    return {
        schemaVersion: ENTERPRISE_TOPOLOGY_SCHEMA_VERSION,
...
function nodeResponsibility(node, now) {
    return {
        schemaVersion: ENTERPRISE_TOPOLOGY_SCHEMA_VERSION,
...
function pushNodes(topology, ...nodes) {
    topology.nodes.push(...nodes);
    topology.responsibilities.push(...nodes.map((node) => nodeResponsibility(node, node.createdAt)));
}
function pushRelations(topology, ...relations) {
    topology.relations.push(...relations);
}
function flowRelation(id, label, relationType, fromType, fromId, toType, toId, now) {
    return {
        schemaVersion: ENTERPRISE_TOPOLOGY_SCHEMA_VERSION,
        entityType: "relation",
        id,
