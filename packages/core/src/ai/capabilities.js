...
import { resolveAIConnection, } from "./index.js";
...
export function resolveEmbeddingProviderResolutionSnapshot(memory) {
...
function resolveContextWindow(resolution) {
    ...
    if (model.includes("claude"))
        return 200_000;
    if (resolution.providerId === "ollama" || resolution.providerId === "llama")
        return null;
    return 128_000;
}
function detectEndpointMismatch(resolution) {
    if (!resolution.providerId)
        return capability("unknown", "provider가 설정되지 않았습니다.");
...
export function getProviderCapabilityMatrix(params) {
    ...
    if (!params.forceRefresh && !params.checkResult && cached && Date.parse(cached.expiresAt) > now.getTime())
        return cached;
...
export function attachCapabilityProfileToTrace(trace, matrix) {
    return {
        ...trace,
