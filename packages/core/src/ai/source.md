...
- `ai.defaultProvider`나 `ai.defaultModel` 같은 기본 선호값만으로 외부 AI backend가 활성화되지는 않습니다.
- 자동 선택과 fallback은 설정 파일에 실제로 저장된 backend만 후보로 삼고, 환경변수만으로 외부 AI backend를 암묵 활성화하지 않습니다.
