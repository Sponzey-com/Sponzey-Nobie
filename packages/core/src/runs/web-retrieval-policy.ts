...
function canonicalUrl(value: unknown): { href: string; domain: string | null } {
  ...
  if (!raw) return { href: "", domain: null }
  try {
    ...
    for (const [key, nested] of entries) next.searchParams.append(key, nested)
...
function freshnessPolicyFromParam(value: unknown): SourceFreshnessPolicy | null {
  if (value === "normal" || value === "latest_approximate" || value === "strict_timestamp") return value
  return null
}

function inferFreshnessPolicyFromSource(input: { href: string; domain: string | null; method: WebRetrievalMethod; sourceKind: SourceKind }): SourceFreshnessPolicy {
  if (input.sourceKind === "search_index" || input.sourceKind === "browser_evidence") return "latest_approximate"
  const href = input.href.toLowerCase()
  const domain = input.domain ?? ""
  if (domain === "www.google.com" && href.includes("/finance/quote/")) return "latest_approximate"
  if (domain.endsWith("investing.com") && /\/(indices|currencies|equities|crypto)\//i.test(href)) return "latest_approximate"
  if (domain === "finance.yahoo.com" && href.includes("/quote/")) return "latest_approximate"
  if (domain.endsWith("finance.naver.com") || domain.endsWith("finance.daum.net")) return "latest_approximate"
  return "normal"
}

function sourceKindFromDomain(domain: string | null, method: WebRetrievalMethod): SourceKind {
  if (method === "browser_search") return "browser_evidence"
  if (!domain) return method === "fast_text_search" ? "search_index" : "unknown"
  if (/(go\.kr|gov|kma\.go\.kr|weather\.go\.kr|data\.go\.kr|law\.go\.kr|moleg\.go\.kr)$/i.test(domain)) return "official"
  if (/(openai\.com|anthropic\.com|google\.com|microsoft\.com|apple\.com)$/i.test(domain)) return "first_party"
  return method === "fast_text_search" ? "search_index" : "third_party"
}

function reliabilityFor(kind: SourceKind): SourceReliability {
  switch (kind) {
...
export function buildWebRetrievalPolicyDecision(input: WebRetrievalPolicyInput): WebRetrievalPolicyDecision | null {
  ...
  const method: WebRetrievalMethod = waitForSelector || mode === "screenshot" ? "browser_search" : "direct_fetch"
