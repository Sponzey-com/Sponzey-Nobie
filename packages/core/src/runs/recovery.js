...
function normalizeCommandFailureKey(toolName, output, params) {
...
function commandFailureTargetFingerprint(params) {
...
export function describeRecoveryAlternatives(alternatives) {
...
export function describeAssistantTextDeliveryFailure(params) {
    const channelLabel = params.source === "telegram"
        ? "텔레그램"
        : params.source === "webui"
            ? "WebUI"
            : params.source === "slack"
...
function buildPathAliasRecoveryHintLines(originalRequest, failedTools) {
    const combined = [
        originalRequest,
...
function mentionsDownloadLocation(value) {
    return /(downloads?|download\s*folder|다운로드|다운\s*로드|다운도르|다운\s*도르)/iu.test(value);
}
function stringifyRecoveryParams(value) {
...
export function describeWorkerRuntimeErrorReason(message) {
...
export function buildAiRecoveryAvoidTargets(targetId, workerRuntimeKind) {
...
function normalizeAiRecoveryFingerprint(reason, message) {
    ...
    if (/rate limit|too many requests|429/i.test(combined))
        return "rate-limit";
    if (/cloudflare|challenge|auth|unauthorized|forbidden|401|403|api key/i.test(combined))
        return "auth";
    if (/context|token|too large|max context|maximum context/i.test(combined))
        return "context-limit";
    if (/schema|parameter|unsupported|invalid_request|tool|function/i.test(combined))
...
function inferGenericExecutionAlternatives(toolNames) {
...
function inferDirectArtifactDeliveryAlternatives(source) {
    const alternatives = [{ kind: "same_channel_retry", label: "같은 채널 재전송 시도" }];
    alternatives.push({ kind: "other_tool", label: "다른 전달 도구 또는 다른 실행 경로 검토" });
    return alternatives;
}
function isExtensionPreferredToolName(toolName) {
    return /^(screen_capture|mouse_|keyboard_|shell_exec|app_launch|process_kill|yeonjang_)/.test(toolName);
}
function isScheduleLikeToolName(toolName) {
    return /schedule/i.test(toolName);
}
function dedupeRecoveryAlternatives(alternatives) {
    const seen = new Set();
    const result = [];
...
export function shouldRetryTruncatedOutput(params) {
    if (params.review.status !== "ask_user")
        return false;
    if (!params.requiresFilesystemMutation)
        return false;
    const combined = [
