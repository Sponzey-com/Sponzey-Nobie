...
async function reviewDelegatedChildCompletion(input) {
    ...
    return reviewer({
        ...
        input.dependencies.appendRunEvent(input.params.runId, `parent_run_child_result_review_failed:intake_followup;child_run=${input.childRunId};error=${error instanceof Error ? error.message : String(error)}`);
        return null;
    });
}
...
export async function runIntakeBridgePass(params, dependencies, moduleDependencies = defaultModuleDependencies) {
    ...
    if (scheduleActions.length > 0 || delegatedActions.length > 0 || intake.intent.category === "schedule_request") {
        ...
        for (const delegatedAction of delegatedActions) {
            ...
            const route = decisionRoute.kind === "explicit_provider_target" || decisionRoute.kind === "delegate_to_child"
                ? decisionRoute.route
                : undefined;
            if (!route)
                continue;
