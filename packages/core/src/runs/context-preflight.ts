...
function collectBlockedCompactionReasons(
  messages: Message[],
  deterministicState: ReturnType<typeof extractRootSessionDeterministicState>,
): string[] {
  const blockedReasonCodes: string[] = []
  if (!hasBalancedToolUsePairs(messages)) blockedReasonCodes.push("blocked_by_unmatched_tool_pair")
...
async function selectCompactedWindow(input: {
  ...
  capsule: Parameters<typeof rewriteRootSessionActiveWindow>[0]["capsule"]
}): Promise<{
  messages: Message[]
  preflight: ContextPreflightResult
  restorePathCodes: Array<"maintenance_restore" | "prompt_time_recall">
  degradeMode?: "smaller_raw_tail" | "retrieval_only"
  retrievalSnippetCount?: number
  degradedTailMessageCount?: number
  ...
  let degradedTailMessageCount: number | undefined
  let degradeMode: "smaller_raw_tail" | "retrieval_only" | undefined
