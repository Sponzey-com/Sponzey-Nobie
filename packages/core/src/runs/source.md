...
- `preflight.ts`는 execution queue 진입 전의 빠른 실패 경계입니다. AI 연결 없음, 기본 모델 없음, Telegram/Slack 런타임 중지, Yeonjang 실행이 필요한 요청에서 연결된 연장 스냅샷 없음은 큐에 넣지 않고 실패 메시지를 채널에 직접 전달합니다.
