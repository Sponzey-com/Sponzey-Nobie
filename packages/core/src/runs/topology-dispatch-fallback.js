...
function isPromptPreflightReason(reasonCode) {
...
export function resolveTopologyDispatchFollowupDecision(input) {
    const topologyOutcomes = input.dispatchResult.outcomes.filter(isTopologyOutcome);
    if (input.dispatchResult.attempted <= 0 || topologyOutcomes.length === 0)
        return undefined;
    if (topologyOutcomes.some((outcome) => outcome.status === "completed"))
        return undefined;
...
function topologyNodeIdFromOutcome(input) {
    if (input.outcome.topologyExecutorId?.trim())
        return input.outcome.topologyExecutorId;
...
function insertTraceEvent(db, event) {
    db.prepare(`INSERT INTO topology_trace_events
     ...
     ON CONFLICT(trace_event_id) DO UPDATE SET
       ...
       at = excluded.at,
       sequence = excluded.sequence`).run(event.traceEventId, event.topologyRunId, event.nodeRunId, event.workOrderId, null, event.phase, event.component, event.reasonCode, JSON.stringify(event.delegationPath), event.payload !== undefined ? JSON.stringify(event.payload) : null, JSON.stringify(event), event.at, event.sequence);
}
export function recordTopologyDispatchFollowupTrace(input) {
    ...
    const tx = db.transaction(() => {
       ...
       ON CONFLICT(topology_run_id) DO UPDATE SET
         ...
         metadata_json = excluded.metadata_json`).run(topologyRunId, input.topologyId, input.topologyVersion ?? null, null, input.runId, topologyStatus, input.entryNodeId, now, now + events.length, now, now + events.length, JSON.stringify({
        ...
        for (const outcome of selectedOutcomes) {
         ...
         ON CONFLICT(work_order_id) DO UPDATE SET
           ...
           work_order_json = excluded.work_order_json`).run(workOrderId, topologyRunId, nodeRunId, null, ROOT_NODE_ID, "node", nodeId, JSON.stringify([ROOT_NODE_ID, nodeId]), JSON.stringify({
                workOrderId,
                topologyRunId,
...
function followupTracePhase(action) {
    switch (action) {
        case "self_solve":
