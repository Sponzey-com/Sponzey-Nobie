...
function isRecoveryAttemptKind(kind) {
    return kind === "filesystem_retry" || kind === "truncated_recovery";
}
function mapRecoveryKind(kind) {
    switch (kind) {
...
function resolveAttemptExecutionKind(kind) {
    switch (kind) {
        case "primary":
            return "new_request";
        case "followup":
...
function resolveRunRelationshipKind(run, kind, index) {
    switch (kind) {
        case "primary":
            return "root_request";
        case "verification":
...
function computeTaskStatus(params) {
    ...
    const activeVisibleAttempts = params.attempts.filter((attempt) => attempt.userVisible && ACTIVE_RUN_STATUSES.includes(attempt.status));
    const activeAttempts = params.attempts.filter((attempt) => ACTIVE_RUN_STATUSES.includes(attempt.status));
    if (activeVisibleAttempts.some((attempt) => attempt.status === "awaiting_approval"))
        return "awaiting_approval";
    if (activeVisibleAttempts.some((attempt) => attempt.status === "awaiting_user"))
        return "awaiting_user";
    if (activeVisibleAttempts.some((attempt) => attempt.status === "running"))
        return "running";
    if (activeVisibleAttempts.some((attempt) => attempt.status === "queued"))
        return "queued";
    if (params.delivery.status === "delivered" && activeVisibleAttempts.length === 0)
        return "completed";
    if (activeAttempts.some((attempt) => attempt.status === "awaiting_approval"))
        return "awaiting_approval";
    if (activeAttempts.some((attempt) => attempt.status === "awaiting_user"))
        return "awaiting_user";
    if (activeAttempts.some((attempt) => attempt.status === "running"))
        return "running";
    if (activeAttempts.some((attempt) => attempt.status === "queued"))
        return "queued";
    ...
    if (statuses.includes("awaiting_approval"))
        return "awaiting_approval";
    if (statuses.includes("awaiting_user"))
        return "awaiting_user";
...
function computeTaskSummary(groupRuns) {
    const activeRun = groupRuns
        .filter((run) => ACTIVE_RUN_STATUSES.includes(run.status))
        .sort((a, b) => b.updatedAt - a.updatedAt)[0];
    if (activeRun?.summary?.trim())
        return activeRun.summary.trim();
...
function computeTaskRequest(groupRuns) {
    ...
    if (latestUserFacingRun?.prompt.trim())
...
function mapRunStatusToChecklistStatus(status) {
    switch (status) {
        case "queued":
            return "pending";
        case "running":
...
function isExecutionAttemptKind(kind) {
    switch (kind) {
        case "primary":
        case "followup":
        case "approval_continuation":
...
function mapTerminalFailureStatus(status) {
    return status === "cancelled" || status === "interrupted" ? "cancelled" : "failed";
}
function expandDisplayPath(value) {
    if (value.startsWith("~/"))
...
function resolveTaskDeliverySignal(orderedRuns, attempts) {
    ...
    const failedEvent = recentEvents.find((event) => /(전달.*실패|실패.*전달|완료 신호 전달에 실패|delivery failed)/i.test(event.label));
    if (failedEvent) {
        return {
            status: "failed",
            ...(sourceAttemptId ? { sourceAttemptId } : {}),
...
function isRecoveryAttemptStatusActive(status) {
    return ACTIVE_RUN_STATUSES.includes(status);
}
function resolveTaskActivityKind(status, recovery) {
    switch (status) {
        case "awaiting_approval":
...
function buildTaskActivities(taskId, attempts, orderedRuns) {
    ...
    if (deliverySignal.status !== "not_requested" && deliverySignal.at !== undefined) {
        activities.push({
            ...
            taskId,
            kind: deliverySignal.status === "pending"
                ? "delivery.pending"
...
function buildTaskMonitor(attempts, recoveryAttempts, delivery) {
...
function mapContinuitySnapshot(snapshot) {
    if (!snapshot)
        return undefined;
...
function extractPromptSourceIds(snapshot) {
    return extractPromptSources(snapshot).map((source) => source.sourceId);
}
function extractPromptSources(snapshot) {
...
function extractPromptSourceVersion(snapshot) {
...
function getFailureDetailLines(run, summary) {
...
function describeFailureOutcome(status) {
    switch (status) {
        case "cancelled":
            return "취소";
        case "interrupted":
...
function describeAttemptFailureTitle(kind, status) {
...
function deriveTaskFailure(orderedRuns, attempts, delivery) {
    ...
    const failedRun = [...orderedRuns]
        ...
        .find((run) => run.status === "failed" || run.status === "cancelled" || run.status === "interrupted");
    if (!failedRun)
        return undefined;
