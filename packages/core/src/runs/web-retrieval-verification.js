...
function containsExactNormalized(haystack, needle) {
...
function asText(content) {
    ...
    if (Array.isArray(content))
        return content.map((item) => asText(item)).join("\n");
...
function targetLabels(target) {
    ...
    for (const symbol of target.symbols ?? []) {
        if (symbol.trim())
            labels.push({ kind: "symbol", value: symbol, weight: 0.45, field: "target.symbols" });
    }
    if (target.canonicalName?.trim())
        labels.push({ kind: "canonical_name", value: target.canonicalName, weight: 0.45, field: "target.canonicalName" });
    if (target.locationName?.trim())
        labels.push({ kind: "location", value: target.locationName, weight: 0.45, field: "target.locationName" });
    return labels;
}
function firstTargetLabelNearValue(vicinity, target) {
    for (const label of targetLabels(target)) {
        if (containsExactNormalized(vicinity, label.value))
            return label.value;
    }
    if (target.rawQuery && containsExactNormalized(vicinity, target.rawQuery))
...
function addSignal(signals, signal) {
    ...
    if (signals.some((existing) => `${existing.kind}:${normalizeForMatch(existing.value)}:${existing.evidenceField}` === dedupeKey))
        return;
    signals.push(signal);
}
function buildBindingSignals(input) {
    ...
    if (input.unit)
        addSignal(signals, { kind: "unit", value: input.unit, weight: 0.08, evidenceField: "candidate.unit" });
...
function detectUnit(raw) {
    ...
    if (/%/u.test(raw))
        return "percent";
    if (/포인트|points?|pts?|pt/iu.test(raw))
        return "point";
    if (/₩|원|KRW/iu.test(raw))
        return "KRW";
    if (/\$|달러|USD/iu.test(raw))
        return "USD";
    if (/€|EUR/iu.test(raw))
        return "EUR";
    if (/¥|JPY/iu.test(raw))
...
export function extractRetrievedValueCandidates(input) {
    if (!input.sourceEvidenceId.trim())
        throw new Error("sourceEvidenceId is required for candidate extraction");
    if (!input.sourceEvidence)
        throw new Error("sourceEvidence is required for candidate extraction");
...
function sourceIsOfficialEnough(source) {
    if (!source)
        return false;
    return source.sourceKind === "official" || source.sourceKind === "first_party";
}
function signalKinds(candidate) {
    return new Set(candidate.bindingSignals.map((signal) => signal.kind));
}
function classifyBindingStrength(candidate) {
    const kinds = signalKinds(candidate);
    if (kinds.has("symbol") || kinds.has("canonical_name") || kinds.has("location"))
        return "strong";
    if (kinds.has("quote_card") || kinds.has("table_row") || kinds.has("page_title") || kinds.has("url_path"))
        return "acceptable";
    if (kinds.has("unit") || kinds.has("timestamp"))
        return "weak";
    return "none";
}
function targetConflictBucket(target) {
    ...
    for (const group of CONFLICT_GROUPS) {
        for (const [bucket, labels] of Object.entries(group)) {
            if (labels.some((label) => containsExactNormalized(text, label)))
                return bucket;
        }
    }
    return null;
}
function candidateConflictBuckets(candidate) {
...
function detectConflicts(candidate, target) {
...
export function verifyRetrievedValueCandidate(input) {
    if (!input.candidate)
        return missingCandidateVerdict(input.target, input.policy);
...
export function verifyRetrievedValueCandidates(input) {
    ...
    if (conflictResolution.status === "selected" && conflictResolution.selectedVerdict)
        return conflictResolution.selectedVerdict;
...
export function sourceKindSatisfiesOfficialRequired(kind) {
    return kind === "official" || kind === "first_party";
}
//# sourceMappingURL=web-retrieval-verification.js.map