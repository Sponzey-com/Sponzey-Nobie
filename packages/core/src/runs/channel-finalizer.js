...
export function buildNamedResultDeliveryEvent(input) {
    ...
    if (!validation.ok) {
        throw new Error(`invalid named delivery event: ${validation.issues.map((issue) => issue.path).join(",")}`);
    }
    return event;
}
export function recordApprovalAggregation(input) {
    ...
    const eventId = recordMessageLedgerEvent({
        ...
        status: blockedApprovalIds.length > 0
            ? "failed"
...
export function listPendingFinalizers(input = {}) {
    ...
    return events
        .filter((event) => event.event_kind === "final_answer_generated" &&
        event.delivery_key &&
...
//# sourceMappingURL=channel-finalizer.js.map