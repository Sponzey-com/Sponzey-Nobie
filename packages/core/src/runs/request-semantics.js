export function shouldReuseConversationContext(message) {
...
export function detectActiveQueueCancellationMode(message) {
    const trimmed = message.trim();
    if (!trimmed)
        return null;
    if (/(일정|예약|알림|스케줄|schedule|reminder|notification|alarm)/iu.test(trimmed))
        return null;
    if (!/(취소|중단|멈춰|그만|cancel|abort|stop)/iu.test(trimmed))
        return null;
...
export function extractVerificationSourceRequest(value) {
    const normalized = value.trim();
    const marker = "원래 사용자 요청:";
    if (normalized.startsWith("[Task Intake Bridge]")) {
        const line = normalized.split("\n").find((item) => item.startsWith(marker));
        if (line) {
            return line.slice(marker.length).trim() || value;
        }
    }
    return value;
}
...
//# sourceMappingURL=request-semantics.js.map