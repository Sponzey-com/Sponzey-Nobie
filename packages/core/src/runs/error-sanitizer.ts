...
function normalizeRawError(message: string | undefined): string {
...
function hasEncodingDamage(message: string): boolean {
  return /�{1,}|����|���|\uFFFD/i.test(message)
}

function isDeliveryFailure(message: string): boolean {
...
function isToolFailure(message: string): boolean {
...
export function actionHintForSanitizedErrorKind(kind: SanitizedErrorKind): string {
  switch (kind) {
    case "auth":
...
function withActionHint(summary: SanitizedErrorSummary): SanitizedErrorSummary {
  return {
    ...summary,
...
function sanitizeUserFacingErrorCore(message: string | undefined): SanitizedErrorSummary {
...
export function sanitizeUserFacingError(message: string | undefined): SanitizedErrorSummary {
