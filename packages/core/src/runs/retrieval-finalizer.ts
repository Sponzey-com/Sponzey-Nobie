import { getMessageLedgerEventByIdempotencyKey, listMessageLedgerEvents, type DbMessageLedgerEvent } from "../db/index.js"
import {
  hashLedgerValue,
...
export type RetrievalCompletionStatus =
  | "completed_value_found"
  | "completed_approximate_value_found"
  | "completed_limited_no_value"
  | "completed_insufficient_evidence"
  | "completed_policy_blocked"
  | "awaiting_user"
...
function resolveOwner(input: { requestGroupId?: string | null; runId?: string | null }): string {
  return normalizeText(input.requestGroupId) || normalizeText(input.runId) || "unknown-run"
}

function resolveTarget(input: { threadKey?: string | null; sessionKey?: string | null }): string {
...
function finalAnswerDelivered(event: DbMessageLedgerEvent): boolean {
  return event.event_kind === "final_answer_delivered" && eventSucceeded(event)
}

function containsNumberLike(value: string): boolean {
...
export function buildProgressMessageIdempotencyKey(input: RecordProgressMessageInput): string {
...
export function canGenerateFinalAnswerFromVerdict(input: {
  ...
  if (!input.verdict) return { allowed: false, reason: "retrieval_verdict_required" }
  if (input.verdict.canAnswer) return { allowed: true }
  if (containsNumberLike(input.text)) return { allowed: false, reason: "unverified_numeric_final_answer" }
  return { allowed: true }
}

function recordLedger(input: MessageLedgerEventInput): string | null {
  return recordMessageLedgerEvent(input)
}

export function recordProgressMessageSent(input: RecordProgressMessageInput): string | null {
...
export function recordFinalAnswerDelivery(input: RecordFinalAnswerDeliveryInput): FinalAnswerDeliveryReceipt {
  const text = normalizeText(input.text)
...
function completionStatusForVerdict(verdict: RetrievalVerificationVerdict): RetrievalCompletionStatus {
  ...
  if (verdict.evidenceSufficiency === "insufficient_candidate_missing") return "completed_limited_no_value"
  if (verdict.evidenceSufficiency === "blocked") return "completed_policy_blocked"
