...
const ACTIVE_STATUSES = ["queued", "running", "awaiting_approval", "awaiting_user"];
...
function classifyIssue(value) {
    ...
    if (/(vector|embedding|fts|stale_embedding|dimension|벡터|임베딩)/i.test(value))
        return { kind: "vector", key: "vector" };
    if (/(memory|메모리)/i.test(value))
        return { kind: "memory", key: "memory" };
    if (/(schedule|scheduler|cron|예약)/i.test(value))
        return { kind: "schedule", key: "schedule" };
...
function pushIssue(issues, value, at) {
    if (!value?.trim())
        return;
    if (!isIssueText(value))
        return;
...
function collectIssues(runs, tasks) {
    const issues = [];
    for (const run of runs) {
        if (run.status === "failed" || run.status === "interrupted")
            pushIssue(issues, run.summary, run.updatedAt);
        for (const event of run.recentEvents)
            pushIssue(issues, event.label, event.at);
    }
    for (const task of tasks) {
        pushIssue(issues, task.failure?.summary, task.updatedAt);
        if (task.delivery.status === "failed")
            pushIssue(issues, task.delivery.summary ?? "delivery failed", task.updatedAt);
        for (const event of task.diagnostics?.memoryEvents ?? [])
            pushIssue(issues, event, task.updatedAt);
        for (const event of task.diagnostics?.toolEvents ?? [])
            pushIssue(issues, event, task.updatedAt);
        for (const event of task.diagnostics?.deliveryEvents ?? [])
            pushIssue(issues, event, task.updatedAt);
        for (const event of task.diagnostics?.recoveryEvents ?? [])
            pushIssue(issues, event, task.updatedAt);
        for (const event of task.diagnostics?.latencyEvents ?? [])
            pushIssue(issues, event, task.updatedAt);
    }
    return issues;
}
function aggregateRepeatedIssues(issues) {
...
function healthItem(key, label, issues, match) {
...
function buildStaleCandidate(run, kind, now) {
    return {
        runId: run.id,
...
function buildStaleSummary(runs, now, thresholdMs) {
