let lastStartupRecoverySummary = buildStartupRecoverySummary({ runs: [], schedules: [] });
...
export function classifyStartupRecovery(run, continuity) {
...
export function summarizeInterruptedScheduleRun(row) {
    return {
...
export function buildStartupRecoverySummary(input) {
    ...
    const count = (status) => runs.filter((run) => run.recoveryStatus === status).length;
...
export function setLastStartupRecoverySummary(summary) {
    lastStartupRecoverySummary = summary;
}
export function getLastStartupRecoverySummary() {
