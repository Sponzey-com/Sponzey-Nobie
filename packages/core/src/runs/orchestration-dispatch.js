...
function taskProfileForScope(scope) {
    const haystack = [
        scope.goal,
        scope.intentType,
        scope.actionType,
    ...
    if (/(review|검토|리뷰)/.test(haystack))
        return "review";
    if (/(research|retrieve|search|조사|검색|자료)/.test(haystack))
        return "research";
    if (/(plan|planning|계획|설계)/.test(haystack))
        return "planning";
    if (/(operate|deploy|release|운영|배포)/.test(haystack))
        return "operations";
    return "general_chat";
}
...
function reportFor(input) {
    const outputStatus = input.status === "completed"
        ? "satisfied"
...
function resultSummary(resultReport) {
...
export function validateDispatchToChildExecutorInput(input) {
    ...
    const validatedByDecision = reasonCodes.has("execution_decision_selected_executor") ||
...
function orchestrationTaskFromTeamTask(task, parentTask) {
    return {
        taskId: task.taskId,
...
export async function dispatchDelegatedSubAgentTasks(params, dependencies) {
    ...
    const dispatchAgentTask = async (task, agent) => {
        ...
        const outcome = await runner.runSubSession({
            ...
            appendParentEvent(params.parentRunId, `sub_agent_dispatch_pending_result:${task.taskId}:${agent.agentId}:${started.runId}`);
            ...
            const reportStatus = childRun && !failedStatuses.has(childRun.status) ? "completed" : "failed";
            await controls.emitProgress(reportStatus === "completed"
                ? "서브 에이전트 실행을 완료했습니다."
                : "서브 에이전트 실행이 실패했습니다.", "running");
            return reportFor({
        ...
        if (outcomeRecord.reasonCode === "prompt_bundle_preflight_failed") {
            appendParentEvent(params.parentRunId, `sub_agent_dispatch_preflight_failed:${task.taskId}:${agent.agentId}:${outcome.subSession.subSessionId}`);
        }
        if (outcome.status === "cancelled") {
            appendParentEvent(params.parentRunId, `sub_agent_dispatch_cancelled:${task.taskId}:${agent.agentId}:${outcome.subSession.subSessionId}`);
        }
        appendParentEvent(params.parentRunId, [
    ...
    const dispatchTeamTask = async (task, teamId) => {
        ...
        const teamPlan = buildTeamExecutionPlan({
            ...
            ...(dependencies.idProvider
                ? { idProvider: (prefix) => `${prefix}:${dependencies.idProvider?.()}` }
                : {}),
        });
        ...
        appendParentEvent(params.parentRunId, `team_execution_planned:${teamPlan.plan.teamExecutionPlanId}:${teamId}:assignments=${teamPlan.plan.memberTaskAssignments.length}`);
        const expandedTasks = teamPlan.plan.memberTaskAssignments
            ...
            .sort((left, right) => teamTaskOrder(left.taskKind) - teamTaskOrder(right.taskKind) ||
    ...
    appendParentEvent(params.parentRunId, `sub_agent_dispatch_finished:attempted=${attempted};completed=${completed};failed=${failed};skipped=${skipped}`);
    return { attempted, completed, failed, skipped, outcomes };
}
//# sourceMappingURL=orchestration-dispatch.js.map