...
interface ScenarioFixture extends SubAgentBenchmarkScenarioDefinition {
  metrics: Omit<
    SubAgentBenchmarkScenarioMetrics,
    "parallelEfficiency" | "promptCacheHitRate" | "tokenTotal"
  >
  recommendation?: Omit<CompiledWorkflowRecommendation, "recommendationId" | "autoApply">
}
...
const SCENARIO_FIXTURES: ScenarioFixture[] = [
  {
    ...
    description:
      "Measures spawn ack, first progress, and wall-clock reduction for three independent research branches.",
  ...
  },
  {
    ...
    description:
      "Measures hot registry snapshot reuse, planner hot path, prompt cache hit rate, and token cost for codebase exploration.",
  ...
  },
  {
    ...
    description:
      "Measures review-loop LLM calls, duplicate final suppression, and parent final-answer ownership.",
  ...
  },
  {
    ...
    description:
      "Measures expansion from a team target to owner direct-child member tasks without executing the team as a permission owner.",
  ...
  },
  {
    ...
    description:
      "Measures restart-resume projection and success rate for a long-running sub-session after process recovery.",
  ...
  },
  {
    ...
    description:
      "Measures denied capability handling without auto-escalation or final-answer duplication.",
  ...
  },
  {
    ...
    description:
      "Measures focused follow-up routing while preserving final-answer ownership and memory isolation.",
  ...
  },
  {
    ...
    description:
      "Measures repeated-task detection and emits a non-automatic compiled workflow recommendation skeleton.",
...
const benchmarkRuns = new Map<string, SubAgentBenchmarkSuiteResult>()
let latestBenchmarkRunId: string | null = null
...
function statusFromIssues(warnings: string[], blockingFailures: string[]): SubAgentBenchmarkStatus {
...
function buildMetrics(
  ...
  return {
    ...
    promptCacheHitRate:
      promptEvents === 0 ? 0 : round(fixture.metrics.promptCacheHits / promptEvents),
    tokenTotal: fixture.metrics.inputTokens + fixture.metrics.outputTokens,
    ...overrides,
  }
}

function evaluateScenario(metrics: SubAgentBenchmarkScenarioMetrics): {
  warnings: string[]
  blockingFailures: string[]
} {
  ...
  if (!metrics.memoryIsolationMaintained) blockingFailures.push("memory_isolation_broken")
  if (!metrics.permissionDeniedHandled) blockingFailures.push("permission_denied_boundary_broken")
  return { warnings, blockingFailures }
}

function scenarioResult(input: {
  fixture: ScenarioFixture
  suiteStartedAtMs: number
  offsetMs: number
  seed: string
...
function aggregateScenarios(
  ...
  const cacheMisses = scenarios.reduce(
    (sum, scenario) => sum + scenario.metrics.promptCacheMisses,
    0,
  )
  ...
  return {
    ...
    totalSequentialWallClockMs: scenarios.reduce(
      (sum, scenario) => sum + scenario.metrics.sequentialWallClockMs,
      0,
    ),
    averageParallelEfficiency:
      scenarios.length === 0
        ? 0
        : round(
            scenarios.reduce((sum, scenario) => sum + scenario.metrics.parallelEfficiency, 0) /
              scenarios.length,
          ),
    totalLlmCallCount: scenarios.reduce((sum, scenario) => sum + scenario.metrics.llmCallCount, 0),
    promptCacheHitRate:
      cacheHits + cacheMisses === 0 ? 0 : round(cacheHits / (cacheHits + cacheMisses)),
    ...
    totalCostEstimateUsd: round(
      scenarios.reduce((sum, scenario) => sum + scenario.metrics.costEstimateUsd, 0),
      COST_PRECISION,
    ),
    duplicateFinalAnswerCount: scenarios.reduce(
      (sum, scenario) => sum + scenario.metrics.duplicateFinalAnswerCount,
      0,
    ),
    restartRecoverySuccessRate:
      restartScenarios.length === 0
        ? null
        : round(
            restartScenarios.filter((scenario) => scenario.metrics.restartRecoverySucceeded)
              .length / restartScenarios.length,
          ),
    userInterventionCount: scenarios.reduce(
      (sum, scenario) => sum + scenario.metrics.userInterventionCount,
      0,
    ),
    hotRegistrySnapshotP95Ms: percentile95(
      scenarios.map((scenario) => scenario.metrics.hotRegistrySnapshotP95Ms),
    ),
    plannerHotPathP95Ms: percentile95(
      scenarios.map((scenario) => scenario.metrics.plannerHotPathP95Ms),
    ),
...
export function listSubAgentBenchmarkScenarios(): SubAgentBenchmarkScenarioDefinition[] {
  return SCENARIO_FIXTURES.map(
    ({ metrics: _metrics, recommendation: _recommendation, ...definition }) => ({
      ...definition,
    }),
  )
}

export function evaluateSubAgentBenchmarkReleaseGate(
  suite: Pick<SubAgentBenchmarkSuiteResult, "generatedAt" | "scenarios" | "aggregate">,
  ...
  const warnings = suite.scenarios.flatMap((scenario) =>
    scenario.warnings.map((warning) => `${scenario.scenarioId}: ${warning}`),
  )
  const blockingFailures = [
    ...missingScenarioIds.map((id) => `${id}: required benchmark scenario missing`),
    ...suite.scenarios.flatMap((scenario) =>
      scenario.blockingFailures.map((failure) => `${scenario.scenarioId}: ${failure}`),
    ),
  ]
  if (
  ...
  if (suite.aggregate.duplicateFinalAnswerCount > 0) {
    blockingFailures.push(
      `duplicate_final_answer_count:${suite.aggregate.duplicateFinalAnswerCount}`,
    )
  }
  if (
    suite.aggregate.restartRecoverySuccessRate != null &&
    suite.aggregate.restartRecoverySuccessRate < 1
  ) {
    blockingFailures.push(
      `restart_recovery_success_rate:${suite.aggregate.restartRecoverySuccessRate}`,
    )
  }
  if (
    suite.scenarios.some(
      (scenario) => scenario.recommendation && scenario.recommendation.autoApply !== false,
    )
  ) {
...
function recordScenarioLatencyMetrics(result: SubAgentBenchmarkScenarioResult): void {
...
export function runSubAgentBenchmarkSuite(
  input: RunSubAgentBenchmarkSuiteInput = {},
  ...
  const fixtures = scenarioIds.map((id) => {
    ...
    if (!fixture) throw new Error(`Unknown benchmark scenario: ${id}`)
    return fixture
  })
  let offsetMs = 0
  const scenarios = fixtures.map((fixture) => {
    ...
    if (input.recordLatencyMetrics) recordScenarioLatencyMetrics(result)
    return result
  })
...
export function buildSubAgentBenchmarkReleaseGateSummary(
  input: { now?: Date; result?: SubAgentBenchmarkSuiteResult } = {},
): SubAgentBenchmarkReleaseGateSummary {
  return (input.result ?? runSubAgentBenchmarkSuite(input.now ? { now: input.now } : {}))
    .releaseGate
}

export function runAndStoreSubAgentBenchmarkSuite(
  input: RunSubAgentBenchmarkSuiteInput = {},
): SubAgentBenchmarkSuiteResult {
...
export function getSubAgentBenchmarkRun(
  benchmarkRunId: string,
): SubAgentBenchmarkSuiteResult | undefined {
  return benchmarkRuns.get(benchmarkRunId)
}

export function getLatestSubAgentBenchmarkRun(): SubAgentBenchmarkSuiteResult | undefined {
  return latestBenchmarkRunId ? benchmarkRuns.get(latestBenchmarkRunId) : undefined
}

export function resetSubAgentBenchmarkRunsForTest(): void {
  benchmarkRuns.clear()
