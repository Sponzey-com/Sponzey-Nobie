...
export const MIGRATIONS: Migration[] = [
  ...
  },
  {
    version: 23,
    up(db) {
      const scheduleRunsTable = db.prepare<[], { name: string }>(
        "SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'schedule_runs'",
      ).get()
      if (scheduleRunsTable) {
        const scheduleRunColumns = db.prepare(`PRAGMA table_info(schedule_runs)`).all() as Array<{ name: string }>
        const hasRunColumn = (name: string) => scheduleRunColumns.some((column) => column.name === name)
...
function schemaMigrationsTableExists(db: Database.Database): boolean {
  return Boolean(db.prepare<[], { name: string }>(
...
export function getAppliedMigrationVersions(db: Database.Database): number[] {
  ...
  return db.prepare<[], { version: number }>(
