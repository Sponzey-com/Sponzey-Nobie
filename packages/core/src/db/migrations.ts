...
const MEMORY_SCOPE_SQL =
  "'global', 'session', 'task', 'artifact', 'diagnostic', 'long-term', 'short-term', 'schedule', 'flash-feedback'"

function rebuildMemoryScopeTables(db: Database.Database): void {
  db.exec(`
...
export const MIGRATIONS: Migration[] = [
  ...
  },
  {
    ...
    up(db) {
      ...
      db.exec(
        `CREATE INDEX IF NOT EXISTS idx_messages_root_run ON messages(root_run_id, created_at)`,
      )
    },
  },
  {
    version: 6,
    up(db) {
      const rootRunColumns = db.prepare(`PRAGMA table_info(root_runs)`).all() as Array<{
        name: string
      }>
      ...
      db.exec(
        `UPDATE root_runs SET request_group_id = id WHERE request_group_id IS NULL OR request_group_id = ''`,
      )
      db.exec(
        `CREATE INDEX IF NOT EXISTS idx_root_runs_request_group ON root_runs(request_group_id, updated_at DESC)`,
      )
    },
  },
  {
    ...
    up(db) {
      ...
      db.exec(
        `UPDATE root_runs SET context_mode = 'full' WHERE context_mode IS NULL OR context_mode = ''`,
      )
      db.exec(
        `CREATE INDEX IF NOT EXISTS idx_root_runs_worker_session ON root_runs(worker_session_id, updated_at DESC)`,
      )
    },
  },
  {
  ...
  },
  {
    ...
    up(db) {
      ...
      db.exec(
        `UPDATE schedules SET execution_driver = 'internal' WHERE execution_driver IS NULL OR execution_driver = ''`,
      )
    },
  },
  {
  ...
  },
  {
    ...
    up(db) {
      ...
      db.exec(
        `UPDATE root_runs SET lineage_root_run_id = request_group_id WHERE lineage_root_run_id IS NULL OR lineage_root_run_id = ''`,
      )
      db.exec(`UPDATE root_runs SET run_scope = 'root' WHERE run_scope IS NULL OR run_scope = ''`)
      db.exec(
        `CREATE INDEX IF NOT EXISTS idx_root_runs_lineage_root ON root_runs(lineage_root_run_id, updated_at DESC)`,
      )

      const memoryColumns = db.prepare(`PRAGMA table_info(memory_items)`).all() as Array<{
        name: string
      }>
      ...
      `)
      db.exec(
        `CREATE INDEX IF NOT EXISTS idx_memory_items_scope_updated ON memory_items(memory_scope, updated_at DESC)`,
      )
      db.exec(
        `CREATE INDEX IF NOT EXISTS idx_memory_items_run_scope ON memory_items(run_id, memory_scope, updated_at DESC)`,
      )
    },
  },
  {
  ...
  },
  {
    version: 17,
    up(db) {
      const continuityColumns = db.prepare(`PRAGMA table_info(task_continuity)`).all() as Array<{
        name: string
      }>
  ...
  },
  {
    version: 21,
    up(db) {
      const auditColumns = db.prepare(`PRAGMA table_info(audit_logs)`).all() as Array<{
        name: string
      }>
  ...
  },
  {
    ...
    up(db) {
      ...
      const hasColumn = (name: string) => scheduleColumns.some((column) => column.name === name)
  ...
  },
  {
    ...
    up(db) {
      ...
      if (scheduleRunsTable) {
        const scheduleRunColumns = db.prepare(`PRAGMA table_info(schedule_runs)`).all() as Array<{
          name: string
        }>
        const hasRunColumn = (name: string) =>
          scheduleRunColumns.some((column) => column.name === name)
  ...
  },
  {
    ...
    up(db) {
      ...
      db.exec(
        `CREATE INDEX IF NOT EXISTS idx_root_runs_runtime_manifest ON root_runs(runtime_manifest_id, updated_at DESC)`,
      )
    },
  },
  {
  ...
  },
  {
    ...
    up(db) {
      ...
      const needsMembershipRebuild =
        !membershipColumns.has("membership_id") ||
      ...
      if (needsMembershipRebuild) {
        ...
        const legacyMemberships = db
          ...
          >(
            `SELECT team_id, agent_id, status, role_hint, schema_version, audit_id, created_at, updated_at
        ...
        for (const row of legacyMemberships) {
          ...
          insertMembership.run(
            `${row.team_id}:membership:${sortOrder + 1}`,
            row.team_id,
            row.agent_id,
            null,
            JSON.stringify([primaryRole]),
            primaryRole,
            nextStatus === "removed" ? 0 : 1,
            null,
            nextStatus,
            row.role_hint,
            sortOrder,
      ...
      const deleteNicknameNamespaceByEntity = db.prepare<[string, string]>(
        "DELETE FROM nickname_namespaces WHERE entity_type = ? AND entity_id = ?",
      )
      const existingNicknameNamespace = db.prepare<
        [string],
        {
      ...
      >(
        "SELECT normalized_nickname, entity_type, entity_id FROM nickname_namespaces WHERE normalized_nickname = ?",
      )
      const insertNicknameNamespace = db.prepare(
      ...
      const agentRows = db
        ...
        >(
          `SELECT agent_id, nickname, status, source, config_json, created_at, updated_at
         FROM agent_configs
         ORDER BY updated_at DESC, agent_id ASC`,
        )
        .all()
      const updateAgentConfig = db.prepare<[string | null, string | null, string | null, string]>(
      ...
      for (const row of agentRows) {
        ...
        if (existing && (existing.entity_type !== "agent" || existing.entity_id !== row.agent_id))
          continue
        insertNicknameNamespace.run(
          normalizedNickname,
          "agent",
          row.agent_id,
          row.nickname ?? normalizedNickname,
          row.status,
          row.source,
          row.created_at,
          row.updated_at,
      ...
      const teamRows = db
        ...
        >(
          `SELECT team_id, nickname, status, source, config_json, role_hints_json, member_agent_ids_json, created_at, updated_at
         FROM team_configs
         ORDER BY updated_at DESC, team_id ASC`,
        )
        .all()
      const updateTeamConfig = db.prepare<
        [
          string | null,
          string | null,
          string | null,
      ...
      for (const row of teamRows) {
        ...
        const requiredTeamRoles = Array.from(
          new Set([...asStringArray(config?.["requiredTeamRoles"]), ...fallbackRoleHints]),
        )
        const requiredCount = Array.isArray(config?.["memberships"])
          ? (config["memberships"] as unknown[]).filter((membership) => {
        ...
        const normalizedNickname = row.nickname ? normalizeNickname(row.nickname) : null
        updateTeamConfig.run(
          normalizedNickname,
          ownerAgentId,
          leadAgentId,
          memberCountMin,
          memberCountMax,
          JSON.stringify(requiredTeamRoles),
          JSON.stringify(asStringArray(config?.["requiredCapabilityTags"])),
  ...
  },
  {
    version: 38,
    up(db) {
      if (!tableExists(db, "capability_delegations")) return
      db.exec(`
        PRAGMA foreign_keys = OFF;
        PRAGMA defer_foreign_keys = ON;
...
export function getAppliedMigrationVersions(db: Database.Database): number[] {
  if (!schemaMigrationsTableExists(db)) return []
  return db
...
export function getPendingMigrationVersions(db: Database.Database): number[] {
...
export function createPreMigrationBackupIfNeeded(
  ...
  const backupPath = join(
    backupDir,
    `pre-migration-v${pendingVersions[0]}-to-v${latestPending}-${Date.now()}.sqlite3`,
  )
...
export function runMigrations(db: Database.Database, options: RunMigrationsOptions = {}): void {
  ...
  try {
    ...
    if (!verifyReport.ok) {
      throw new Error(
        `Migration verify failed: missing tables=${verifyReport.missingTables.join(",")}; missing indexes=${verifyReport.missingIndexes.join(",")}; integrity=${verifyReport.integrityCheck}`,
      )
    }
