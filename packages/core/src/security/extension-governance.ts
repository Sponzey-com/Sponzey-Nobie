...
function safeFileChecksum(filePath: string | null | undefined): string | null {
...
function riskToPermissionScope(riskLevel: RiskLevel): ExtensionPermissionScope {
  if (riskLevel === "dangerous") return "dangerous"
  if (riskLevel === "moderate") return "moderate"
  return "safe"
}

function permissionNeedsApproval(scope: ExtensionPermissionScope, trustLevel: ExtensionTrustLevel): boolean {
  return scope === "dangerous" || (trustLevel === "external" && scope !== "safe")
}

function trustPolicyFor(input: { trustLevel: ExtensionTrustLevel; permissionScope: ExtensionPermissionScope; approved?: boolean; externalRecommended?: boolean }): ExtensionTrustPolicy {
...
function statusFor(enabled: boolean, ready: boolean | null, error: string | null, extensionId: string): ExtensionStatus {
  ...
  if (failure?.degraded) return "degraded"
...
function applyFailureState(entry: ExtensionRegistryEntry): ExtensionRegistryEntry {
...
function makeEntry(input: Omit<ExtensionRegistryEntry, "checksum" | "trustPolicy" | "failureCount" | "degradedReason" | "rollbackAvailable"> & {
...
function mcpServerEntry(name: string, serverConfig: McpServerConfig, status: MinimalMcpServerStatus | undefined): ExtensionRegistryEntry {
  const enabled = serverConfig.enabled !== false
  const transport = serverConfig.transport ?? (serverConfig.url ? "http" : "stdio")
  const permissionScope: ExtensionPermissionScope = transport === "http" || serverConfig.url ? "network" : "local"
...
function mcpToolEntry(serverName: string, tool: MinimalMcpToolStatus, serverConfig: McpServerConfig): ExtensionRegistryEntry {
  const timeoutMs = Math.max(1, serverConfig.toolTimeoutSec ?? 30) * 1000
  return makeEntry({
...
function toolEntry(tool: AnyTool): ExtensionRegistryEntry {
  const kind: ExtensionKind = tool.name.startsWith("mcp__")
    ? "mcp_tool"
...
function skillEntry(skill: SkillConfigItem): ExtensionRegistryEntry {
  const sourcePath = skill.path ?? null
  const fileChecksum = safeFileChecksum(sourcePath)
  const trustLevel: ExtensionTrustLevel = skill.source === "builtin" ? "builtin" : "local"
  return makeEntry({
    id: `skill:${skill.id}`,
    kind: "skill",
...
export function buildExtensionRegistrySnapshot(input: {
  ...
  for (const [name, serverConfig] of Object.entries(config.mcp?.servers ?? {})) {
    ...
    for (const tool of status?.tools ?? []) entries.set(`mcp-tool:${tool.registeredName}`, mcpToolEntry(name, tool, serverConfig))
  }

  for (const skill of config.skills?.items ?? []) entries.set(`skill:${skill.id}`, skillEntry(skill))
  ...
  for (const state of failureStates.values()) {
    if (entries.has(state.extensionId)) continue
    const kind: ExtensionKind = state.extensionId.startsWith("mcp:")
      ? "mcp_server"
...
export function extensionIdsForToolName(toolName: string): string[] {
  const ids = [`tool:${toolName}`, `mcp-tool:${toolName}`]
  const mcpMatch = toolName.match(/^mcp__(.+?)__/)
  if (mcpMatch?.[1]) ids.push(`mcp:${mcpMatch[1]}`)
  if (toolName.startsWith("yeonjang_")) ids.push(`yeonjang-tool:${toolName}`)
  return ids
}

export function isToolExtensionSelectable(toolName: string): boolean {
  return extensionIdsForToolName(toolName).every((id) => !failureStates.get(id)?.degraded)
}

export function getExtensionFailureState(extensionId: string): ExtensionFailureState | null {
  const state = failureStates.get(extensionId)
  return state ? { ...state } : null
}

export function listExtensionFailureStates(): ExtensionFailureState[] {
  return [...failureStates.values()].map((state) => ({ ...state }))
}

export function resetExtensionFailureState(extensionId?: string): void {
  if (extensionId) failureStates.delete(extensionId)
...
export function recordExtensionFailure(input: {
  ...
  const state: ExtensionFailureState = {
...
export async function runExtensionHookSafely<T>(input: {
  extensionId: string
  hookName: string
  timeoutMs?: number
  runId?: string | null
  requestGroupId?: string | null
}, hook: () => Promise<T> | T): Promise<{ ok: true; result: T } | { ok: false; error: string; state: ExtensionFailureState }> {
...
export function activateExtensionWithTrustPolicy(entry: ExtensionRegistryEntry, input: { approved?: boolean } = {}): ExtensionActivationResult {
