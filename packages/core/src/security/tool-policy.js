...
export function evaluateAndRecordToolPolicy(input) {
...
export function recordToolPolicyDecision(record) {
    getDb().prepare(`INSERT INTO tool_policy_decisions
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(record.id, record.runId ?? null, record.requestGroupId ?? null, record.sessionId ?? null, record.channel ?? null, record.toolName, record.riskLevel, record.sourceTrust, record.approvalId ?? null, record.permissionScope, record.paramsHash, record.decision, record.reasonCode, record.userMessage ?? null, record.diagnostic ? JSON.stringify(record.diagnostic) : null, record.createdAt);
}
export function sanitizePolicyDenialForUser(record) {
