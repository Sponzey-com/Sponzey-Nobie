...
export type CapabilityApprovalDenialReason =
  | "permission_denied"
...
export function normalizeSkillMcpAllowlist(
  input: Partial<SkillMcpAllowlist> | null | undefined,
): SkillMcpAllowlist {
  const secretScopeId =
    typeof input?.secretScopeId === "string" ? nonEmpty(input.secretScopeId) : undefined
  return {
...
export function resolveToolCapabilityRisk(
  toolName: string,
  fallback: RiskLevel | CapabilityRiskLevel = "safe",
): CapabilityRiskLevel {
...
export function classifyDepthScopedToolKind(toolName: string): DepthScopedToolKind {
...
function normalizedDepth(value: number | undefined): number | undefined {
...
function namesMatchAny(names: Set<string>, values: string[]): boolean {
...
function resolveAgentBindingDecision(input: {
  ...
  | { ok: true; bindingId?: string; diagnostic?: Record<string, unknown> }
  | {
      ok: false
      reasonCode: string
      userMessage: string
  ...
  if (input.mcpTool) {
    ...
    if (
      binding.capability_kind !== "mcp_server" ||
...
function fallbackAllows(input: {
  ...
  const names = new Set<string>([normalizeToken(input.toolName)])
  if (input.bindingId) names.add(normalizeToken(input.bindingId))
  if (input.mcpTool) {
...
export function evaluateAgentToolCapabilityPolicy(input: {
  ...
  const requiresAgentContext = Boolean(
    mcpTool || input.ctx.agentId || policy.permissionProfile || policy.skillMcpAllowlist,
  )
  ...
  const allowlist = policy.skillMcpAllowlist
    ...
    : EMPTY_ALLOWLIST
  const parentSecretFallbackRequested =
    Boolean(input.ctx.parentSecretScopeId) && !input.ctx.secretScopeId && !allowlist.secretScopeId
  ...
  const secretScopeId =
    input.ctx.secretScopeId ?? allowlist.secretScopeId ?? input.ctx.parentSecretScopeId
  ...
  const capabilityBlockReason = resolvePermissionProfileBlockReason(
    input.toolName,
    capabilityRisk,
  ...
  const approvalRequired = isRiskAtLeast(
    capabilityRisk,
    policy.permissionProfile.approvalRequiredFrom,
  )
  const rateLimitKey = [
    "agent",
    input.ctx.agentId,
...
export function toAgentCapabilityCallContext(ctx: ToolContext): AgentCapabilityCallContext | null {
  ...
  const secretScopeId =
    ctx.secretScopeId ?? skillMcpAllowlist?.secretScopeId ?? ctx.parentSecretScopeId
...
export function acquireAgentCapabilityRateLimit(input: {
  ...
  if (state.concurrent >= maxConcurrent) {
    throw new Error(
      `agent capability rate limit exceeded: concurrent=${state.concurrent}, max=${maxConcurrent}`,
    )
  }
  if (input.decision.rateLimit.maxCallsPerMinute !== undefined) {
    ...
    if (state.calls.length >= maxCalls) {
      throw new Error(
        `agent capability rate limit exceeded: calls_per_minute=${state.calls.length}, max=${maxCalls}`,
      )
    }
  }
...
function parseDelegationContract(
  row: ReturnType<typeof getCapabilityDelegation>,
): CapabilityDelegationRequest | undefined {
  if (!row) return undefined
  try {
    const parsed = JSON.parse(row.contract_json) as unknown
    return parsed && typeof parsed === "object"
      ? (parsed as CapabilityDelegationRequest)
      : undefined
  } catch {
    return undefined
  }
...
function parseSubSessionContract(
  row: ReturnType<typeof getRunSubSession>,
...
function recordCapabilityDelegationAudit(input: {
  ...
  try {
    insertAuditLog({
      ...
      result:
        input.status === "denied" || input.status === "expired" || input.status === "failed"
          ? "failed"
          : "success",
      duration_ms: 0,
      ...
      retry_count: 0,
      stop_reason:
        input.status === "denied" || input.status === "expired" ? input.reasonCode : null,
    })
  } catch {
...
const CAPABILITY_DELEGATION_TRANSITIONS: Record<
  CapabilityDelegationRequest["status"],
  CapabilityDelegationRequest["status"][]
> = {
...
export function updateCapabilityDelegationLifecycle(input: {
  ...
  const next: CapabilityDelegationRequest = {
    ...delegation,
    status: input.status,
...
export function applyCapabilityDelegationApprovalDecision(input: {
  ...
  const status: CapabilityDelegationRequest["status"] =
  ...
  if (runId) {
    ...
    if (status === "approved") {
      ...
      updateRunStatus(
        runId,
        "running",
        "Capability approval accepted; continuing sub-session.",
        true,
      )
    } else {
      setRunStepStatus(
        ...
        "awaiting_approval",
        "cancelled",
        `Capability approval stopped: ${reasonCode}`,
      )
  ...
  if (input.subSessionId) {
    ...
    if (subSession) {
      const nextStatus: SubSessionContract["status"] =
...
export function mapDangerousFixtureRiskLevel(
  riskLevel: DangerousCapabilityFixtureRiskLevel,
): CapabilityRiskLevel {
  switch (riskLevel) {
    case "low":
...
export function buildDangerousCapabilityFixtureMatrix(): DangerousCapabilityApprovalFixture[] {
  const rows: Array<{
    riskLevel: DangerousCapabilityFixtureRiskLevel
    approvalActor: CapabilityApprovalActor
    approvalTimeoutMs: number
