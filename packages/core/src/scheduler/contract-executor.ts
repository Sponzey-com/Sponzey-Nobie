...
export type ScheduleContractExecutionResult =
  | { handled: false }
  | { handled: true; result: ScheduledExecutionResult }
...
function logInfo(dependencies: ScheduleContractExecutorDependencies | undefined, message: string, payload?: Record<string, unknown>): void {
  dependencies?.logInfo?.(message, payload)
}

function logWarn(dependencies: ScheduleContractExecutorDependencies | undefined, message: string): void {
  dependencies?.logWarn?.(message)
}

function logError(dependencies: ScheduleContractExecutorDependencies | undefined, message: string, payload?: Record<string, unknown>): void {
  dependencies?.logError?.(message, payload)
}

function normalizeDeliveryChannel(channel: string | null | undefined): DeliveryChannel {
  switch (channel) {
    case "telegram":
    case "slack":
    case "webui":
...
function resolveEffectiveDelivery(contract: ScheduleContract, schedule: DbSchedule): DeliveryContract {
...
export function resolveScheduleDueAt(params: {
  ...
  if (match?.[1]) {
    ...
    if (!Number.isNaN(dueAt.getTime())) return dueAt.toISOString()
  }
...
async function defaultTelegramTextDelivery(sessionId: string, text: string): Promise<unknown> {
  const telegram = getActiveTelegramChannel()
  if (!telegram) throw new Error("telegram channel is not running")
  return telegram.sendTextToSession(sessionId, text)
}

function resolveSlackTarget(sessionId: string): { channelId: string; threadTs: string } {
  ...
  if (!match) throw new Error(`Slack session ${sessionId} has invalid source_id`)
  const channelId = match[1]
  const threadTs = match[2]
  if (!channelId || !threadTs) throw new Error(`Slack session ${sessionId} has invalid source_id`)
  return { channelId, threadTs }
}

async function defaultSlackTextDelivery(sessionId: string, text: string): Promise<unknown> {
...
async function defaultSlackFileDelivery(sessionId: string, filePath: string, caption?: string): Promise<unknown> {
  const config = getConfig()
  if (!config.slack?.enabled || !config.slack.botToken) {
    throw new Error("slack channel is not configured")
  }
...
async function deliverText(params: {
  ...
  plan: DeliveryPlan
  text: string
  dependencies?: ScheduleContractExecutorDependencies | undefined
  ...
  try {
    switch (params.plan.delivery.channel) {
      case "telegram": {
        ...
        if (!sessionId) throw new Error("telegram target session is not configured for this schedule")
      ...
      case "slack": {
        const sessionId = params.plan.delivery.sessionId
        if (!sessionId) throw new Error("slack target session is not configured for this schedule")
  ...
  try {
    switch (params.plan.delivery.channel) {
      ...
      case "telegram": {
        ...
        const result = await (params.dependencies?.dispatchTool ?? ((name, toolParams, ctx) => toolDispatcher.dispatch(name, toolParams, ctx)))(
          "telegram_send_file",
          { filePath: artifact.artifact_path, caption: params.contract.summary ?? params.contract.displayName ?? undefined },
          ctx,
        )
...
export async function executeScheduleContract(input: ExecuteScheduleContractInput): Promise<ScheduleContractExecutionResult> {
