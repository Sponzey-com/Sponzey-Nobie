...
export function normalizeScheduleMaxRetries(value: number | null | undefined): number {
  if (!Number.isFinite(value)) return DEFAULT_SCHEDULE_RETRY_POLICY.maxRetries
  return Math.max(0, Math.min(10, Math.floor(value ?? DEFAULT_SCHEDULE_RETRY_POLICY.maxRetries)))
}

export function computeScheduleRetryDelayMs(attempt: number, policy: Partial<ScheduleRetryPolicy> = {}): number {
