...
import type { ExpectedOutputContract } from "../contracts/sub-agent-orchestration.js"
...
import {
  ...
  type NodeRuntimeAuthorityDecision,
} from "./authority-checker.js"
import {
  dispatchChildWorkOrders,
  type ChildDispatchSummary,
  type ChildRuntimeRunner,
...
import {
  ...
  type NodeExhaustionCheckResult,
} from "./exhaustion-checker.js"
import {
  generateFailureReport,
} from "./failure-report.js"
import {
  checkNodeRuntimePermission,
  type NodeRuntimePermissionDecision,
} from "./permission-checker.js"
import {
  buildNodeRecoveryReview,
  type NodeRecoveryControllerOptions,
...
import {
  ...
  type AggregatedNodeValidationResult,
} from "./validation.js"
import {
  dispatchPlannedNodeTools,
  type NodeToolExecutionSummary,
  type TopologyToolDispatcher,
...
export type NodeRuntimeInputValidationResult =
  | { ok: true; issues: [] }
  | { ok: false; issues: NodeRuntimeInputValidationIssue[] }
...
export type NodeRuntimeSelfExecutor =
  (context: NodeRuntimeSelfExecutionContext) => NodeRuntimeSelfExecutionResult | Promise<NodeRuntimeSelfExecutionResult>
...
export async function runNodeRuntime(input: RunNodeRuntimeInput): Promise<NodeRuntimeExecutionResult> {
  ...
  const stateTransitions: NodeRuntimeStateTransition[] = []
  const traceEvents: TraceEvent[] = []
  let childDelegation: ChildDispatchSummary | undefined
  let toolExecution: NodeToolExecutionSummary | undefined
  let aggregation: AggregationResult | undefined
  let validation: AggregatedNodeValidationResult | undefined
  ...
  const workOrderTraceContext = (): EnterpriseMetadata => buildWorkOrderTraceContext({
    workOrder,
    expectedOutputIds: envelope.expectedOutputs.map((expectedOutput) => expectedOutput.outputId),
  })
...
export function validateNodeRuntimeInputSchema(
  ...
  if (schema === undefined) return { ok: true, issues: [] }

  const issues: NodeRuntimeInputValidationIssue[] = []
  ...
  for (const [key, propertySchema] of Object.entries(properties)) {
    ...
    if (!Object.prototype.hasOwnProperty.call(workOrder.input, key) || workOrder.input[key] === undefined) continue
...
function defaultNodeRuntimeSelfExecute(context: NodeRuntimeSelfExecutionContext): NodeRuntimeSelfExecutionResult {
  return {
    status: "completed",
...
function defaultOutputValueForExpectedOutput(
  workOrder: WorkOrder,
  ...
  if (expectedOutput.outputId === `${workOrder.workOrderId}:expected-output-schema`) {
    const required = Array.isArray(workOrder.expectedOutputSchema.required)
      ? workOrder.expectedOutputSchema.required.filter((item): item is string => typeof item === "string" && item.trim().length > 0)
      : []
    return {
...
function buildDefaultChildRuntimeRunner(input: {
  now: () => number
  component: string
  parentChildDelegation: NodeRuntimeChildDelegationOptions
}): ChildRuntimeRunner {
...
function childDelegationOptionsForRecursiveChild(
  options: NodeRuntimeChildDelegationOptions,
...
function childDelegationRiskCodes(summary: ChildDispatchSummary | undefined): string[] {
...
function toolExecutionRiskCodes(summary: NodeToolExecutionSummary | undefined): string[] {
  if (summary === undefined) return []
...
function normalizeSelfExecutionResult(
  selfExecution: NodeRuntimeSelfExecutionResult,
  ...
  for (const expectedOutput of expectedOutputs) {
    if (outputsById.has(expectedOutput.outputId)) continue
    outputs.push({
      outputId: expectedOutput.outputId,
      status: "missing",
    })
...
function decideSelfExecutionReport(input: {
  ...
  selfExecution: NodeRuntimeSelfExecutionResult
  outputs: NodeResultOutput[]
}): { status: NodeResultStatus; reasonCode: string; risksOrGaps: string[] } {
...
function finalStateForNodeResultStatus(status: NodeResultStatus): NodeRuntimeState {
...
function missingOutputsForExpected(expectedOutputs: ExpectedOutputContract[]): NodeResultOutput[] {
...
function getNodeRuntimeInputSchema(nodeContractSnapshot: NodeContract): Record<string, EnterpriseMetadataValue> | undefined {
...
function enterpriseValueMatchesSchemaType(value: EnterpriseMetadataValue | undefined, expectedType: string): boolean {
  switch (expectedType) {
    case "string":
