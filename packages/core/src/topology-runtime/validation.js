export function validateAggregatedNodeResult(input) {
    ...
    for (const aggregationIssue of input.aggregation.issues) {
        if (aggregationIssue.code === "duplicate_output_removed")
            continue;
        issues.push({
            code: aggregationIssue.code,
            reasonCode: aggregationIssue.reasonCode,
...
export function validationStatusToNodeResultStatus(status) {
    if (status === "valid")
        return "completed";
...
function decideValidationStatus(issues, allowPartialSuccess) {
    if (issues.some((issue) => issue.reasonCode === "success_criterion_unmet"))
        return "failed_candidate";
    if (issues.some((issue) => issue.severity === "blocked")) {
        return allowPartialSuccess ? "partial_success" : "failed_candidate";
    }
    if (issues.some((issue) => issue.severity === "needs_revision"))
        return "needs_revision";
...
function validateOutputSchemaValue(workOrder, output) {
    ...
    for (const key of required) {
        if (Object.prototype.hasOwnProperty.call(output.value, key) && output.value[key] !== undefined)
            continue;
        issues.push({
...
function expectedSchemaOutputId(workOrder) {
    return `${workOrder.workOrderId}:expected-output-schema`;
}
function validationIssuesToRisks(issues) {
    return issues.map((issue) => {
        if (issue.criterionId !== undefined)
            return `${issue.reasonCode}:${issue.criterionId}`;
        if (issue.outputId !== undefined)
            return `${issue.reasonCode}:${issue.outputId}`;
        if (issue.sourceIds !== undefined && issue.sourceIds.length > 0)
            return `${issue.reasonCode}:${issue.sourceIds.join(",")}`;
