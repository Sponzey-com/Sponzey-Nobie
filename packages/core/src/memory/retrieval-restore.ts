...
import {
  ...
  insertMemoryCapsule,
  insertMemoryCapsuleRollup,
  insertMemoryCapsuleSource,
  insertMemoryRecallEvent,
  listMemoryCapsuleRollups,
  listMemoryCapsulesForOwner,
  type MemorySearchFilters,
  type TaskContinuitySnapshot,
} from "../db/index.js"
import type { MemoryCapsule, MemoryCapsuleArtifactRef, MemoryCapsuleOwnerScope } from "./capsule.js"
...
function extractLatestUserQuery(messages: Message[]): string | undefined {
...
function isRollupCapsule(capsule: MemoryCapsule): boolean {
...
function sameOwnerScope(
  left: MemoryCapsuleOwnerScope,
  right: MemoryCapsuleOwnerScope,
): boolean {
...
function renderRecentCapsulesSection(capsules: MemoryCapsule[]): string | undefined {
...
function isSameSessionRecall(result: MemoryChunkSearchResult, currentSessionId: string | undefined): boolean {
  if (!currentSessionId) return false
  if (!["session", "short-term", "flash-feedback"].includes(result.chunk.scope)) return false
  return result.chunk.owner_id === currentSessionId
}

function matchesChannelThread(
  result: MemoryChunkSearchResult,
  ...
  if (channelKey && storedChannelKey && channelKey !== storedChannelKey) return false
  if (threadKey && storedThreadKey && threadKey !== storedThreadKey) return false
  return true
}
...
export async function buildPromptTimeRecallContext(input: {
  ...
  const results = candidateResults.filter((result) => {
    ...
    if (!allowed) blockedReasonCodes.push("cross_channel_thread_restore_blocked")
    return allowed
  })
  const sameSessionResultCount = results.filter((result) => isSameSessionRecall(result, input.sessionId)).length
  const promptBlockBase = buildMemoryInjectionContext(
    results as Parameters<typeof buildMemoryInjectionContext>[0],
    { maxChunks: limit, maxChars: 1_000, maxChunkChars: 280 },
  )
