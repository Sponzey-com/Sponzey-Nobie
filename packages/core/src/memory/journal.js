...
export function condenseMemoryText(text, maxChars = MAX_SUMMARY_CHARS) {
...
export function extractFocusedErrorMessage(text, maxChars = MAX_SUMMARY_CHARS) {
...
function buildStoredContent(text) {
    const normalized = normalizeWhitespace(text);
    if (normalized.length <= MAX_STORED_CONTENT_CHARS)
        return normalized;
    return `${normalized.slice(0, MAX_STORED_CONTENT_CHARS).trimEnd()}…`;
}
function defaultTitle(kind, summary) {
    const prefix = (() => {
        switch (kind) {
            case "instruction":
                return "instruction";
...
function buildFtsQuery(query) {
    ...
    const sanitized = normalizeWhitespace(query)
        ...
        .trim();
    const tokens = Array.from(new Set(sanitized
        .split(/\s+/)
