const MEMORY_CAPSULE_FORBIDDEN_PATTERNS = [
    {
...
function findForbiddenReasonCodes(capsule) {
    const reasonCodes = new Set();
    for (const value of collectCapsuleStrings(capsule)) {
        for (const entry of MEMORY_CAPSULE_FORBIDDEN_PATTERNS) {
            if (entry.pattern.test(value))
                reasonCodes.add(`forbidden_capsule_content:${entry.code}`);
        }
    }
    return [...reasonCodes];
}
export function normalizeMemoryCapsuleOwnerScope(scope) {
    const normalized = {
    ...
    if (requestGroupId)
        normalized.requestGroupId = requestGroupId;
    if (lineageId)
        normalized.lineageId = lineageId;
    if (channelKey)
        normalized.channelKey = channelKey;
    if (threadKey)
        normalized.threadKey = threadKey;
    return normalized;
}
export function normalizeMemoryCapsule(input) {
    const normalized = {
...
export function validateMemoryCapsule(input, options = {}) {
    ...
    if (!capsule.capsuleId.trim())
        reasonCodes.push("capsule_id_missing");
    if (!capsule.ownerScope.ownerId.trim())
        reasonCodes.push("owner_id_missing");
    if (!capsule.summary.trim())
        reasonCodes.push("summary_missing");
...
export function applyMemoryCapsuleDeterministicState(input) {
...
export function buildSessionSnapshotProjectionFromMemoryCapsule(input) {
    const capsule = normalizeMemoryCapsule(input);
    if (!capsule.ownerScope.sessionId)
        return undefined;
...
export function buildTaskContinuityProjectionFromMemoryCapsule(input) {
    const capsule = normalizeMemoryCapsule(input);
    if (!capsule.ownerScope.lineageId)
        return undefined;
