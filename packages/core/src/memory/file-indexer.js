...
export class FileIndexer {
    async indexFile(filePath) {
        ...
        const existing = db
            .prepare("SELECT mtime FROM file_chunks WHERE file_path = ? LIMIT 1")
            .get(filePath);
    ...
    searchByText(query, limit = 5) {
        ...
        try {
            return db
                .prepare(`SELECT fc.file_path, fc.chunk_index, fc.content, rank as score
    ...
    async searchByVector(query, limit = 5) {
        ...
        const db = getDb();
        const rows = db
            .prepare("SELECT id, file_path, chunk_index, content, embedding FROM file_chunks WHERE embedding IS NOT NULL")
            .all();
