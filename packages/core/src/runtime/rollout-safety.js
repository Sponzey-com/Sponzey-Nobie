...
export function ensureRolloutSafetyTables(db) {
    db.exec(`
...
export function listFeatureFlags(db = getDb()) {
    ensureRolloutSafetyTables(db);
    const rows = db.prepare(`SELECT * FROM runtime_feature_flags ORDER BY feature_key ASC`).all();
    return mergeFeatureRows(rows);
}
export function getFeatureFlag(featureKey, db = getDb()) {
...
export function setFeatureFlagMode(input) {
    ...
    db.prepare(`INSERT INTO runtime_feature_flags
     ...
     ON CONFLICT(feature_key) DO UPDATE SET
       ...
       evidence_json = excluded.evidence_json`).run(input.featureKey, input.mode, compatibilityMode ? 1 : 0, now, input.updatedBy ?? null, input.reason ?? null, input.evidence ? stableStringify(input.evidence) : null);
    insertAuditLog({
        timestamp: now,
        session_id: null,
...
export function shouldUseNewPath(flag) {
    return flag.mode === "enforced";
}
export function shouldShadowWrite(flag) {
    return flag.mode === "shadow" || flag.mode === "dual_write" || flag.mode === "enforced";
}
export function shouldReadCompatibilityPath(flag) {
...
export function recordRolloutEvidence(input) {
    ...
    db.prepare(`INSERT INTO rollout_evidence
     (id, created_at, feature_key, mode, stage, status, run_id, request_group_id, summary, detail_json)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, input.now ?? Date.now(), input.featureKey, mode, input.stage, input.status ?? "ok", input.runId ?? null, input.requestGroupId ?? null, input.summary, input.detail ? stableStringify(input.detail) : null);
    return id;
}
export function recordShadowCompare(input) {
    ...
    db.prepare(`INSERT INTO rollout_shadow_compares
     (id, created_at, feature_key, target_kind, target_id, run_id, request_group_id,
      old_hash, new_hash, match, summary, detail_json)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, input.now ?? Date.now(), input.featureKey, input.targetKind, input.targetId ?? null, input.runId ?? null, input.requestGroupId ?? null, oldHash, newHash, matched ? 1 : 0, summary, stableStringify({ ...(input.detail ?? {}), oldHash, newHash }));
    const controlEventId = insertControlEvent({
...
function readRows(db, sql, fallback) {
    try {
...
export function buildRolloutSafetySnapshot(dbPath = PATHS.dbFile) {
    ...
    try {
        const featureRows = tableExists(db, "runtime_feature_flags")
            ? readRows(db, `SELECT * FROM runtime_feature_flags ORDER BY feature_key ASC`, [])
            : [];
        const active = tableExists(db, "migration_locks")
            ? db.prepare(`SELECT * FROM migration_locks WHERE status = 'active' ORDER BY updated_at DESC LIMIT 1`).get() ?? null
            : null;
        const latest = tableExists(db, "migration_locks")
            ? db.prepare(`SELECT * FROM migration_locks ORDER BY updated_at DESC LIMIT 1`).get() ?? null
            : null;
        const shadowCounts = tableExists(db, "rollout_shadow_compares")
            ? db.prepare(`SELECT COUNT(*) AS total, SUM(CASE WHEN match = 0 THEN 1 ELSE 0 END) AS mismatch_count FROM rollout_shadow_compares`).get()
            : { total: 0, mismatch_count: 0 };
        const recentMismatches = tableExists(db, "rollout_shadow_compares")
            ? readRows(db, `SELECT * FROM rollout_shadow_compares WHERE match = 0 ORDER BY created_at DESC LIMIT 10`, [])
            : [];
        const evidenceCounts = tableExists(db, "rollout_evidence")
            ? db.prepare(`SELECT COUNT(*) AS total, SUM(CASE WHEN status = 'warning' THEN 1 ELSE 0 END) AS warning_count, SUM(CASE WHEN status = 'blocked' THEN 1 ELSE 0 END) AS blocked_count FROM rollout_evidence`).get()
            : { total: 0, warning_count: 0, blocked_count: 0 };
        const latestEvidence = tableExists(db, "rollout_evidence")
            ? readRows(db, `SELECT * FROM rollout_evidence ORDER BY created_at DESC LIMIT 10`, [])
            : [];
        return {
...
//# sourceMappingURL=rollout-safety.js.map