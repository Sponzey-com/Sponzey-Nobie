    ...
    <div className="space-y-5 rounded-2xl border border-stone-200 bg-white p-5">
      ...
      <div className="space-y-4">
        {value.servers.map((server) => {
          ...
          return (
            <div key={server.id} className="rounded-2xl border border-stone-200 bg-stone-50 p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <div className="mt-4 grid gap-4 md:grid-cols-2">
                <div>
                  ...
                  <input
                    className="input"
                    value={server.name}
                    onChange={(event) => updateServer(server.id, { name: event.target.value, status: "disabled", reason: undefined, tools: [] })}
                    placeholder={text("예: file_tools", "Example: file_tools")}
                  />
                  {serverErrors?.name ? <p className="mt-2 text-xs leading-5 text-red-600">{serverErrors.name}</p> : null}
                </div>
                <div>
                  ...
                  {serverErrors?.transport ? <p className="mt-2 text-xs leading-5 text-red-600">{serverErrors.transport}</p> : null}
                </div>
              </div>
              ...
              <div className="mt-4 grid gap-4 md:grid-cols-2">
                ...
                <label className="flex items-center gap-3 text-sm font-medium text-stone-700">
                  <input
                    ...
                    onChange={(event) => updateServer(server.id, { required: event.target.checked })}
                  />
              ...
              {server.transport === "stdio" ? (
                <div className="mt-4 space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(event) => updateServer(server.id, { command: event.target.value, status: "disabled", reason: undefined, tools: [] })}
                      placeholder={text("예: node", "Example: node")}
                    />
                    {serverErrors?.command ? <p className="mt-2 text-xs leading-5 text-red-600">{serverErrors.command}</p> : null}
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(event) => updateServer(server.id, { argsText: event.target.value, status: "disabled", reason: undefined, tools: [] })}
                  ...
                  <div>
                    ...
                    <input
                      className="input font-mono"
                      value={server.cwd}
                      onChange={(event) => updateServer(server.id, { cwd: event.target.value, status: "disabled", reason: undefined, tools: [] })}
                ...
                <div className="mt-4">
                  ...
                  <input
                    className="input font-mono"
                    value={server.url}
                    onChange={(event) => updateServer(server.id, { url: event.target.value, status: "disabled", reason: undefined, tools: [] })}
                    placeholder={text("예: http://127.0.0.1:3001", "Example: http://127.0.0.1:3001")}
                  />
                  {serverErrors?.url ? <p className="mt-2 text-xs leading-5 text-red-600">{serverErrors.url}</p> : null}
                </div>
              )}
              ...
              {serverErrors?.status ? <p className="mt-2 text-xs leading-5 text-red-600">{serverErrors.status}</p> : null}
