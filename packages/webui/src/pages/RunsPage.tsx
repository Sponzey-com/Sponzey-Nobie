  ...
  return (
    <div className="rounded-2xl border border-stone-200 bg-white p-5">
      ...
      <div className="mt-4 flex flex-wrap gap-2 text-xs">
        <Link
          to="/advanced/audit"
          className="rounded-xl border border-stone-200 px-3 py-2 font-semibold text-stone-700 hover:bg-stone-50"
        >
    ...
    <div className="rounded-2xl border border-stone-200 bg-white p-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-xs leading-5 text-stone-500">
            {text(
              ...
              "Summarizes repeated issues and stale waiting states.",
            )}
          </div>
        </div>
        <button
        ...
        >
          {cleanupRunning
            ? text("정리 중", "Cleaning")
            : text("오래된 대기 정리", "Clean stale waits")}
        </button>
      </div>
              ...
              {[
                ...
                .map((item) => (
                  ...
                  >
                    ...
                    <div className="mt-1 break-words [overflow-wrap:anywhere]">
                      {item.runId} · {formatTime(item.updatedAt)}
                    </div>
                  </div>
                ))}
            </div>
    ...
    <div className="rounded-2xl border border-stone-200 bg-white p-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-xs leading-5 text-stone-500">
            {text(
              "답변에 사용된 메모리 chunk와 source checksum을 운영 진단용으로 표시합니다.",
              "Shows memory chunks and source checksums used by the answer for diagnostics.",
            )}
          </div>
        </div>
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <span className="font-semibold text-stone-900">
                    {displayText(trace.scope ?? "unknown")} · {displayText(trace.result_source)}
                  </span>
                  <span>{formatTime(trace.created_at)}</span>
                </div>
                <div className="mt-2 grid gap-2 sm:grid-cols-2">
                  <div>
                    {text("청크", "Chunk")}: {displayText(trace.chunk_id ?? "n/a")}
                  </div>
                  <div>
                    {text("체크섬", "Checksum")}: {displayText(checksum)}
                  </div>
                  <div>
                    {text("점수", "Score")}: {score}
                  </div>
    ...
    <div className="rounded-2xl border border-stone-200 bg-white p-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-xs leading-5 text-stone-500">
            {text(
              "검색, 후보값, 검증, 전달 상태를 control timeline에서 재구성합니다.",
              "Reconstructs search, candidates, verdicts, and delivery from the control timeline.",
            )}
          </div>
        </div>
        <span className="rounded-full bg-stone-100 px-2.5 py-1 text-[11px] font-semibold text-stone-600">
      ...
      {summary ? (
        <div className="mt-4 grid gap-2 sm:grid-cols-3">
          ...
          <div className="rounded-xl border border-stone-200 bg-stone-50 px-3 py-2 text-xs text-stone-600">
            {text("후보/검증", "Candidates/Verdicts")}: {summary.candidates}/{summary.verdicts}
          </div>
          <div className="rounded-xl border border-stone-200 bg-stone-50 px-3 py-2 text-xs text-stone-600">
            {text("전달", "Delivery")}: {displayText(summary.finalDeliveryStatus ?? "unknown")}
          </div>
          <div className="rounded-xl border border-stone-200 bg-stone-50 px-3 py-2 text-xs text-stone-600">
            {text("중복 억제", "Dedupe")}: {summary.dedupeSuppressed}
          </div>
          <div className="rounded-xl border border-stone-200 bg-stone-50 px-3 py-2 text-xs text-stone-600">
            {text("충돌", "Conflicts")}: {summary.conflicts}
          </div>
          <div className="rounded-xl border border-stone-200 bg-stone-50 px-3 py-2 text-xs text-stone-600">
            {text("중단 사유", "Stop reason")}: {displayText(summary.stopReason ?? "none")}
          </div>
        </div>
      ) : null}
      <div className="mt-4 space-y-2">
        {events.length > 0 ? (
          events.slice(0, 12).map((event) => {
            const sourceLabel = [event.source.toolName, event.source.method, event.source.domain]
              .filter(Boolean)
              .join(" · ")
            ...
            return (
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <span className="font-semibold text-stone-900">
                    {displayText(event.kind)} · {displayText(event.eventType)}
                  </span>
                ...
                {sourceLabel || verdictLabel || event.duplicate ? (
                  <div className="mt-2 flex flex-wrap gap-2 text-[11px] text-stone-500">
                    ...
                    {verdictLabel ? (
                      <span className="rounded-full bg-white px-2 py-1">
                        {displayText(verdictLabel)}
                      </span>
                    ) : null}
                    {event.duplicate ? (
                      <span className="rounded-full bg-amber-50 px-2 py-1 text-amber-700">
                        duplicate {event.duplicate.kind} #{event.duplicate.occurrence}
                      </span>
                    ) : null}
                  </div>
                ) : null}
    ...
    try {
      ...
      setCleanupNotice({
        ...
        auditHint: text(
          ...
          "Failed to clean stale waits. Check operational health and audit logs.",
        ),
      })
    } finally {
