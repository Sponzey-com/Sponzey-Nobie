...
- 현재는 slash command가 아니면 대부분 LLM intake 분석으로 들어간다.
...
- 따라서 자연어 영문 요청은 곧바로 intake LLM으로 간다.
- 자연어를 LLM으로 해석하는 방향 자체는 맞다.
- 문제는 자연어를 LLM으로 해석하는 것 자체가 아니라, 단순 요청까지 접수 응답 전에 무거운 intake와 재분석 루프를 모두 거친다는 데 있다.
...
- 영문 자연어가 느리다: `/` 명령어가 아니면 intake LLM으로 가서
...
### 2. 자연어 해석은 LLM 중심으로 유지

- 자연어 요청은 명령어처럼 deterministic parser로 처리하기보다, LLM이 의미를 해석하는 것이 더 적절하다.
- 정규화는 `명령 변환`이 아니라 `보조 컨텍스트 생성` 수준으로 제한하는 편이 맞다.
- 즉 `원문 -> 정규화된 참고 정보 -> LLM 해석` 구조가 바람직하다.
- 자연어를 바로 명령 체계로 환원하면 에이전트 성격이 리모트 컨트롤러처럼 변질될 수 있다.
- 따라서 개선 방향은 `LLM 제거`가 아니라, `LLM 해석 이전/이후의 구조를 가볍게 하고 실패 루프를 줄이는 것`이어야 한다.
...
- 다만 이 구조화는 자연어를 강제로 명령문으로 바꾸기 위한 것이 아니라, LLM이 해석한 결과를 후속 실행 단계에서 안정적으로 전달하기 위한 표현이어야 한다.
